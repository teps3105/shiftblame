...
1. **更新 SLUG §3 目前節點表**：確認「節點」「最近交付」欄反映當前實際狀態（如 `開發` + 正在做的功能與 commit）。
2. **更新 SLUG §7 交接摘要**：以 3～5 行白話記錄當前工作落點：
...
- sb-save **只更新 SLUG.md**，不建立快照檔、不動 G1／G2／G3 內容、不動 repo。
- sb-save 是**顯式記錄**；即使不執行 sb-save，`/sb-resume` 仍能從 SLUG + G1~G3 恢復部分資訊，但可能遺漏「進行到哪」的細節。sb-save 確保落點完整，讓 resume 無縫。
