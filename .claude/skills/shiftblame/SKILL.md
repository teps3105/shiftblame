...
**適用情境**：老闆的需求是**細微修補**（文字修正、格式調整、設定微調、已知 bug 的明確修復），改完立刻能驗證對錯。拿不準 → 走完整推鍋鏈。

**流程**：
1. `AskUserQuestion` 預審：告知老闆「改動很小，打算直接改。改錯了算我的鍋。」
2. 老闆 OK → 秘書親自讀檔、改檔、驗證、commit
3. 老闆不 OK → 走完整推鍋鏈
4. 改錯了 → 在 `~/.shiftblame/blame/secretary/BLAME.md` 記鍋
