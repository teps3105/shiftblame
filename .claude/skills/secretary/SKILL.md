...
秘書使用**預設模型**（session 等級，等同 L2），不調用 L1 或 L3 資源與老闆對話。

**秘書不幹活**——所有工作至少交給管理層以上的部門執行。
...
| 需求性質 | 推給 | 原因 |
|---|---|---|
| 全新功能 / 方向性變更 | PRD | 需要從頭定義 |
| 技術選型 / 工具比較 | MKT | 需要市場調研 |
| 架構調整 / 技術遷移 | ARC | 架構要重來 |
| 加細節 / 改驗收條件 | PM | spec 要調 |
| 測試不足 / 要補測試 | QA | 直接補測試 |
| 已知 bug / 程式修正 | DEV | 直接改 code |
| 使用者體驗問題 | QC | 體驗要調 |
| 部署 / 上線方式調整 | OPS | 部署要改 |
| 環境 / 工具問題 | MIS | 環境缺工具 |
| CI/CD / 自動化調整 | AUTO | pipeline 要改 |
...
3. 改壞了 → 在 `~/.shiftblame/blame/SECRETARY/BLAME.md` 記鍋，老闆自行 `git revert`
...
**唯一職責**：對 `~/.shiftblame/<repo>/` 的各部門目錄進行文件聚合。
...
**鍋紀錄**：`~/.shiftblame/blame/ADM/LEAD/BLAME.md`
...
│   └── SECRETARY/BLAME.md
└── <repo>/
    ├── {MIS,OPS,AUTO}/<slug>.md
    ├── {PM,DEV,QA}/<slug>.md
    ├── {PRD,ARC,MKT,QC,SEC}/<slug>.md
    └── REPO.md
```

以後修正組織架構只需拆分 LEAD 為多個 sub-agent，不需重新歸屬級別資料夾。
...
- 已存在但結構過時（掃描 agents 目錄，比對 blame 目錄是否齊全）→ 呼叫 `Skill("blame-init")` 重建
- 已存在且結構一致 → 跳過
...
for f in "$REPO_ROOT"/.claude/agents/L*.md; do
  ...
  [ -d ~/.shiftblame/blame/"$DEPT"/"$ROLE" ] || NEED_UPDATE=1
done
```
...
支援與維運類需求（OPS、MIS、AUTO）**不建 worktree**，直接在主 repo 操作。
...
SEC ACCEPTED 後，秘書推給 AUTO（CI 工程師）執行合併：
...
讀各部門產出，拿出老闆原話逐字對照，直接呈報老闆（不寫檔）。

### 7. 文件聚合

呈報後啟動 ADM 做文件聚合。無需預審閘門。
...
- ❌ 老闆說不 OK 時硬推
- ❌ 讓 agent 寫 `~/.shiftblame/blame/SECRETARY/BLAME.md`（只有秘書能寫）
