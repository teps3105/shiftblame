...
| 既有功能的架構調整 / 技術遷移 | L3 ARC | PRD 不變，架構要重來 |
| 既有功能加細節 / 改驗收條件 | L2 PM | PRD + ARC 不變，PM 要調 |
| 測試不足 / 要補測試 | L2 QA | 上游文件都在，直接補測試 |
| 已知 bug / 程式邏輯修正 | L2 DEV | 直接改 code |
| 使用者體驗問題 | L3 QC | 功能沒壞，體驗要調 |
| 部署 / 上線方式調整 | L1 OPS | 程式沒問題，部署要改 |
...
|---|------|------|---------|
| 1 | L3 | PRD  | 把老闆原話轉 PRD |
| 2 | L3 | ARC  | 技術選型、模組拓撲、檔案結構、介面簽章、部署方案 |
| — | L1 | MIS  | 讀 ARC 盤點環境、安裝工具（需老闆核准） |
| — | L1 | OPS  | 基建需求（若 MIS 轉介） |
| 3 | L2 | PM   | 功能拆解、驗收條件、任務依賴 |
| 4 | L2 | QA   | 拆分測試給三位測試工程師（單元/整合/E2E），產出測試設計（TDD 紅） |
| 5 | L2 | DEV  | 寫最小實作讓測試全綠（TDD 綠），子 agent：fe + be |
| 6 | L3 | QC   | 執行 E2E + 邊緣 + 模糊測試，撰寫品管報告 |
| 7 | L3 | SEC  | 整條鏈路驗收 + 紅藍隊安全掃描，回傳 ACCEPTED / REJECTED / ALERT |
| 8 | L1 | OPS  | 在 main 依 ARC 方案實際上線 |
...
L3 PRD 到 L3 QC 在共享 worktree 的 feature 分支上 append-only commit。秘書合併到 main 後，L3 SEC 在 main 上做驗收 + 安全掃描。L1 OPS 在 main 上部署。L1 MIS 在 ARC 後介入做環境準備。
...
檢查 `$REPO_ROOT/.shiftblame/` 是否存在且 symlink 正確：
- 不存在 → 呼叫 `Skill("shiftblame-init")` 執行初始化，完成後繼續
- 已存在 → 跳過，直接進入 step 1
...
2. **原話逐字保存**（最後一步要用）
3. 從需求中提 kebab-case **slug**
4. Glob 檢查 `~/.shiftblame/<repo>/L3/PRD/<slug>.md` 是否存在
5. 建立共享 worktree + symlink：
   ```bash
   ...
   ```
6. 記下 `WORKTREE_PATH`、`BRANCH`、老闆原話
...
|---|------|------|------|
| 1 | L3 | PRD  | 老闆原話（`<<< ... >>>`） |
| 2 | L3 | ARC  | `L3/PRD/<slug>.md` |
| — | L1 | MIS  | `L3/ARC/<slug>.md` |
| — | L1 | OPS  | MIS 報告中的 L1 轉介項目（若有） |
| 3 | L2 | PM   | `L3/PRD` + `L3/ARC` |
| 4 | L2 | QA   | `L3/ARC` + `L2/PM` |
| 5 | L2 | DEV  | `L2/QA` + `L3/ARC` |
| 6 | L3 | QC   | `L2/DEV` |
| 7 | L3 | SEC  | `L3/QC`（worktree 稽核）+ 合併後 main HEAD（安全掃描） |
| 8 | L1 | OPS  | 合併後 main HEAD hash + 主 repo 路徑 |
...
記下合併後 main HEAD hash，交棒 L3 SEC（安全掃描階段）。掃描 ACCEPTED 後再交棒 L1 OPS 部署。feature 分支保留。

### 10. 秘書最終對照

L1 OPS 回報 SUCCESS 後：

1. Read `~/.shiftblame/<repo>/{L3/PRD,L2/PM,L3/SEC,L1/OPS}/<slug>.md`
2. 拿出老闆原話逐字稿，逐句對照
...
**prompt 要點**：
1. 掃描 `~/.shiftblame/<repo>/` 下各層各部門目錄（L1/MIS、L1/OPS、L1/AUTO、L2/PM、L2/DEV、L2/QA、L3/PRD、L3/ARC、L3/QC、L3/SEC）
...
- ❌ 在預告或選項裡暴露角色名
- ❌ 老闆說不 OK 時硬推 / 偷懶一律退回 L3 PRD
- ❌ step 10 之前讀 `~/.shiftblame/<repo>/L1~L3/` 的產出檔案
