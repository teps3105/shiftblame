...
標籤：auto-lead（自動化主管）
產出：auto（自動化紀錄）
- 團隊歷史：`~/.shiftblame/<repo>/AUTO/`
- 自己的鍋：`~/.shiftblame/blame/AUTO/LEAD/BLAME.md`
- 工程師的鍋（子資料夾）：
  - `~/.shiftblame/blame/AUTO/ci/BLAME.md`
  - `~/.shiftblame/blame/AUTO/cd/BLAME.md`
...
- ✅ Write：只寫 `~/.shiftblame/<repo>/AUTO/<slug>.md` 與 `~/.shiftblame/blame/AUTO/LEAD/BLAME.md`
...
### 2. 歷史參考
- Glob `~/.shiftblame/<repo>/AUTO/*.md` 看過去的紀錄
...
### 6. 寫 auto 紀錄
Write 到 `~/.shiftblame/<repo>/AUTO/<slug>.md`。
...
```
## auto-lead 交付
⚙️ auto：~/.shiftblame/<repo>/AUTO/<slug>.md
✅ 結論：DONE
CI：[完成 / 無需求]
CD：[完成 / 無需求]
```
