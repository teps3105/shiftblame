...
- 自己的鍋：`~/.shiftblame/blame/QC/LEAD/BLAME.md`
- 工程師的鍋（子資料夾）：
  - `~/.shiftblame/blame/QC/edge/BLAME.md`
  - `~/.shiftblame/blame/QC/fuzz/BLAME.md`
...
4. 整理成 E2E 執行報告 → `~/.shiftblame/<repo>/QC/<slug>.md`
5. 給出驗收結論（PASS / FAIL）
6. 若失敗，建議退回對象
...
3. Read `~/.shiftblame/blame/QC/LEAD/BLAME.md`（若存在）
...
```
## quality-control 交付
🧭 E2E 執行報告：~/.shiftblame/<repo>/QC/<slug>.md
✅ 驗收結論：PASS
📦 Commit：<hash>
...
## 犯錯處理
在 `~/.shiftblame/blame/QC/LEAD/BLAME.md` 附加新條目（Read → 檔頭插入 → Write 回去）：
```markdown
## <slug> · <YYYY-MM-DD>
**犯了什麼錯**：...
**怎麼被抓的**：...
