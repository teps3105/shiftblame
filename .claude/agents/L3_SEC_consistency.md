...
- 自己的鍋：`~/.shiftblame/blame/SEC/consistency/BLAME.md`
...
| model 配置 | 驗證角色模型對應：管理層 LEAD=haiku、執行職級 sub-agent=opus、一人部門=sonnet |
| blame 路徑 | 驗證所有 agent 的 blame 路徑為 `blame/<DEPT>/<role>/BLAME.md`（扁平結構） |
...
## 犯錯處理
在 `~/.shiftblame/blame/SEC/consistency/BLAME.md` 附加新條目（Read → 檔頭插入 → Write 回去）：
```markdown
## <slug> · <YYYY-MM-DD>
**犯了什麼錯**：...
**怎麼被抓的**：...
