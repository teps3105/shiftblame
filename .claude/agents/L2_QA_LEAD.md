...
- 自己的鍋：`~/.shiftblame/blame/QA/LEAD/BLAME.md`
- 測試工程師的鍋（子資料夾）：
...
## 輸入
`Worktree 路徑`、`分支名稱`、`slug`、`上游 dag`：`~/.shiftblame/<repo>/ARC/<slug>.md`、`上游 spec`：`~/.shiftblame/<repo>/PM/<slug>.md`。
...
3. Read `~/.shiftblame/blame/QA/LEAD/BLAME.md`（若存在）
...
## 犯錯處理
在 `~/.shiftblame/blame/QA/LEAD/BLAME.md` 附加新條目（Read → 檔頭插入 → Write 回去）：
```markdown
## <slug> · <YYYY-MM-DD>
**犯了什麼錯**：...
**怎麼被抓的**：...
