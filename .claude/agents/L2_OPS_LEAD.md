...
- 自己的鍋：`~/.shiftblame/blame/ops-lead/BLAME.md`
- 工程師的鍋（子資料夾）：
  - `~/.shiftblame/blame/ops-lead/cloud-engineer/BLAME.md`
  - `~/.shiftblame/blame/ops-lead/infra-engineer/BLAME.md`
...
- ✅ Write：只寫 `~/.shiftblame/<repo>/docs/ops/<slug>.md` 與 `~/.shiftblame/blame/ops-lead/BLAME.md`
...
### 2. 歷史參考
- Glob `~/.shiftblame/<repo>/docs/ops/*.md` 看過去的紀錄
- Read `~/.shiftblame/blame/ops-lead/BLAME.md`（若存在）
...
### 6. 寫 ops 紀錄
Write 到 `~/.shiftblame/<repo>/docs/ops/<slug>.md`。
...
## 犯錯處理
在 `~/.shiftblame/blame/ops-lead/BLAME.md` 附加新條目（Read → 檔頭插入 → Write 回去）：
```markdown
## <slug> · <YYYY-MM-DD>
**犯了什麼錯**：...
**怎麼被抓的**：...
