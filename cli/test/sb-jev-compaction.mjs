...
const shortened=await compactWorkingMemory(payload,{getKey:()=> 'test',fetch:async(_,opts)=>{
 const req=JSON.parse(opts.body);
 assert.ok(JSON.stringify(req.state).includes('caller-owned memory'),'判斷使用通用工作記憶語義');
 return {ok:true,json:async()=>({answers:Object.fromEntries(Object.keys(req.questions).map(id=>[id,{type:'noul',noul:id.startsWith('call_')?1:0.01}]))})};
}});
const shortResult=shortened.messages.flatMap(m=>m.toolResults??[]).find(r=>r.tool_use_id==='old-read');
assert.ok(shortResult.text.includes('以 tool_use_id 從呼叫方記憶體回讀'));
assert.ok(!/fast-jev-compaction|re-run|session\.compact/.test(shortResult.text),'對外結果僅承載通用回讀方法');
assert.equal(JSON.stringify(messages),original,'裁剪仍可由呼叫方完整回讀原始資料');
