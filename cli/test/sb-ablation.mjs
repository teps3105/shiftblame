...
ablation('G 檔寫入矩陣 checkGFileMatrix（1.7.3 段-檔承載）', () => {
  ...
  const payload = (script) => { const r = mkSandbox({ state: { node: 'test' }, files: { '.shiftblame/demo/001/G3.md': 'x' } }); const h = hookRun(script, { cwd: r, hook_event_name: 'PreToolUse', tool_name: 'Edit', tool_input: { file_path: join(r, '.shiftblame/demo/001/G3.md'), old_string: 'a', new_string: 'b' } }); rmSync(r, { recursive: true, force: true }); return h.status; };
  assert.equal(payload(GUARD), 2, 'intact：test 段（落地邊）寫 G3 被擋');
...
ablation('層間停靠鎖 checkLayerStopover（老闆決策邊 --boss-ok）', () => {
  ...
  const payload = (script) => { const r = mkSandbox({ state: { node: 'intent' } }); const h = hookRun(script, { cwd: r, hook_event_name: 'PreToolUse', tool_name: 'Bash', tool_input: { command: 'sb next audit' } }); rmSync(r, { recursive: true, force: true }); return h.status; };
  assert.equal(payload(GUARD), 2, 'intact：intent→audit 缺 --boss-ok 被擋');
...
ablation('審計痕跡標記 markAuditEvidence（1.7.3 審計先於研究）', () => {
  const neu = neutralize(GUARD, [['function markAuditEvidence(root, tool, cmd, toolInput) {\n  if (!root) return;', 'function markAuditEvidence(root, tool, cmd, toolInput) {\n  return; // ABLATED\n  if (!root) return;']]);
  const payload = (script) => { const r = mkSandbox({ state: { node: 'audit' } }); hookRun(script, { cwd: r, hook_event_name: 'PreToolUse', tool_name: 'Read', tool_input: { file_path: join(r, 'app.txt') } }); const d = !!stateOf(r).auditEvidence?.done; rmSync(r, { recursive: true, force: true }); return d; };
  assert.equal(payload(GUARD), true, 'intact：audit 段 repo 查證標記');
  assert.equal(payload(neu), false, 'ablated：拆掉後查證不標記（審計閘的鑰匙失效）');
});
...
ablation('CLI 老闆決策邊鑰匙閘 needsBossOk（CARD②③ CLI 層）', () => {
  const neu = neutralize(SB, [["const needsBossOk = (from, to) =>\n  (from === 'intent' && to === 'audit') || (from === 'plan' && to === 'test') || (from === 'verify' && to === 'done');", "const needsBossOk = (from, to) =>\n  false && ((from === 'intent' && to === 'audit') || (from === 'plan' && to === 'test') || (from === 'verify' && to === 'done')); // ABLATED"]]);
  const payload = (script) => { const r = mkSandbox({ state: { node: 'intent' }, files: { '.shiftblame/demo/001/G1.md': BDD_G1 } }); const h = cliRun(script, r, 'next', 'audit'); rmSync(r, { recursive: true, force: true }); return h.status; };
  assert.equal(payload(SB), 1, 'intact：intent→audit 缺 --boss-ok 被 CLI 擋');
  assert.equal(payload(neu), 0, 'ablated：拆掉後 CLI 層繞過決策邊（hooks 層獨撐）');
});

ablation('CLI 時點對抗宣告閘 adversarialEdge×SLUG 對照（CARD③ CLI 層）', () => {
  ...
  const payload = (script) => { const r = mkSandbox({ state: { node: 'plan' }, files: { '.shiftblame/demo/001/G1.md': BDD_G1, '.shiftblame/demo/001/G2.md': G2, '.shiftblame/demo/001/G3.md': G3, '.shiftblame/demo/SLUG.md': '# SLUG\n- 時點①對抗：完成\n' } }); const h = cliRun(script, r, 'next', 'test', '--boss-ok'); rmSync(r, { recursive: true, force: true }); return h.status; };
...
ablation('審計痕跡閘（audit→research 邊驗，1.7.3）', () => {
  const neu = neutralize(SB, [['if (st.node === \'audit\' && target === \'research\') {', 'if (false && st.node === \'audit\' && target === \'research\') { // ABLATED']]);
  const payload = (script) => { const r = mkSandbox({ state: { node: 'audit' }, files: { '.shiftblame/demo/001/G1.md': BDD_G1, '.shiftblame/demo/001/G2.md': G2 } }); const h = cliRun(script, r, 'next', 'research'); rmSync(r, { recursive: true, force: true }); return h.status; };
  assert.equal(payload(SB), 1, 'intact：零現況查證推進被擋（字面研究死路）');
  assert.equal(payload(neu), 0, 'ablated：拆掉審計閘後未審計即研究');
});
...
ablation('BDD 行為規格閘 validateG1Acceptance（1.7.3＋1.8.0 消融鍵）', () => {
  ...
  const payload = (script) => { const r = mkSandbox({ state: { node: 'audit', auditEvidence: { done: true } }, files: { '.shiftblame/demo/001/G1.md': badG1 } }); const h = cliRun(script, r, 'next', 'research'); rmSync(r, { recursive: true, force: true }); return h.status; };
