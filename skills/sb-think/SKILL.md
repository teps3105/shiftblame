...
階段完成、測試全綠、單一驗收通過、一次唯讀檢閱回傳、壓縮將至或老闆沉默都不是「需要老闆決策」。主對話須吸收結果、更新 `Goal／Core／Verified／Open／Next` 並立即續跑；進度用 commentary 回報，不得以 final 把局部完成誤報成整體終點。`1.0.x` 移除階段子代理後，這項原由上層隱性完成的吸收責任由主對話秘書明確承擔，不得為此恢復持久子代理。
