...
- **兩種觸發樣態（1.7.2）——主動觸發必須停等。** 老闆以 sb-think 調用形式輸入（`/sb-think`、`$sb-think` 或裸名 `sb-think` 開頭）＝主動觸發：理解以六欄完整形態呈現後**本輪即停**——行動凍結（hooks 硬擋寫入類與流程推進；唯讀查證、Skill 調用、tmp 傾倒自由），待老闆終審回覆；回覆為確認即分發執行，為修正則更新理解重呈現並仍停等再確認——呈現理解後自行推進＝違反停等。被動觸發（老闆一般輸入由機制路由 sb-think）＝理解宣告落流＋事後曝光，理解成立直接續跑（差異＝曝光時點，SKILL §0 兩種觸發樣態）。
