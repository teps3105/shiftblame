...
6.5. 秘書依據 MIS 差異報告更新 REPO.md（見 LIFECYCLE.md 步驟 0.5）
...
- RES 維護輪不走流程：當老闆指示為維護輪時，RES 獨立研究後交 MIS 執行變更與收尾。RES 維護輪不走完整流程。
...
用於日常維護操作（如模型切換呈報、版本升級、參數微調等）的輕量級運作模式。

- 不走 RES → MIS 完整流程
- 每次操作前須透過 AskUserQuestion 呈報老闆覆核，確認後才執行
- 適用場景：模型額度檢視與呈報、小規模定義檔修正、版本號更新等
- 與初等模式的區別：初等模式仍走 RES 研究 → MIS 收尾流程；日常運作模式跳過研究階段，直接由秘書呈報老闆後執行
