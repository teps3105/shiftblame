...
【步驟 2】依 task.md（本次循環版本）與 conclusion.md 執行分工任務（003+ 依上一輪 review.md 修正）
...
【步驟 2】監督主執行者的 result.md，**從自身面向**逐一驗證其中列出的每個項目是否確實完成：
...
**監督者面向分工（DEV 部門適用）：**
|- codex 面向：邏輯正確性 + 測試覆蓋度（靜態分析、代碼審查、邏輯分支完整性、測試覆蓋度、邊界條件處理）
|- gemini 面向：功能完整性 + 規格一致性（對照 PRD 驗證需求覆蓋、功能端到端完整性、規格偏差、遺漏功能）
