...
17. **選用外部角色整合（以 codex 為首例：反方複審／成果驗收／外部查證擔任者）**：未安裝 codex 時框架完整可用（反方由子代理/管理者內部擔任、驗收由管理者自驗、外部查證由內建 MCP/web 搜尋擔任）；已安裝且生效時，下列需要外部獨立性／利益衝突隔離的工作交 codex 子代理——反方獨立複審、實作成果驗收、code review、圖像識別／外部資訊查證（本地工具不足時）。codex 整合**預設以 `/codex:transfer` 傳遞完整 session 上下文**（讓 codex 自行推斷老闆意圖、不經管理者二手轉述），派任務為例外（見「一律走官方 slash command」）。與 shiftblame 定義檔明文要求的產出衝突時，shiftblame 定義檔條款優先。
    - **與固定雙子代理的關係（雙層）**：codex 不取代子代理反方、不是常駐第三軌。雙層運作——子代理先做初步反方（保留內部對抗），codex 再疊加獨立複審（外部利益衝突隔離）；正方仍由子代理/管理者撰寫。雙子代理結構不變，codex 是疊加的獨立複審工具。transfer（上下文移交）服務於框架內的 codex 驗證環節——讓 codex 看懂框架正在做的事，不改變管理者收斂權威與雙軌結構，session 主控權與收斂責任仍留管理者
    - **啟用判準**：判斷 codex 是否啟用，問「這件事以內部身份做會不會自我背書／利益衝突／超出本地工具能力」——是，則交 codex；否，則內部做
    - **一律走官方 slash command**：**transfer 預設優先**——codex 驗證（反方複審／成果驗收／查證）一律先用 `/codex:transfer` 把當前 session 完整上下文移交 codex，讓它從老闆原話、G1/G2 正反、收斂與決策理由自行推斷意圖，**不經管理者二手轉述**（二手轉述會使 codex 過度工程、誤解老闆意圖）；只在無需完整上下文的單點查證（查版本號、bundle size、單一外部事實等）才退回派任務模式：深度根因／大任務／反方複審標的「無 diff」→ `/codex:rescue`；反方對抗審查標的「已實作 diff」→ `/codex:adversarial-review`；一般 review → `/codex:review`；查狀態→`/codex:status`，取結果→`/codex:result <task-id>`。禁止手搓 `Agent(subagent_type:...)`、自 Bash 跑 companion、Monitor 輪詢。**派發後管理者主動查 `/codex:status` 確認進度、`/codex:result <task-id>` 取結果——codex companion 背景 task 不在 harness 追蹤範圍，不會自動發完成通知，禁止被動等待**（詳 CF-SOP #20）
    ...
    - **成果驗收門檻（slug PASS 前置，非 commit 前置）**：驗收把關的是「成果可否宣告完成／推進」，不是「可否 commit」。
        ...
        - **slug 結束前強制驗收（兜底）**：無論各 NNN 是否驗收，整條 slug 在老闆拍板 PASS 之前，本輪成果須交 codex 做獨立驗收（成果＋管理者驗收方法與數據交 `/codex:rescue` 或 `/codex:adversarial-review`）；diff 過大時 codex 可要求分切片補驗。**三事件時序**：管理者宣告完成→codex 驗收→老闆拍板 PASS（驗收落於宣告完成與 PASS 之間）。codex 判退修→不得宣告完成、不得推進 PASS，rework 後重驗（預設疊新 commit，可逆可追溯）。此為 slug PASS 前置，不是 NNN gate
