...
配套指令：`sb state`（目前節點與各下一步前置條件）、`sb report`（自包含外部審計報告，供無法讀取 repo 的外部 agent 審計，`sb-report` 技能承載）、`sb lock <測試碼...>`（斷言初篩＋sha256 鎖定基準，判決前核對 hash 防「為綠燈逕改測試」的機械化）。未觸發重流程條件的「預設直接修正」走 `sb next commit --direct`（release→commit 留痕跳過執行層時序）。
