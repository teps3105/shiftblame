...
1. `<repo>/.shiftblame/SOP.md`、`<repo>/.shiftblame/ROADMAP.md` — 理解專案規則與路線圖（兩者受每 ms 審查三問約束——§1.9 修剪迴路：載入是為了執行與修剪，不是盲從）。
