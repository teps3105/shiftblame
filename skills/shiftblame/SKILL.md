...
metadata:
  version: "2.4.1"
description: 以時序制衡約束 agent——主對話秘書是唯一持久角色，連續承載意圖、需求、研究、計畫、測試、實作與驗收；意圖揭露與六段流程（requirement→research→plan→test→build→verify）以兩層兩段式承載（定義層與實作層完全對稱），所有段間切換一律旗標切段（sb next＝flow-state 同步＋留痕＋寫入權開關三位一體）。未覆蓋即凍結：老闆新輸入未經意圖揭露前 hooks 硬擋一切寫入與推進——揭露第一動由機械強制。pass/fail 判定權只在老闆兩時點（時點 1＝requirement→research 邊——審意圖→需求翻譯；時點 2＝build→verify 邊——審驗收資格：GWT 回指、假綠燈），審核資源集中在兩接縫、中鏈零審核，兩時點皆對抗在前、老闆判定在後；verify＝真驗收執行——GWT 逐條＝驗收劇本實操、行為證據落回指區，驗收完成老闆看行為證據終審 pass 出口；main 模式最小制衡環：意圖揭露→迭代→提交→老闆判 pass/fail。閘門只讀 git 事實與 flow-state；雙流模型（輸入流唯增＋理解流必然曝光）由機械層承擔。機制細節見 references/MECHANISMS.md。
---
...
> §0 主圖給流程形狀；下圖給階段遷移對應。**授權連續性**——各段條件分別成立時觸發，單一動作完成不是停點：主對話吸收產出後立即檢查下一條件，條件成立就自動觸發下一段，只有遇到老闆決策點（意圖確認、時點 1 需求翻譯推進、時點 2 進段判定、驗收終審出口、開新里程碑）才停止。**時點 1 停靠**：requirement 段完成需求定義後，先完成時點 1 對抗（審意圖→需求翻譯，出口三分類處理至乾淨）與人話簡報，揭露後停等老闆判定，pass 才以 `sb next research --boss-ok --adversarial` 推進（G1 契約同邊封存）；不等判定自行推進即繞過老闆 checkpoint，CLI 與 hooks 雙重攔截。**plan→test 機械推進**：定義層二收斂（§10 秘書自查）後 plan→test 無老闆停靠——中鏈零審核（2.4.0：老闆不看中間產物）。**時點 2 進段停靠**：收斂期 E2E 綠燈收斂後先完成時點 2 對抗至乾淨，交老闆判定 pass/fail——pass 才 `sb next verify --boss-ok --adversarial` 推進真驗收（2.4.1 前移），fail 視為老闆新輸入回意圖揭露。**終審停靠**：verify 真驗收完成（GWT 逐條行為證據落回指區）老闆看行為證據終審 pass/fail——pass 走出口（`--new-ms`／`sb end`），fail 視為老闆新輸入。
