...
| repo | DEVELOPER | G1／G2／G3 兩兩雙向一致（判準見 §10）且通過開發門檻 |
...
三軌角色是主對話內的角色切換，不是子代理。AUDITOR、RESEARCHER 在主導文件定稿前 MUST 派發唯讀子代理取得外部獨立研究或內部獨立審核；開發後三者重審前，AUDITOR MUST 派發唯讀子代理獨立審核。子代理只提供意見，不取得路由權、修改權或判決權，主對話 MUST 複核並承擔責任。無法取得所需獨立意見時，該文件不得定稿（開發後審核則標「未驗」）。應執行而未跑的 e2e MUST 標「未驗」。

AUDITOR 對 repo 永遠唯讀；其主導的 G1 為管理文件。**DEVELOPER 的檔案、字串、grep 等自驗只能證明實作存在，不能取代使用者可觀察的業務行為驗收**；PASS 只由老闆拍板。
...
> 對應 §1.3、§4、§5、§1.7 的引用。本節為兩兩一致的唯一權威定義。
...
| 三權制衡 | 開發 | G1、G2、G3 各由主導者產出；三份兩兩雙向一致（制衡完成，判準見 §10）；AUDITOR、RESEARCHER 已複核對應外部獨立研究 | 不一致者調整自己主導的文件，留在三權制衡 |
...
| `<nnn>` 完成 | 老闆路由（開新 `<nnn>`／結束 `<slug>`） | SECRETARY 已完成輕量保鮮（§1.7.1，更新 SLUG 技術債／臨時租約） | 留在 `<nnn>` 完成，先補輕量保鮮 |
