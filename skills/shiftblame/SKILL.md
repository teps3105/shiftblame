...
**對抗—修復—再對抗閉環**：對抗檢閱產生的必修項，修復後 MUST 再取一次對抗（針對修復本身），直至某一輪對抗**零必修項**（僅剩誠實殘餘）才可宣稱修復完成並提交——「修復一次即視為修好」屬假完成。sb 流程中此閉環由 hash 錨定機械強制：返工→新存檔→新驗收報告→新 hash→review 記錄必須重錨（等於強制重新對抗）；框架演化與其他無錨定路徑以本條文＋hooks 注入承載，修復提交前 MUST 補跑對抗複核。
...
**權威唯一原則**：權威文件只有 G1~G3 與 SLUG（G1 另有契約封存＋快照）；`tmp/` 內的 review、verify、release-brief、report 等一律是**可拋棄的衍生證據**——只服務當次推進的閘門查核，不提供不可變保存保證，也沒有事後完整性執法（歷史檔被清理或輪替不構成違規）。例外只有機械閘門鎖定檔（`g1-contract.md`、`test-lock.json`）——它們是**當前 ms 生命週期內**的執法依據，遺失或被改即擋，隨 ms 結束失效，不是跨歷史保存。agents MUST NOT 自行新增檔案類別、保存層或任何機械不可變保證——這類擴充一律經老闆指示後才可引入。
