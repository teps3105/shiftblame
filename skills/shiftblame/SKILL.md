...
本圖是流程權威。**秘書**（主對話）是唯一持久角色，連續承載需求定義→研究→規劃→測試→實作→驗收等**工作狀態**。狀態不是角色身份，也不是派發邊界；主對話保留完整需求脈絡並依序切換工作約束，避免上下文切換造成意圖飄移。**RAM/ROM 分層（1.8.1）**：G1~G3／SLUG＝ROM——永遠只寫「審計過後收斂」的執行前草案（定義區）與執行後回指紀錄（回指區）；`.shiftblame/tmp/`＋flow-state＝RAM——對抗產物（報告、攻擊點、修復輪次）、查證過程、運行狀態（運行時數據不寫硬碟）。**段-檔承載規格（1.7.3）**：八段由四份文件承載，每份文件一條閉環軸（定義邊＋落地邊）——**SLUG.md** 承載 intent（意圖沉澱）與 done（ms 收斂），管理層出入口；**G1** 承載 requirement（需求定義）與 verify（驗收裁判），需求閉環；**G2** 承載 research（技術定義）與 build（實作落地），技術閉環；**G3** 承載 plan（計畫定義）與 test（測試化落地），計畫閉環。推進呈 **Z 字形**：三個定義段依序定義（requirement→research→plan），三個落地段反向回收各自承載檔——**test 回指 G3 驗收排程（AC-ID 映射）、build 回指 G2 技術方案（偏離顯式記錄）、verify 逐項回指 G1 需求契約（AC 判定）**——文件定義後於落地邊復活當裁判，定義與實作由回指縫合。§10 兩兩一致於放行前一次核對後放行。**輪內單向定律（1.7.3）**：每輪內定義層單向推進（requirement→research→plan），每個階段一次產出、產出即定稿；修正＝回 intent 開新輪——輪次計數記 flow-state（時序由 history 承擔），新輪重寫自洽（1.8.2 rev 快照退役——歷史不可變性由 git 承擔）。§3 規定外部子代理檢閱的三個**固定強制對抗時點**——計畫放行前對抗方向、每個功能驗收後判決前對抗成果、收斂複驗對抗成果——與技術不可可靠裁定時的強制技術意見；主對話一律複核後自行裁定，檢閱不承載工作狀態。**所有老闆輸入第一步路由回 sb-think，不字面執行**（§6）。session 冷啟動的載入程序見 §9。
...
7. **管理文件唯一寫入矩陣**：
   ...
   **寫入矩陣機械攔截（hooks＋CLI）**：測試碼（測試慣例路徑）僅 `test` 段可寫，定稿即 commit；實作碼（`.shiftblame/` 以外 repo 檔）寫入白名單＝`build`（實作段）、`ended`（PASS 後收尾歸檔），其餘段（intent/requirement/research/plan/verify/done）對 repo 唯讀。**G 檔寫入權分區（1.8.1 RAM/ROM）**：`G1.md` 定義區＝`requirement` 段、回指區＝`verify` 段；`G2.md` 定義區＝`research` 段、回指區＝`build` 段；`G3.md` 定義區＝`plan` 段（＋秘書補寫 §2.5 例外）、回指區＝`test` 段——**跨區（落地段改定義區）＝下游倒推上游（綁架 G1/G2）死路**：hooks 比對段位（整檔粒度——落地段可寫回指區），定義區被動由 CLI 分區 hash 於 sb next 兜底（殘餘如實標註）；修正輪唯一路徑＝回 intent 開新輪（依段位推進重寫；1.8.2 rev 快照退役——零快照寫入）。`archive/` 由 CLI 於收尾時寫入；`.shiftblame/` 其餘內容永遠可寫（tmp 傾倒）。hooks 對寫檔類工具（Edit/Write/ApplyPatch/MCP 寫檔）比對段即攔；`sb next verify` 核對 working tree 乾淨（`git status --porcelain`）——進驗收前 working tree 保持乾淨。Bash 內的檔案寫入與 MCP 讀取類工具不在矩陣內（殘餘，如實標註）。
...
**機制發明屬老闆決策域（1.8.2 防復發）**：agent 自行發明的機制（非老闆拍板語義的直接實作）MUST 在設計揭露中顯性標注為「新發明」並單獨取得老闆同意；藏在實作細節裡靜默通過屭越權設計，一經發現即拆除＋同類清查（rev 快照即此例：1.7.3 夾帶、四版靜默、實跑暴露）。意圖對抗與審計邊的標準攻擊點含「夾帶未標注的新發明機制」。
