...
**權威唯一原則**：權威文件只有 G1~G3 與 SLUG（G1 另有契約封存＋快照）；`tmp/` 內的 review、verify、release-brief、report 等一律是**可拋棄的衍生證據**——只服務當次推進的閘門查核，不提供不可變保存保證，也沒有事後完整性執法（歷史檔被清理或輪替不構成違規）。agents MUST NOT 自行新增檔案類別、保存層或任何機械不可變保證——這類擴充一律經老闆指示後才可引入。
