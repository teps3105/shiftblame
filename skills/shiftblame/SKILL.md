...
所有部門（RES/SEC/QA/PRD/DEV/QC/EXP）均支持多輪迭代。多輪分為三種途徑：
...
**動態增量（秘書復判閘門）**：見「動態增量模式」區段。跨部門新增需求，直接派工對應部門。
...
| 動態增量 | Slug 級（跨部門） | 秘書+老闆 | 新增需求 | 沿用同一 slug |

**多輪規則**：
- 通訊目錄：沿用同一 `<NNN>`，不新建目錄
- task.md：主動迭代不更新；被動退回依「退回增量記錄」規則處理；動態增量由秘書透過 clarify 確認後新增
