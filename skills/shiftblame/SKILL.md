...
| 大型研究（陌生領域、多方案抉擇、高風險選型的深度調研） | 大型研究 MUST 外部唯讀子代理承擔主要調研（Web 查證不足代），複核吸收後寫 G2；**外部證據打底**——進 research 段後至少一次外部工具調用，事實由對話呈現承載（A2：調用工具、查證對象與證據落點於對話揭露；2.7.2 機械閘已除——外部證據機械綁定與真實使用脫鉤，不做 CLI 阻擋，偽造抽查承擔） | 否 |
