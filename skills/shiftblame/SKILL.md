...
**標準攻擊點清單（對抗任務組裝 MUST 轉錄）**：組裝任何對抗任務時 MUST 把本清單轉錄給子代理——**通用面**：誤讀意圖、漏邊界、範圍走私、夾帶未標注的新發明機制、過時描述（描述已不存在的機制）、忽略同 slug 過往 ms 定案（重複發明既有方案）；**提交對抗面**（sb adversarial／commit 閘）：staged 程式碼變更與相關永續層文件零變更的正當性（same-commit）、註釋與行為不一致（過時註釋、註釋承諾程式碼沒做的事）；**時點②面**：假綠燈、證據不足、漏驗、行為不符 G1。清單是最低覆蓋——子代理可加攻擊面，不可少於此；時點①③的專屬攻擊面（方向偏差／premortem／ms 價值）由對應時點的任務描述承載。轉錄由對抗檢閱記錄＋老闆抽查承擔，機械層不驗轉錄。
