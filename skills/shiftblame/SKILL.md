...
metadata:
  version: "2.3.1"
description: 以時序制衡約束 agent——主對話秘書是唯一持久角色，連續承載意圖、需求、研究、計畫、測試、實作與驗收；七段流程 intent→requirement→research→plan→test→build→verify 以兩層兩段式承載——定義層（理論：逐功能寫規劃循環＋整體規劃收斂循環）與實作層（實際：逐功能迭代循環＋收斂期 E2E 循環）完全對稱；所有段間切換一律旗標切段（sb next CLI 動作＝flow-state 同步＋留痕＋寫入權開關三位一體）。未覆蓋即凍結：老闆新輸入未經意圖揭露（理解流無覆蓋條目）前 hooks 硬擋一切寫入與推進類動作——揭露第一動由機械強制，不依賴自覺補路由。pass/fail 判定權只在老闆兩時點——時點 1 定義層放行（plan 後）、時點 2 ms 出口前——兩時點皆先對抗（子代理檢閱＋出口三分類處理至乾淨）再交老闆判定 pass/fail：pass 才 --boss-ok 放行／出口；對抗／閘出口三分類：乾淨續行／僅需修復 agents 自動旗標切段回指定 node 續迭代不停／需老闆裁決停經 think。老闆任何新輸入（含 fail）一律回到意圖揭露，揭露後由 intent 路由器路由到正確位置；intent 與 done 同在流程段循環之外。段內提交閘（sb adversarial 提交對抗→sb commitmsg 發章→hooks 於 commit 消費）承載單功能單提交，判決屬 agents。main 模式（不開 slug）最小制衡環：意圖揭露→迭代→對抗→老闆判 pass/fail。閘門只讀 git 事實與 flow-state.json，不可變性由 git 承擔；雙流模型（輸入流唯增＋理解流必然曝光）由機械層承擔，抗上下文壓縮。審計層（對抗審計＋意圖審計）留痕複審。RAM/ROM 分層（G/SLUG＝ROM 收斂產出、tmp/flow-state＝RAM 運行層）；迴圈斷路器（同操作重複即擋；計數純觀測——工作做到完成為止）；每 ms 遙測結算由 git baseline 時序分析錨定；SOP／ROADMAP 每 ms 審查。技術證據不足時強制外部唯讀技術意見，主對話複核後自行承擔裁定。commit、判決、放行、路由、ms 出口一律由主對話獨佔。
---
...
**接入健康先於正式寫入與提交**：`sb state`、hooks 寫入檢查、對抗宣告與提交檢查共用狀態分類，區分未初始化紀錄、合法直接實行紀錄、七段流程、ended（含完結戳）與異常。異常時維持錯誤判定；對抗宣告、提交與 git 寫入保持封閉，**診斷與狀態修復自由**（唯讀查證、修復腳本、flow-state／tmp 寫入——修復是異常模式的目的，非唯讀白名單的附屬）。恢復完成須重新查證 `sb state` 並依既有授權路由；狀態可辨識不等於老闆批准，也不要求開 slug。完整行為規範見 `skills/think/SKILL.md`「流程接入失敗先修復」。工具層能辨識的寫入與 shell 呼叫才由 hooks 攔截；未知包裝器及直接改檔仍由主對話紀律與抽查承擔，不宣稱能辨識任意程式的副作用。

**回合與流程分離**：平台的 final／Stop 只結束回合，流程完成仍由既有閘門認定。插入疑問解答後，主對話接續原有已授權未完工作；實際改變理解的補充／修正先完成 intent 回退與查證。final 前的路由結果與合法停點檢查，統一依 `skills/think/SKILL.md`「回合結束與流程接續」。SessionStart／UserPromptSubmit 注入此責任；Stop 執行停點偵測（§1.12——活動流程無申報擋停一次，條件式、單次、不代做路由；申報／停等／無流程放行），提示注入與擋停都不等於機械判定語義上的未完工作。

**工作連續性**：每個階段完成時，秘書 MUST 吸收產出、更新短帳本並立即判定下一個已授權狀態；階段完成、局部綠燈、壓縮或老闆沉默都不是停止許可。**連續以層為界**：定義層與實作層內部連續推進不停，定義層→實作層的放行邊界（時點 1）是強制停靠點（先揭露後推進，§11）——MUST 完成停靠才續進。

本圖是流程權威。**秘書**（主對話）是唯一持久角色，連續承載需求定義→研究→規劃→測試→實作→驗收等**工作狀態**。狀態不是角色身份，也不是派發邊界；主對話保留完整需求脈絡並依序切換工作約束，避免上下文切換造成意圖飄移。**RAM/ROM 分層**：G1~G3／SLUG＝ROM——永遠只寫「審計過後收斂」的執行前草案（定義區）與執行後回指紀錄（回指區）；`.shiftblame/tmp/`＋flow-state＝RAM——對抗產物（報告、攻擊點、修復輪次）、查證過程、運行狀態（運行時數據不寫硬碟）；`.shiftblame/tmp/` 內的子代理工作區＝受治理寫入子代理的落點（research／build 段並行——過程狀態由子代理一次性上下文承載、工作產物落 tmp，主對話依結果整合回當前分支，§3）。**段-檔承載規格**：七段由四份文件承載，每份文件一條閉環軸（定義邊＋落地邊）——**SLUG.md** 承載 intent（意圖沉澱）與 ms 出入口（管理層樞紐）；**G1** 承載 requirement（需求定義）與 verify（驗收裁判），需求閉環；**G2** 承載 research（技術定義）與 build（實作落地），技術閉環；**G3** 承載 plan（計畫定義）與 test（測試化落地），計畫閉環。推進呈 **Z 字形**：三個定義段依序定義（requirement→research→plan），三個落地段反向回收各自承載檔——**test 回指 G3 驗收排程（AC-ID 映射）、build 回指 G2 技術方案（偏離顯式記錄）、verify 逐項回指 G1 需求契約（AC 判定）**——文件定義後於落地邊復活當裁判，定義與實作由回指縫合。§10 兩兩一致於放行前一次核對後放行。**輪內單向定律**：每輪內單向推進（定義層 requirement→research→plan、實作層 test→build→verify），每個階段一次產出、產出即定稿；**段間切換一律旗標切段**——`sb next` CLI 動作三位一體：flow-state.node 同步＋history 留痕＋hooks 按 node 開關寫入權（agent 機械認知當前段位，杜絕跨段寫入——帶內允許邊推進也經 CLI，不零旗標自行切換）；修正＝回 intent 開新輪——輪次計數記 flow-state（時序由 history 承擔），新輪重寫自洽——歷史歸屬分層：repo 檔由 git 承擔，G/SLUG 覆寫即無歷史（時序歸 flow-state，操作紀律見 shiftblame:rewrite）。

**兩層兩段式（流程主幹）**：定義層寫理論（「理論上要怎麼做」）、實作層寫實際（「實際上做出了什麼」），兩層完全對稱各分兩段——**定義層一＝逐功能寫規劃循環**（requirement→research→plan 逐功能推進，plan 完成回 requirement 接下一功能，逐功能把三面向寫齊）→規劃收斂→**定義層二＝整體規劃收斂循環**（requirement→research→plan 整體對齊循環，循環補正至三份兩兩雙向一致）→**時點 1**；**實作層一＝逐功能迭代循環**（test→build→verify，單功能單提交——判決通過經提交閘 commit 後回 test 接下一功能）→功能迭代完成→**實作層二＝收斂期 E2E 循環**（test 寫 E2E→build 調環境→verify 跑完整流程，紅燈回 test 修至綠燈收斂）→**時點 2**。兩個收斂循環與兩個逐功能循環形狀對稱。

**pass/fail 判定權只在老闆兩時點**：時點 1（定義層放行，plan 後）與時點 2（ms 出口前）——兩時點皆是**對抗在前、老闆判定在後**：先完成時點對抗（子代理檢閱；對抗／閘出口三分類——①乾淨續行②僅需修復 agents 自動旗標切段回指定 node 續迭代不停③需老闆裁決停經 think——處理至乾淨），對抗乾淨後才交老闆判定 pass/fail。pass 才以 `--boss-ok` 留痕放行／出口（adversarialLog point 條目對照，兩時點皆 --adversarial）；中間過渡一律中性流程推進（「規劃收斂」「功能迭代完成」），agents 不代老闆判 pass/fail（段內提交閘的 AC 判決是 agents 判決，屬功能迭代內建機制，不佔時點編號）。

**intent 與 done 在循環外；老闆新輸入回揭露點**：老闆任何新輸入——運行中任何段位、任何時刻，含兩時點 fail——一律回到意圖揭露（shiftblame:think；「未覆蓋即凍結」機械保證，見下），揭露後由 intent 路由器路由到正確位置；intent 與 done 同在流程段循環之外，不是線性段。**返工輪計數**：僅老闆輸入驅動的返工（經揭露回 intent）計輪＋rewrite 載入閘；旗標切段、段內修復環、閘口自動返工不計輪。**出口**：時點 2 pass 後先進 done 判定——`sb next intent --new-ms --boss-ok --adversarial`（開下一里程碑，回 intent 展開新 ms）或 `sb end --boss-ok --adversarial`（結束 slug 進 ended）。

**段內提交閘（單功能單提交）**：實作層一每個功能完成即一次提交——測試碼＋實作碼同 commit，提交對抗（sb adversarial 發章）→sb commitmsg→hooks 於 git commit 消費焚章；AC 判決屬段內秘書判決（功能級）。

**未覆蓋即凍結（揭露第一動的機械保證）**：每則老闆輸入到達（UserPromptSubmit 記入輸入流）後，須經 shiftblame:think 調用（args＝理解宣告）落理解流，該輸入才算「被理解覆蓋」。最新輸入尚無理解覆蓋的期間，hooks 的 PreToolUse 硬擋一切寫入類與流程推進類動作（Edit／Write／Bash 寫入類／`sb next` 等），擋停訊息指向先意圖揭露；唯讀查證、Skill 調用（含 think 本身）、tmp 傾倒照常放行；理解宣告落檔即自動解凍。這把「曝光事後審」升級為「無理解＝無行動」的機械前置——agent 不再依賴自覺「意識到才補路由」。理解仍不被當鑰匙消費（曝光＋老闆終審不變），機械只保證揭露先行。main 模式的「意圖揭露」第一步同受此保證（flow-state 雙流記錄與模式無關）。

**審計層貫穿全程**：時點 1／2 對抗與提交對抗的報告構成**對抗審計**（`references/AUDIT.md`——唯讀子代理、邊界、攻擊點、證據義務、複審閉環、閘門對接）；回 intent 事件（fail 回指、補充、追加、修約）構成**意圖審計**面——每次回指在 SLUG 留回指紀錄、history 留時序，對照理解流可回審「回指是否忠於老闆意圖」。審計層只讀流程事實（git＋flow-state＋tmp 報告），本身零寫入權。

§3 規定外部子代理檢閱的兩個**固定強制對抗時點**——時點 1 定義層放行前對抗方向、時點 2 ms 出口前對抗收斂成果——與段內提交閘的強制提交對抗（每功能 commit 前，不佔時點編號）、技術不可可靠裁定時的強制技術意見；主對話一律複核後自行裁定，檢閱不承載工作狀態。**所有老闆輸入第一步路由回 shiftblame:think，不字面執行**（§6）。session 冷啟動的載入程序見 §9。
...
1. **三面向制衡與輪內單向定律**：G1 需求、G2 技術、G3 計畫三面向權限對等，由秘書調控其對應工作階段依序產出。**輪內單向定律**：每輪內定義層單向推進——時序固定 需求定義(G1)→研究(G2)→規劃(G3)，每個階段**一次產出、產出即定稿**；產出時直接向前對齊（G2 對齊 G1、G3 對齊 G1＋G2）。**修正＝回 intent 開新輪**：`sb next intent` 時輪次計數 +1（記 flow-state）——時序由 history 承擔，新輪重寫自洽（對新 G1 全量重新對齊），零補丁疊加；重寫的操作紀律由 shiftblame:rewrite 承載（定義區整檔重寫、回指區同鍵覆寫、SLUG 既有列更新——G/SLUG 是當下事實的單一權威，不是歷史紀錄集合）。歷史歸屬分層：repo 檔由 git 承擔；G/SLUG 不入 git、覆寫即無歷史——跨輪時序由 flow-state history 承擔，需對照脈絡先傾倒 tmp（shiftblame:rewrite）。**G1 承接 shiftblame:think 已確認的完整語義直接定稿，不另問；G2 外部證據打底、G3 對齊推進——規模由需求決定（一次精準查證到完整調研皆可，不堆官僚文件），外部性是機械底線（進 research 段後至少一次外部工具調用，hooks 標記 externalEvidence、research→plan 邊驗；返工同判；大型研究 MUST 外部唯讀子代理承擔，§3）。** §10 兩兩一致是文件結構判準，於放行邊（plan→test）一次核對。核對結果二分處置（對抗出口三分類適用）：**G1 清晰下的自身細節缺漏**（如 G2 漏列測試方式、G3 步驟漏依賴）＝僅需修復——agents 自動旗標切段回責任面向對應段一次補正（CONFORMS 語義——G1 內容不變、重新放行時同 hash 重封存；history 留時序），不計返工輪；**G2/G3 與 G1 不一致**（矛盾、歧義、無法唯一對齊——含 G2 與 G3 之間的矛盾，根源都是 G1 定義不夠正確）＝需老闆裁決——停經 shiftblame:think 呈現語義出入，老闆裁決後回 intent 開新輪（計返工輪＋rewrite 載入閘）由 requirement 段重新定義 G1（G1 定義權唯一屬需求面向），依序重新對齊 G2/G3，仍輪內單向不打轉。只有改變產品方向、G1 語義或授權範圍的重大例外（§1.4.1）才停止開發退回；符合 G1 的架構或技術改選由主對話自行裁定並細化 G2／G3。
2. **文件即制約規則**：每份文件只在其對應工作狀態產出，保持乾淨的決策結論；主對話切換狀態後，其他面向文件保持原產出（改寫權屬其對應工作狀態）。**秘書有全域時序調控權**，決定狀態推進與段落寫入歸屬（如 SLUG 收斂執行記錄由秘書補寫）。執行中間產物與對抗歷史存 `<repo>/.shiftblame/tmp/`（RAM）；G*.md 定義區只放決策結論（放行時分區封存），回指區由對應落地段收斂寫入（verify→G1 判定、build→G2 偏離、test→G3 映射）——不當流水帳。向前對齊時發現缺漏依 §1.1 二分處置：G1 清晰下的自身細節缺漏＝旗標切段回責任面向對應段一次補正（CONFORMS——G1 不變，不計返工輪）後依序重走；與 G1 不一致（矛盾、歧義）＝停經 shiftblame:think 老闆裁決後回 intent 開新輪由 requirement 段重新定義 G1——語義出入由 G1 唯一裁定。
3. **單一面向、兩兩一致**：每份文件只回答自己面向（G1 需求／驗收、G2 技術、G3 實作計畫），但三份 MUST 兩兩雙向一致才可開發——一致性於放行邊（plan→test）依 §10 一次核對，缺漏＝旗標切段回責任面向對應段一次補正（CONFORMS——G1 不變，不計返工輪）後依序重走重核（輪內單向定律，§1.1）。一致的對齊軸為 G1 需求項，判準（三對六向的正向承接＋反向回指）見 §10。
4. **兩層兩段式（開發主幹）**：開發由**主對話秘書**連續切換工作狀態並依 G3 推進；commit、判決合格／返工、放行、路由、reset、ms 出口皆由主對話獨佔。**每個 `<ms>` 就是一個里程碑**（一組功能構成的使用者可觀察完整價值）；功能是 **commit 單位**，不屬於本 ms 的功能只記入 SLUG §3 待辦清單。定義層寫理論（理論上要怎麼做）、實作層寫實際（實際上做出了什麼），兩層完全對稱。

   **定義層**：**定義層一（逐功能寫規劃循環）**——每個功能依序 requirement（G1 需求項）→research（G2 技術分析）→plan（G3 實作步驟＋驗收設計），plan 完成回 requirement 接下一功能，逐功能把三面向寫齊；**規劃收斂**後進**定義層二（整體規劃收斂循環）**——requirement→research→plan 整體對齊循環，三份兩兩雙向一致（§10）未成立前循環補正（缺漏由責任面向旗標切段回對應段），成立即收斂。**時點 1**：對抗（子代理對抗方向檢閱，出口三分類處理至乾淨）→交老闆判定 pass/fail→pass 以 `sb next test --boss-ok --adversarial` 放行（point 1 留痕）。

   **實作層**：**實作層一（逐功能迭代循環）**——每個功能 test（依 G3 驗收排程定義並撰寫該功能測試，AC-ID 映射，定義「過」）→build（回指 G2 寫碼，執行單點與到達接合點的整合測試，偏離 G2 處顯式記錄）→verify（核對當次功能證據，依 G3 當次功能回指 G1 需求契約 AC 判定；判決＝段內秘書動作；verify 段對 repo 唯讀）——判決通過經**提交閘**（提交對抗→sb commitmsg→commit，測試碼＋實作碼同 commit，單功能單提交）後回 test 接下一功能；**功能迭代完成**（本 ms 所有功能提交且 AC 通過）後進**實作層二（收斂期 E2E 循環）**——test 寫本 ms 所需 E2E→build 調整環境與接線→verify 跑完整使用者流程，相鄰雙向循環修至綠燈收斂。**時點 2**：對抗（子代理對抗成果檢閱，出口三分類處理至乾淨）→交老闆判定 pass/fail→pass 經 done 判定出口（point 2 留痕）。

   **紅燈處置（段內，不停等）**：agents 發現問題依對抗出口三分類——僅需修復者自動旗標切段回指定 node 續迭代（實作問題回 build、測試定義錯誤回 test 附紅燈證據與理由），不計返工輪；綠燈路徑一律經實作修正達成。pass/fail 判定權只在老闆兩時點——中間過渡一律中性流程推進（「規劃收斂」「功能迭代完成」）。

   **三層測試與執行時機**：測試依實際驗證對象分類，與工具、檔名、是否在 CI 執行正交。G2 說明測試邊界與依賴，G3 排定適用層級、接合點及 ms 完整流程；重用既有有效測試，按需求選擇層級，每個功能只配置必要的測試。

   **規模由防護目標推導（測試與防護工具同判）**：測試，以及為防護而建的檢查手段（檢查腳本、靜態分析、linter 規則、驗證工具），其規模 MUST 由防護目標推導——先指名要防的具體風險（如私有直寫、繞契約、循環依賴這類可指名行為），逐項選擇能攔截該風險的第一個足夠手段（§3 最小充分解）。防護目標不需要的精確度——對全部函式做型別推論、副作用分析或資料內容相關性追蹤這類超出指名風險的分析——即規模溢出，屬過度工程：research／plan 段淘汰，test 段判返工。規模判定不依專案類型、規模或成熟度——不存在「專案大就配企業級防護」的規則表；同樣的防護目標在任何專案對應同樣的最小充分手段，專案大小只改變目標數量，不提高單一目標的防護強度。此為執行紀律，由主對話與對抗檢閱判定。

   | 層級 | 驗證對象 | 執行時機 |
   |---|---|---|
   | 單點測試 | 單一函式、元件或規則的輸入、輸出與失敗邊界 | 實作層一 build 隨相關實作變更執行＋收斂期重跑 |
   | 整合測試 | 模組間的介面、資料傳遞與協作 | 實作層一到達 G3 排定的依賴接合、相關介面或共同依賴變更時＋收斂期重跑 |
   | 端到端測試（E2E） | 從真實使用者入口到最終結果的完整流程 | 實作層二（收斂期）：test 段撰寫、build 調整環境、verify 執行，修至綠燈收斂 |

   **功能小循環與 ms 驗收分開**：實作層一每個功能——test 依 G3 驗收排程撰寫當前功能測試並定義「過」；build 做單點與到達接合點的整合驗證；verify 核對功能證據、依 G3 當次範圍作 AC 判定（段內秘書判決，屬 agents 權責）；判決通過經提交閘 commit（測試碼＋實作碼同 commit）後回 test 接下一功能。尚未到接合點的整合項依 G3 保留待驗；E2E 一律屬實作層二（收斂期循環），不提前擴成每功能的全流程要求。單點／整合綠燈只支持各自範圍；ms 完整價值仍以收斂期 E2E 與 G1 行為證據驗收。跨功能完整流程即屬 E2E，改稱局部檢查仍依 E2E 時機執行。

   **證據沿用與有因重驗**：verify 核對測試版本、被測內容、依賴、環境及操作結果是否仍適用於待驗 commit；相同被測內容的存檔或進入 verify 本身不觸發重跑。每次重跑先記明修復、具體變更影響或證據失效原因，以及待驗假設與範圍；採足以涵蓋影響的最小測試集合。通過且仍有效即沿用；同一失敗沒有新處置時先定位原因，證據不足依 §3 取得技術意見或如實標未驗。E2E 發現問題後先回對應環節修復，以單點／整合確認，再於 ms 驗收前重跑受影響的端到端流程；影響橫跨全部流程時才完整重跑，理由落 tmp。此為執行紀律，由主對話與對抗檢閱判定，CLI 不辨識測試層級或自動計算影響範圍。

   **G1 是封存契約，G2／G3 只准單調細化**：plan→test 放行邊（`sb next test --boss-ok --adversarial`）將當下 G1 的 SHA-256 記入 `flow-state.json`；後續每次 `sb next` 都重算核對，G1 被改動即擋。放行前的定義層補正＝旗標切段回對應定義段（§1.1 CONFORMS——`sb next research`／`sb next requirement`，不經 intent、不計返工輪）；放行後的開發中局部技術模型修正（不改變 G1 的使用者可觀察結果、範圍／例外、詞義與驗收成功集合）＝回 intent 開新輪至對應定義段單調細化 G2／G3（計返工輪＋rewrite 載入閘——放行後落地段回定義段必經 intent，G1 契約隨回 intent 解除、重新放行時同 hash 重封存；G 檔寫入權綁定義邊段位，落地段無上游寫入權）；codebase 現況、技術限制或局部最適解作為可行性證據供 G1 裁定——G1 的改寫權在修約路徑。出入一律先分類：**符合（CONFORMS）**→只修正 G2／G3 並繼續；**契約不足（UNDERSPECIFIED）／契約衝突（CONFLICTS）**→回 intent 開新輪（`sb next intent`——老闆補充路徑）重定義 G1，修約差異寫 tmp，G1 定稿後重新對齊 G2／G3、重新放行——語義出入一律顯式分類（CONFORMS／UNDERSPECIFIED／CONFLICTS）。

   **使用者驗收追溯鏈**：G1 每項使用者需求 MUST 以唯一 `AC-ID` 定義使用者／前置／操作／可觀察結果／失敗邊界，證據固定為 `BEHAVIOR`；G3 以相同 ID 排程驗收並映射對應測試。AC-ID 只存在於 G1／G3 與驗收報告——程式碼保持與流程正交（§3）。test 段撰寫的測試碼隨功能實作同 commit 定稿——測試的不可變性由 git 承擔。驗收報告逐項產出 `SATISFIED／UNSATISFIED／UNVERIFIED`、commit、實際操作與觀察；引用專案輸出以**節錄快照**為證，非專案原檔（§3）。`verify` 邊機械核對 working tree 乾淨（實作已存檔，git 判定）；功能判決核對 G3 當次範圍的行為證據，未排到或未驗的項目如實保留 UNVERIFIED（文件層：秘書判定＋提交對抗把關）；時點 2 出口（老闆 pass）才要求全部必填 AC 皆為行為證據 SATISFIED（§1.4.2）。

   **開發序列複雜度門檻**——任一成立時，秘書 將 G3 行動序列拆成 causal 小步；每步只能依賴已完成項；一致性的跨檔修改保持同一步：行動數 ≥ 4、跨檔數 ≥ 4、強依賴對 ≥ 2。

   每個功能（commit 單位）由主對話在 test→build→verify 段內完整走完，判決通過經提交閘 commit 後回 test 接下一功能；紅燈屬段內修復環——旗標切段回 test／build 續迭代，不停等。執行記錄存 `<repo>/.shiftblame/tmp/` 並整理成可判讀的結構化產出，G*.md 只放決策結論。判決含測試真實性核對；測試無真實斷言或與 G1／G3 無對應即判紅燈回 test 重寫（測試定義錯誤附理由——段內不停等）。本 ms 所有功能循環完成後進實作層二收斂期（§1.4.2）。**功能是 commit 單位，ms 是驗收節點，時點 2 老闆 pass 是 ms 的出口邊**。

   **重修路徑（老闆新輸入）**：老闆不滿意成果——任何新輸入（含 fail 判定）一律回到意圖揭露（shiftblame:think），揭露後由 intent 路由器路由：定義級修正回 requirement 段重定義 G1（計返工輪＋rewrite 載入閘），實作級指示路由回實作段處理。已完成出口的追加（新需求）＝揭露後經 intent 路由同 ms 擴充 G1 重走，或老闆授權開新里程碑（--new-ms）；完成類動作（已 commit）一律回應老闆的重修意圖——返工＝疊加新 commit。
   ...
   **1.4.2 實作層二收斂期（ms 驗收 → 時點 2 → pass 出口）**：秘書在**功能迭代完成**（本 ms 所有功能經實作層一小循環提交且 AC 判定通過）自動觸發（開發完成即收斂，不需等待指令）：
   ...
   3. **收斂期 E2E 循環**：test 段撰寫本 ms 所需 E2E（驗證從真實使用者入口到最終結果的完整價值）→build 段調整環境與接線→verify 段執行並核對——相鄰雙向循環，紅燈回對應段修復至綠燈收斂（三分類②不停等）。有效的單點／整合證據沿用，E2E 重驗依 §1.4 的影響範圍處理。對照封存 G1 逐項確認滿足的驗收項；複驗判定收斂寫 G1 回指區（定義區不變——修約路徑唯一變更），過程落 tmp。這是段內收斂判決，不再要求老闆確認；證明契約不足／衝突才走 §1.4.1 顯式修約。
   ...
   6. **輕量動作**：老闆 pass 後秘書更新 SLUG 技術債／臨時租約、§4 段表與**定案索引**（本 ms 一行式定案，見 §1.7.1）；不動 SOP／ROADMAP／archive。
5. **圖文衝突時以圖為準**：其他文件解釋自己負責的面向與箭頭，流程以本圖為唯一權威。
6. **SOP／ROADMAP 硬分欄**：SOP 寫本專案跨 `<slug>` 長期有效、可執行且可查核的本地配置、專案特有規範、資料／服務邊界與驗證入口，可用段落、表格、命令與來源標註完整保存實際值，內容以引用 SKILL 與 `references/` 中央模板已定義的通用流程。ROADMAP 只寫老闆明確授權的產品目標、固定邊界與尚未完成的想做計畫，需求不必先被阻塞。兩者內容＝各自職責欄位（規範／計畫）；需求流程、技術方案、實作計畫、進度、討論、角色行為、歷史流水與未授權方案一律排除。完整准入欄位以 `assets/SOP.md`、`assets/ROADMAP.md` 為準——寫入前依准入欄位檢查，欄外內容改寫入正確文件或移除。
7. **管理文件唯一寫入矩陣**：

   - **SOP** — 寫入者：秘書。前置：新增規範需老闆授權；內容隨 codebase 變更同批更新（same-commit 文件先行，§1.7）。
   - **ROADMAP** — 寫入者：秘書。前置：新增產品意圖需老闆授權；每 slug 結束時移除完成項。
   - **SLUG** — 寫入者：秘書。承載 intent（意圖沉澱）與 ms 出入口（管理層樞紐）；前置：老闆已決定 slug/ms 與節點；只記可獨立理解的授權、目前狀態與回指（shiftblame:save 的工作過程與交接紀錄寫入 tmp，SLUG 僅保留存檔時間與交接路徑）。
   ...
   **寫入矩陣機械攔截（hooks＋CLI）**：測試碼（測試慣例路徑）寫入白名單＝`test`＋`build` 段，隨功能實作同 commit 定稿；實作碼（`.shiftblame/` 以外 repo 檔）寫入白名單＝`build`（實作段）、`ended`（結束後收尾歸檔），其餘段（intent/requirement/research/plan/verify）對 repo 唯讀。**G 檔寫入權分區（RAM/ROM）**：`G1.md` 定義區＝`requirement` 段、回指區＝`verify` 段；`G2.md` 定義區＝`research` 段、回指區＝`build` 段；`G3.md` 定義區＝`plan` 段、回指區＝`test` 段——**跨區（落地段改定義區）＝下游倒推上游（綁架 G1/G2）死路**：hooks 比對段位（整檔粒度——落地段可寫回指區），定義區被動由 CLI 分區 hash 於 sb next 兜底（殘餘如實標註）；**返工輪 rewrite 載入閘**：修正輪（rev 有值）寫 G 檔前 hooks 驗本輪已調用 shiftblame:rewrite（rewriteSeen.rev 比對當前 rev——未載入即擋，載入後本輪放行；返工重寫為當下事實不靠自發，詳 skills/rewrite）；修正輪唯一路徑＝回 intent 開新輪（依段位推進重寫）。`archive/` 由 CLI 於收尾時寫入；`.shiftblame/<slug>/<ms>/`（含 `archive/`）是 ROM 區**僅承載 G1~G3.md**——其餘寫入由 hooks 攔（中間產物一律 tmp）；`.shiftblame/` 其餘內容永遠可寫（tmp 傾倒）。**子代理工作區條款**：`.shiftblame/tmp/**`＝受治理寫入子代理的工作落點——research／build 段並行窗口的子代理在 tmp 內依任務卡產出檔案與報告（寫 tmp 永遠自由、不入庫）；**子代理零 repo 寫入權**——不 commit、不跑 sb、不直接改 repo 檔；整合由主對話依子代理工作結果，以自身寫入權（段位白名單＋提交閘）修改回當前所在分支（main 模式＝main 分支；slug 模式＝`<type>/<slug>` 分支）。tmp 為拋棄式實驗空間（實驗碼不出 tmp）。**RAM 流以 slug 為單位清理**：`sb end` 時 flow-state 累積流（輸入流／理解流／adversarialLog／history）直接清空——曝光與對照價值隨 slug 終結，零副本、零殭屍存續（以任何形式轉移存續都屬違規）；slug 內以**觀測流輪替**保持恆有界（§1.11 第 3 條）。hooks 對寫檔類工具（Edit/Write/ApplyPatch/MCP 寫檔）比對段即攔；`sb next verify` 核對 working tree 乾淨（`git status --porcelain`）——進驗收前 working tree 保持乾淨。Bash 內的檔案寫入與 MCP 讀取類工具不在矩陣內（殘餘，如實標註；Bash 產生的中間產物同樣一律落 `.shiftblame/tmp/`——工作腳本、暫存、報告寫入 repo 其他位置屬非授權雜檔，由攻擊點與抽查承擔）。

**兩層文件模型（文件↔實況對照是一等公民）**：**永續層**＝`docs/`、`SOP.md`、`ROADMAP.md`（＋`README.md`、`skills/`）——專案的持久真相。文件視同程式碼：**專案層文件變更走與程式碼相同的流程與對抗**（時點對抗＋commit 對抗閘），並**隨程式碼即時變更**——行為變更的 commit 帶上對應文件變更（same-commit：改了什麼就行為什麼文件，文件滾動保真——順序固定：文件在前、碼隨後）。**文件先行**：永續層文件先行改到目標狀態、實作碼依文件撰寫（與測試先行同族——文件是依據非事後記錄；客體＝永續層文件 vs 實作碼，G 檔由輪內單向與寫入權分區承載、測試碼由 test 段先行定稿承載）。build 段發現不一致按歸屬處置：永續層描述層過時／錯誤→same-commit 修正文件再繼續（限 build 段——verify 對 repo 唯讀）；G2／G3 與實作現實不符→G2 回指區或回 intent 細化（既有路徑）；需求／驗收語義出入→§1.4.1 修約——永續層承載的語義一律經修約。**過時描述不是執行依據**：任何工作狀態發現永續層描述與實況或當下需求衝突，MUST 浮現衝突並走修正通道——build 段 same-commit 修正；其他狀態在當下產出（文件／報告／計畫）顯式標注衝突並經對應文件變更路徑修正；已確認契約語義走 §1.4.1 修約。把過時描述當規格照做、或以「文件這樣寫」為由執行已知不符實況的行為，與引用不存在機制同屬虛空捏造——陳述對照閘與時點對抗關閉此路。提交時**陳述對照閘**機械驗其可驗陳述（sb 命令／旗標引用 ↔ CLI 實況，單一真相取自 sb.mjs 源碼）——引用不存在的機制即擋，過時假設與虛空捏造在此關閉。**當下層**＝`G1/G2/G3/SLUG`——開發工作文件，ms 壽命、用後即归檔（archive）、保存可獨立理解的收斂定義與回指；對話、工作過程與交接敘事一律 tmp；後續開發引用當下層＝引用歷史脈絡，查現況看永續層＋專案實況本身（G1 的 SHA 封存是開發期間防漂移，非永久維護義務）。**當下層決策寫進永續層＝經 normal 變更流程（與程式碼同）**——文件升級是日常變更的一部分，非獨立儀式。**文件陳述錨**：MUST 級機制的行為測試 MUST 附文件陳述錨——斷言永續層文件仍含該機制的關鍵陳述（錨與行為測試同檔：機制拆除時測試與錨同拆，孤兒錨自然消失）；文件措辭變更 same-commit 同批更新對應錨。錨攔截的是刪除漂移（陳述被刪或漂走即紅燈）；語義級對照（描述對不對）機械不驗——時點對抗＋抽查承擔。

收尾歸檔（`sb end` 後執行；操作步驟見 §1.7.2）：簡單、機械化、快速結束——移 `<slug>/` 至 `<repo>/.shiftblame/archive/`。SOP／ROADMAP／docs／README 的更新在開發期間已隨各 commit 即時完成（same-commit 原則，兩層文件模型）——收尾只歸檔既成事實，零重寫、零補救。每個 ms 通過 pass 出口時的輕量動作＝更新 SLUG 技術債／臨時租約、§4 段表與定案索引。

### 1.7.1 pass 出口的輕量動作

> 對應 §0 圖：時點 2 對抗畢、老闆 pass 後的出口前動作；SLUG §4 段表更新即輕量動作。
...
6. **SOP／ROADMAP 每 ms 審查**（§1.9 修剪迴路）：對照實況跑三問後 `sb sopreview <三問結論>` 留痕——開新 ms（`--new-ms`）與結束（`sb end`）前機械驗本 ms 已審（無 SOP／ROADMAP 的專案不擋）；審查含機械基本功檢查（updated 同步、零日期日誌行、零重複行與重複標題——未過即戳記不發）；刪修結論走正常 commit。
...
### 1.7.2 收尾歸檔（slug 結束：sb end 後）

收尾＝簡單、機械化、快速結束——歸檔既成事實。永續層文件（SOP／ROADMAP／docs／README）在開發期間已隨各 commit 即時更新（same-commit 原則，§1.7 兩層文件模型）；收尾零重寫、零補救。

1. 秘書確認證據、未驗項與老闆結束拍板（pass 出口經 `sb end --boss-ok --adversarial`）；`sb end` 已於出口時留產出遙測於 flow-state（§1.11——每 ms 結算＋最終 diff 統計＋對抗判定＋計數＋耗時）。
...
### 1.9 基質優先與元行為錨定（方法論——重複造輪子與規則堆疊的解法）

兩個系統性問題的共同根：**重複造輪子**——基質已提供的能力被另造一套（git 已給身分錨定（commit hash）、不可變性、時序與 diff 統計，agent 卻建完整克隆＋SHA 清單＋身分反例自測；專案內記錄檔因此無限累加）；**規則堆疊**——框架定義大量規範卻從未基於實測的代理元行為設計，只迭加、從不修剪。解法是三層閉環：

1. **基質優先（立機制／記錄／驗證的準入判準）**：任何新機制、記錄檔、驗證工具 MUST 先對照基質已提供的能力——git（身分錨定、不可變性、時序、diff）、平台 hooks（事件流）、標準庫／既有依賴。基質可答的另造即拆，改引基質；**記錄檔預設不存在**——確需記錄，落 `.shiftblame/tmp/` 或以基質重導（產出遙測＝`sb init` 錨定 git baseline、每 ms 結算＋`sb end` 做 baseline..HEAD 時序分析，是本判準的第一個示範）。
2. **元行為錨定（規則由實測推導）**：新規則 MUST 錨定實際觀測的代理行為證據——觀測層（§1.11：sb-usage.jsonl、每 ms 遙測）承載「agent 實際怎麼花費、怎麼產出、規則被不被觸發」；無實測證據的規則＝想像威脅，立規前先量測。
3. **修剪迴路（每 ms 審查三問）**：SOP／ROADMAP **每 ms 必審**——AI 開發下單一 ms 即足以改變整體方向。三問：①基質可答？（重複 git／平台既有能力即拆）②元行為證據？（規則對應的行為已不發生即退役）③仍被觸發？（死規則即刪）。機械承載＝`sb sopreview <三問結論>` 留痕（結論與戳記寫 flow-state），開新 ms（`--new-ms`）與結束（`sb end`）前閘驗本 ms 已審；無 SOP／ROADMAP 的專案不擋；機械基本功（updated 同步、零日期日誌行、零重複行與重複標題）未過即戳記不發；刪修加減皆可，變更走正常 commit（same-commit 文件先行）。
...
1. **計數（純觀測）**：hooks 於 PreToolUse 計數——`turnUsage`（本回合工具調用數＋起始時間；老闆下一則輸入重置）與 `usageTotals`（slug 累計）。工具調用數＝model 請求數的**上界代理**（每請求至少產出一個工具調用，批次並行時高估）——欄位誠實標名，真值在平台用量紀錄。計數不作任何干預——成本的控制在於看得見、結算得了、歸因得出（§1.11 遙測），由老闆事後依數據處置。
2. **迴圈斷路器（常開）**：hooks 對每次工具調用計**指紋**（工具＋操作全量信號——shell 取完整命令字串、其他工具取完整參數 JSON——的 hash，記 `turnUsage.fingerprints`，上限 128 鍵；同檔不同區段的讀取、同檔不同內容的編輯屬多樣操作各自計數，逐字重跑的同一操作才同指紋）。同指紋回合內**第 4 次**出現即擋該次調用——訊息要求改變策略（修根因／換方法／不同操作）；被擋後仍重複至**第 7 次**＝升級：**自動回 intent**（任何活動段；history 條目留 `budgetExhausted`——歷史鍵名，語義＝迴圈升級）並重置指紋表，依**修正分類**補正 G1~G3 後接續——CONFORMS 級（典型：執行策略失敗）細化 G2／G3、G1 不變；真屬 G1 衝突才走 §1.4.1 修約經老闆確認。**不凍結、不停擺**——升級後工作續行；同指紋**第二次升級**＝死操作（回 intent 補正後仍原樣重跑），本回合封禁該操作（防宏觀升級循環），其餘工作不受影響。CLI 不凍結前進（`escalatedAt`／`escalations` 屬純觀測）。理解停等期間只計數不升級（寫入與推進由停等凍結治理）。Skill 調用與 `sb state`／`sb next intent` 為逃生豁免面。門檻值為暫行值——由 §1.11 觀測數據回饋校正（元行為錨定）。
...
4. **消融**：拆掉擋停（Stop handler 判準）後，無申報之停全數放行——防護消失（消融矩陣對帳，§1.8）。
...
- **pass 出口（時點 2 老闆 pass 後）** — 老闆選擇 ms 出口：`sb next intent --new-ms --boss-ok --adversarial`（開下一里程碑）或 `sb end --boss-ok --adversarial`（結束 slug）；前置：verify 態、時點 2 對抗新鮮、老闆輸入新鮮度（晚於本 ms 末次進 verify）、working tree 乾淨、證據完整。收尾歸檔隨 end 執行（§1.7.2）。
...
flowchart LR
    ...
    SEC -->|放行 · 判決 · commit<br/>路由 · reset · pass 出口| Boss
```
...
### 實作層 — 落地段反向回指承載檔（Z 字形） · test→G3／build→G2／verify→G1

```mermaid
flowchart TB
    TST["test：依 G3 驗收排程撰寫功能測試"] --> DEV["build：回指 G2 實作＋單點／整合測試"]
    DEV --> ACC["verify：回指 G1 逐項 AC 判定"]
    ACC -->|判決通過·提交閘 commit·下一功能| TST
    ACC -->|紅燈·旗標切段回 test／build 續迭代| DEV
    ACC -->|功能迭代完成| CE2E["收斂期：test 寫 E2E → build 調環境 → verify 跑完整流程"]
    CE2E -->|綠燈收斂 → 時點2 對抗畢·老闆pass| OUT["pass 出口：--new-ms 或 sb end"]
```

> - **test 段** = G3 落地邊：主對話把 G3 驗收排程（AC-ID 映射 G1 驗收項）可執行化為測試碼並定義「過」；測試碼隨功能實作同 commit 定稿（不可變性由 git 承擔）。
> - **build 段** = G2 落地邊：主對話回指 G2 寫實作碼並執行相關單點測試、依 G3 接合點執行（必要時補寫）整合測試（偏離顯式記錄）。
> - **verify 段** = G1 裁判邊：主對話依 G3 核對當次功能證據、逐項 AC 判定（SATISFIED／UNSATISFIED／UNVERIFIED），段內對 repo 唯讀；功能判決屬段內秘書判決（agents 權責），pass/fail 判定權只在老闆兩時點。
...
**實作層時序制約**：test 撰寫測試並定義判準，build 執行單點／整合測試，verify 核對證據；收斂期由 test 寫 E2E、build 調整環境、verify 執行完整使用者流程。test 段定義的「過」回指 G3 驗收排程（AC-ID 映射至 G1 驗收項）；測試碼隨功能實作同 commit，不可變性由 git 承擔。紅燈若是實作問題，旗標切段回 build 修復且測試不動；若證明測試定義錯誤，記錄紅燈證據與理由，旗標切段回 test 重新定義——段內修復環不停等。verify 邊機械核對 working tree 乾淨。
...
- **loop**：跨階段、多檔或需跨回合保持狀態；維持 `Goal／Core／Verified／Open／Next` 短帳本（落檔一律 `<repo>/.shiftblame/tmp/`），每個 seam（子任務完成、寫檔、驗證或交付前）重讀並更新。壓縮或長中斷後先恢復帳本再繼續。
...
技術解法依序選擇第一個足夠成立的層級：確認需求確實存在 → 重用 repo 既有能力 → 標準函式庫 → 平台原生能力 → 已安裝依賴 → 最少可用實作。**沿用必經重驗**：既有決策、程式碼、模式與過往定案被沿用前，重驗其與當下需求、現況與實況的一致性——沿用是重驗後仍成立的結論，不是免評估的默認；錯誤決策造成的行為一經發現即修正該決策及其下游，不以「一直這樣做」「既有先例」「歷史慣例」為繼續沿用的理由（溯及既往——既有內容同受盤點）。修 bug 要追到所有 caller 共經的根因，在單一路徑治本——信任邊界驗證、防資料遺失錯誤處理、安全、無障礙或老闆明確要求；非平凡邏輯至少留下最小可執行檢查。已知天花板記入 SLUG 技術債與升級條件，不建立另一套註解或帳本體系。

**命名與內容可獨立辨識**：專案路徑、文件名、slug 名稱、程式識別字、註釋、字串、測試名稱與文件內容 MUST 以對象、功能、行為或責任表達意義，讀者可從專案正式內容找到明確定義與可解析的回指。只能靠當下對話、任務編號、修正輪次或臨時方案代號才能辨識的內容，屬 `.shiftblame/tmp/` 工作材料；補一條通往對話或 tmp 的連結不能替代定義。slug 在建立前依功能語義命名；同類命名沿用專案術語，代號展開成實際對象與行為。

**流程代號不進程式碼**：開發該專案時使用的 ms、slug、AC-ID 與步驟代號保持在流程載體，AC-ID 的定義與測試映射由 G1／G3 承載（§1 使用者驗收追溯鏈）。正式領域術語、技術縮寫、協定名稱、具有本地定義的章節鍵及可解析的機械識別碼依其實際語義使用；Shiftblame 自身對 G1、AC-ID、sb 等功能的實作、範本與測試，以框架內的正式定義為依據。是否依賴對話由主對話與既有對抗檢閱判定，CLI 的字元格式檢查不代表語義合格。

**對話與工作紀錄唯一落點**：凡記錄當下對話、推理討論、執行過程、修正歷程、工作帳本、交接或續跑落點的文件，一律寫入 `<repo>/.shiftblame/tmp/`。G1／G2／G3／SLUG 留在原位，只保留可獨立理解的收斂定義、必要狀態與回指；回指補充證據，定義本身可離開 tmp 理解。flow-state 承載既有機械狀態與輸入／理解流，敘事文件仍歸 tmp。提交訊息只描述變更本身；專案工具鏈原生產物依既有專案位置保存，agent 為交接、查證與報告製作的工作文件仍歸 tmp。既有內容依同一判準處理：工作敘事移入 tmp，正式內容改成自足語義，改名時同步所有引用與實際消費者。

對抗報告的既有路徑檢查由 `sb adversarial` 承擔：報告的 root 錨定路徑與解析連結後的實體位置皆位於 tmp，才可留下宣告；放在工作區根、G 檔旁或經連結越出 tmp 的報告先移入 tmp 並讀回，既有報告同樣適用。這項路徑檢查只驗報告入口，其他文件的語義與落點仍由寫入紀律及對抗檢閱承擔。

**溯及既往**：本規範同樣適用現存內容，舊檔、既有命名、已完成工作及歸檔文件皆納入盤點。接入、恢復與變更時依既有路由檢查目標專案，盤點範圍與逐項處置記入 tmp：可修改者改名、改寫或移出敘事，連同引用與消費者驗證；封存定義及受保護內容先記錄違例與影響，再依既有修約、回退及寫入權處理，列為未解決直至完成。清理針對當前檔案樹，Git 歷史保持原樣；原始工作材料與清理過程留 tmp。歷史慣例不構成豁免，部分檔案清理或字串掃描通過不等於全專案清零。

### 臨時外部子代理與受治理 worker

Shiftblame 不提供、安裝或依賴持久外部分工配置，也不因進入某個工作狀態而自動委派。需求定義、研究、規劃、測試、實作與驗收全部由同一主對話連續承載；外部子代理提供一次性唯讀意見（技術意見或對抗檢閱），以及**受治理寫入子代理**（research／build 段並行窗口，多代理架構）：子代理在 `.shiftblame/tmp/<工作區名>/` 內依任務卡（AC 對應、檔案邊界、單一寫入者）產出檔案與報告，**零 repo 寫入權**——不 commit、不跑 sb、不直接改 repo 檔（僅寫 tmp；tmp 不入庫、不觸發段位寫入權）；整合由主對話依子代理工作結果，以自身寫入權（段位白名單＋提交閘：adversarial＋commitmsg＋hooks）修改回當前所在分支（main 模式＝main 分支；slug 模式＝`<type>/<slug>` 分支）——commit、判決、放行、路由、pass 出口仍由主對話獨佔，子代理是可拋棄的一次性執行器（用完即棄，不成為持久角色）。工作區為拋棄式實驗空間——實驗碼（含測試嘗試與 G 檔實驗副本）不出 tmp；驗證器在 tmp 內跑測試產生執行證據屬 AUDIT 邊界允許的暫存執行推廣；驗完即整合（非同步流式，不等全體），tmp 以 slug 為單位清理（RAM 流清理規則）。

對抗邊推進一律帶 --adversarial 宣告——CLI 對照 adversarialLog point 條目與其新鮮度，缺條目或過期即擋（報告全文落 tmp）。下列情境分成**固定強制時點**、條件強制與選擇性觸發：

| 情境 | 主對話動作 | 路由老闆 |
|---|---|---|
| **時點 1 定義層放行（規劃收斂後、plan→test 放行前）** | **MUST** 取得一次子代理**對抗方向**檢閱——攻擊計畫本身：方向偏差、漏項、更簡路徑、premortem 挑戰、與 G1 語義偏離；複核經反向對抗，出口三分類處理至乾淨，結論呈現於放行簡報＋`sb adversarial <報告> --point 1` 條目（`--adversarial` 對照源＝adversarialLog）才交老闆判定，pass 才 `--boss-ok` 放行 | 否 |
...
簡單工作、例行階段切換與機械批次都不是委派理由；但兩個固定對抗時點與段內提交閘是老闆明文規定的強制項，全部走；純技術不可可靠裁定本身就是強制理由——技術題留在自身責任域，僅產品決策回老闆。固定時點檢閱的對抗任務仍須自包含：只給子代理判斷所需的材料（計畫／驗收報告／G1 契約），要求輸出攻擊點與反證，不移交工作狀態。

臨時檢閱必須使用自包含、一次性的任務描述，只要求來源、技術分析、反證、替代解讀或獨立核對——工作狀態、持久角色、對外視窗控制、repo 寫入、commit、判決與路由權全程留在主對話。主對話複核意見、保留完整上下文並承擔最終責任（結論經複核轉化，決策權在主對話）。固定時點的檢閱記錄 MUST 含四段：「對抗要點」（攻擊點）、「複核結論」（主對話逐點裁定：接受、駁回與對放行／判決／出口的影響，列點式並綁出處）、「反向對抗」與「附錄」。對抗檢閱 MUST 使用外部唯讀子代理——自代介面已從 CLI 移除：子代理工具不可用時流程阻塞等待（閘門不放行），阻塞期間保持等待；阻塞事實與恢復計畫向老闆如實報告即可（技術題仍在自身責任域）。
...
是否建立或沿用 `<slug>/<nnn>` 只由老闆決定。`<slug>` 承載跨循環不變的老闆原始命題、目標、授權邊界與租約；`<ms>` 承載一次收斂循環（三面向制衡→開發→三者重審）。**未開的待辦只寫在 SLUG §3 待辦清單簡述（一句價值），不開 ms**；老闆授權新里程碑（pass 出口 --new-ms）才開 `<ms>`（verify→intent 出口邊、建目錄），開工的待辦從 §3 清單移入 §4 段表。
...
- **既有 `<slug>` 開新 `<ms>`** — 同一大需求的新里程碑。路徑：時點 2 老闆 pass→`sb next intent --new-ms --boss-ok --adversarial`（老闆授權語義由理解流曝光承擔）→回 intent 展開新 ms（ms++）。不需先結束 slug。
- **結束 `<slug>`** — 整個 `<slug>` 所有子需求完成，老闆拍板結束。路徑：時點 2 老闆 pass→`sb end --boss-ok --adversarial` → 收尾歸檔（§1.7.2：移 archive＋closeout 查證）。
- **完結 ended（留 main 直接作業）** — slug 已結束、closeout 完成，老闆指示直接在基底分支繼續工作而不開新 slug。路徑：`sb init --main`（ended 驗證重跑＋當前分支＝closeout 基底；完結戳後直接實行語意——正常提交走 `sb adversarial`＋`sb commitmsg`）。
- **新增待辦到當前 `<slug>`** — 老闆想在當前 slug 加功能但不立即開 ms。路徑：秘書直接寫入 SLUG §3（老闆指示即授權；一句價值簡述、純短名無編號、清單末尾）；不開 ms、不改節點。
- **main 模式（直接實行，不開 slug）** — 框架演化、微修、文件變更（docs／SOP／ROADMAP——依 §1.7 文件先行＋same-commit＋寫入矩陣）或老闆指定 main 的輕量變更與快速試錯。性質＝人機交互快速迭代：老闆在場快速往返，工作直接在 main 分支，不建骨架；新需求進門時 agent 依 think 詢問守則主動提供 main／slug 二選一。
- **slug 模式（開 `<slug>`）** — 大型變更下的流程與進度管理機制（半自主：七段結構＋閘口承載進度，不需額外自主運行機制）。路徑：建立前依 §3 檢查 slug 是否以功能語義命名，再由 `sb init <slug> [type]` 建全骨架（flow-state＋`<slug>/001/`＋SLUG.md＋`archive/`＋`<type>/<slug>` 分支）並切換至 slug 分支從 intent 開始；G1~G3 範本由各定義段自 SLUG 範本節複製。
...
秘書 MAY 基於 §9 載入程序的脈絡在 shiftblame:think 中主動提出路由提議（沿用／開新 `<ms>`、開新 `<slug>`、直接實行、框架演化、完結留 main），提議須附脈絡依據。**提議不等於授權**：建立 `<slug>/<nnn>` 與預建檔案均須老闆明確拍板後建立與執行。老闆尚未決定時，秘書 陳述提議與脈絡後等待裁決。

## 7. 提交規範

歸檔合併的固定訊息依 §1.7.2：`sb commitmsg` 僅在合法 ended 狀態接受目前 slug 的精確 `merge <slug>` 訊息，仍須提交對抗、發章與 hooks 消費（完結戳後固定訊息退役——正常提交走通用格式）。新合併先以 `git merge --no-ff --no-commit` 準備，完成上述檢查後再用相同訊息 `git commit -m`。以下 type 與繁中描述規則適用一般提交。

**任何 commit MUST 過 `sb commitmsg` 機械驗證**（hooks 留痕硬擋），核心不變量：訊息 `<type>: <繁中描述>` 單行 10-30 字、純描述變更本身（分支名表達功能語義，工作紀錄歸 tmp）；slug 開發 MUST 在 `<type>/<slug>` 分支（sb init 自動建立切換；直接 main 為框架演化／緊急修復／輕量調整／完結後直接作業的例外）；repo 先例（歷史訊息、既有慣例）僅作脈絡參考——規範來源以 SKILL 條文與 CLI 閘為準；功能＝commit 單位、精準 add 不夾帶；每個功能實作完成 MUST 即時 commit 存檔建立待驗對象（commit＝存檔非完成印記，先於驗收；驗收邊核對 working tree 乾淨）。

**跨 repo 提交錨定**：外部 session 提交內部 repo 以 `git -C <絕對路徑>` 進行——`-C` 絕對目標即該提交段的查證錨點，staged 系統檔、commit 印章、對抗宣告消費與文件鐵律全對目標 repo 生效（章落目標專案、消費目標的對抗狀態）；相對 `-C` 一律擋（路徑展開元規則）。印章綁定錨定 repo：缺少印章時在本次提交的錨定專案（`-C` 目標或 hook cwd）跑 `sb adversarial`＋`sb commitmsg`。
...
└── skills/
    ...
    └── */SKILL.md               # 功能型技能（shiftblame:think 全域路由＋save/resume/dice 對話狀態與丟棄操作窗口＋rewrite 返工重寫紀律——寫入權與段位由流程與寫入矩陣承載，rewrite 只承載收斂為當下事實的操作紀律，不開新寫入權）
...
├── tmp/                           # agents 自由傾倒區：工作產物任意寫入，流程閘門零依賴（§3）；子代理工作區亦落此（受治理寫入子代理依任務卡產出，主對話整合回當前分支）；只寫不清（老闆手動清理；sb-usage.jsonl 觀測事件與 flow-rotated.jsonl 輪替事實落此——缺檔自動重建）
└── archive/
```
...
**hooks 機械注入（反偏移）**：plugin 內建 `hooks/hooks.json`（`hooks/shiftblame-guard.mjs`；單一 `command` 型配置多平台相容——ZCode 與 Codex 的 hooks schema 交集，同一份 hooks.json 兩端生效）。ZCode plugin hooks 直接生效；Codex（0.149+）安裝或更新 plugin 後須以 `/hooks` 審閱**信任一次**（hash 綁定，變更後重新信任；未信任＝hooks 不跑＝輸入流／理解流／外部證據標記缺失，CLI 閘擋時附 hooks 健康警示——hooks 每次成功執行寫心跳，閘擋對照心跳區分「未授權」與「hooks 故障／未信任」：記錄缺失≠授權缺失，修 hooks 而非繞閘）。四事件：`SessionStart` 注入載入程序＋不變量卡＋輸入流／理解流狀態（壓縮後自動回流——機械抗上下文壓縮）；`UserPromptSubmit` 輸入流唯增記錄＋停等狀態機（shiftblame:think 調用形式輸入設 hold、老闆回覆解凍）＋未審理解必然曝光＋狀態卡注入；`PreToolUse` 理解流記錄（Skill(shiftblame:think) 調用 args＝理解宣告）、停等凍結（hold 期間寫入類與流程推進硬擋——唯讀、外部查證、tmp 傾倒自由）、外部證據標記（WebSearch／WebFetch／webReader／web.run（web__run）／Agent；Codex 事件實名 webrun／collaborationspawn_agent／collaborationfollowup_task）、回合計數與迴圈斷路器（§1.10——計數純觀測零干預；同操作重複即擋、死圈升級自動回 intent 補正 G1~G3 續行，不凍結）、破壞性命令防護（遞迴刪除／覆蓋配相對路徑擋）、`git commit` 驗 `sb commitmsg` 留痕（staged 系統檔不入庫；含 `-C` 絕對目標的跨 repo 錨定——§7）、寫入矩陣（測試碼 test＋build 段、實作碼限 build／ended、G 檔分區——定義區綁定義邊 G1→requirement／G2→research／G3→plan，回指區綁落地段 G1←verify／G2←build／G3←test；跨區由 CLI 分區 hash 兜底）、層間停靠與 git 重定向／alias 防護；`Stop` 事件執行停點偵測（§1.12——活動流程無申報擋停一次；申報／停等／ended／無流程放行；不代做路由）。hooks 對話遺漏時回到文件層：不變量卡（本卡）與 CLI 閘門仍然完備；hooks 與 CLI 兩層 MUST 共用同一 repo root 判定（路徑展開元規則）——若 hooks 的 cwd 判定與 repo root 不一致，一律以錨定 repo root 為準（如 git 命令必以 `-C <絕對路徑root>`，且禁 GIT_DIR、`--git-dir`、`--work-tree` 等重定向繞過）。

載入本 skill 後，秘書 MUST 依序唯讀：

1. `<repo>/.shiftblame/SOP.md`、`<repo>/.shiftblame/ROADMAP.md` — 理解專案規則與路線圖（兩者受每 ms 審查三問約束——§1.9 修剪迴路：載入是為了執行與修剪，不是盲從）。**適用性評判先於執行**：每條規範套用前對照當下需求與實況判定適用性——規範與當下需求或實況衝突時，浮現衝突並走修正通道（修正既有過時內容＝§1.7 same-commit 同批更新；新增規範仍需老闆授權，寫入矩陣 §1.7），不以「SOP 這樣寫」為執行理由——規範是當下有效的方法本體，可被評判與修正，不是免評判的權威；評判結論（沿用／修正／請裁決）隨當下產出留痕。
2. 當下未歸檔的 `<slug>` — 理解當下開發脈絡：SLUG.md（含**定案索引**——同 slug 各過往 ms 的一行式定案：語義邊界／選型／架構決策＋ms 路徑）與目前節點；過往 ms 定案的原文回讀由段義務承載（requirement 對照過往 G1、research 選型查閱過往 G2，見 `references/`）——近者先於遠者。
3. `<repo>/.shiftblame/archive/` — 理解歷史開發脈絡（更遠的跨 slug 脈絡）。

**載入的根據——摘要不作數，讀檔才算數**：對話壓縮後的記憶摘要與 context 既有敘述（含早期對話記憶）不作規範或現狀來源——其中的規範、技能內容、流程狀態、授權與既有結論都可能過時、遺漏或失真，行動依據與現狀引用一律以當次讀檔為準；此把關不因把摘要轉錄到其他檔案而改變。規範與現狀以外部實體檔案為唯一權威（SOP、ROADMAP、SLUG、G1~G3、定案索引——定案索引僅指路，與 G 檔衝突時以 G 檔為準；技能檔含其 references）：凡引用規範、技能內容或流程狀態行事，以當次實際讀檔結果為據——context 中的既有敘述不構成已讀檔證據；與檔案不一致時一律以檔案為準。任務起手與恢復接續（含壓縮後）時重載對應檔案——本節載入程序即此義務的承載點；不變量卡⑫每則輸入注入同源原則。檔案不存在時如實回報缺漏，不視為錯誤。載入時同時依 §3「溯及既往」辨識既有命名與工作敘事問題；先將盤點結果記入 tmp，再依授權與所在段的寫入權處理，冷啟動的唯讀載入本身不授予修改權。
...
提議須附脈絡依據（引用 SOP／ROADMAP／當前 slug＋定案索引／archive——近者先於遠者）。**提議不等於授權**：建立 `<slug>/<nnn>`、預建檔案與寫入 repo 仍須老闆明確拍板後執行（§6）。本程序只把「不提供路由意見」修正為「給出有依據的提議待裁決」，不改變問題揭露不等於修改授權（§2）。路由提議在 shiftblame:think 中呈現，由老闆確認後分發。
...
>
> **授權連續性**——各段條件分別成立時觸發，但單一動作完成不是停點。主對話吸收產出後立即檢查下一條件；條件成立就自動觸發下一段，只有遇到老闆決策點（意圖確認、時點 1 放行、時點 2 pass 出口、開新里程碑）才停止。**時點 1 放行停靠**：規劃收斂後先完成時點 1 對抗（出口三分類處理至乾淨）、§10 核對與人話簡報，揭露後停等老闆判定（--boss-ok 留痕＋老闆輸入時戳新鮮度由 CLI 驗＋理解流曝光承擔），pass 才以 `sb next test --boss-ok --adversarial` 推進；不等判定自行推進即繞過老闆 checkpoint，CLI 與 hooks 雙重攔截（兩層判據一致）。**時點 2 出口停靠**：收斂期綠燈收斂後先完成時點 2 對抗至乾淨，交老闆判定 pass/fail——pass 才走出口（--new-ms 開新 ms 或 sb end 結束 slug），fail 視為老闆新輸入回意圖揭露。
...
- **→ think**（觸發：老闆任何輸入）
  - 通過：所有老闆輸入第一步路由回 shiftblame:think，不字面執行（未覆蓋即凍結機械保證）
...
- **路由指定 → 定義層**（觸發：`sb init` 建骨架，或回 intent 開新輪重走）
  - 通過：老闆已明確指定開新 `<slug>`、開新 `<ms>` 或沿用 `<ms>`
  - 不通過：停留，等待老闆決定
- **定義層 → 實作層**（時點 1：plan→test 放行邊——§10 核對＋時點 1 對抗畢＋老闆 pass `--boss-ok` 留痕）
  - 通過：定義層一逐功能完成 G1、G2、G3；規劃收斂進定義層二對齊至三份兩兩雙向一致（§10 核對通過）；時點 1 對抗方向檢閱並複核至乾淨（記錄寫 tmp）後老闆判定 pass。其他 §3 條件已觸發者須記錄結論或如實標示未驗。
  - 不通過：缺漏由責任面向旗標切段補正後重核（不打轉）；老闆 fail＝新輸入回意圖揭露。
- **實作層 → 出口判定**（時點 2：功能迭代完成＋收斂期 E2E 綠燈收斂＋時點 2 對抗畢＋老闆 pass）
  - 通過：本 ms 所有功能經實作層一小循環（test→build→verify→提交閘 commit）通過 → 收斂期 E2E 循環綠燈收斂、彙整逐項使用者行為證據、對照封存 G1 複驗（判定收斂寫 G1 回指區，不另問確認）→ 時點 2 對抗成果檢閱並複核至乾淨（adversarialLog point 2 條目）→ 三面向重審皆通過 → 老闆判定 pass；含 §1.7.1 輕量動作。
  - 不通過：紅燈旗標切段回對應段續迭代（不停等）；老闆 fail＝新輸入回意圖揭露，經 intent 路由器路由。
- **出口判定 → think**（觸發：pass 出口——老闆授權 `sb next intent --new-ms` 開新 ms；或老闆授權結束 → `sb end --boss-ok --adversarial` 結束 slug）
  - 通過：開新 ms——老闆明確決定開新 `<ms>`（通常從 SLUG §3 待辦清單取）；SLUG §4 加新列（§3 待辦清單移出）；舊 `<ms>` 列節點隨出口定案。結束 slug——老闆明確拍板整個 `<slug>` 結束。
