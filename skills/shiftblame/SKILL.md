...
metadata:
  version: "1.0.0"
description: 以時序制衡約束 agent——主對話秘書是唯一持久角色，連續承載需求、技術、計畫、測試、實作與驗收等工作狀態；階段是同一上下文中的狀態，不是角色身份或委派邊界。三份文件面向不變：G1 需求／G2 技術／G3 計畫。定義層依一次定律單向推進，§10 於放行前核對兩兩一致；執行層以測試＝G1 落地、實作＝G2 落地、驗收＝G3 落地正向同構。階段內依複雜度控制認知負載並選擇最小充分解，測試碼離開測試狀態後鎖定。只有確實需要不同視角時才臨時取得唯讀檢閱意見；檢閱不接管上下文或決策。commit、判決合格／返工、放行、路由、reset、PASS 一律由主對話獨佔。
---
...
flowchart TD
    ...
    subgraph Consult["定義層 · 定義該做什麼 · 一次定律：單向一次推進"]
        direction LR
        G1["審計狀態 · G1 需求<br/>主對話 · 一次確認定稿"] -->|薄研究<br/>向前對齊一次定稿| G2["研究狀態 · G2 技術<br/>主對話 · 選型結論＋可行性"]
...
flowchart TB
    ...
    subgraph Consult["定義層 · 定義該做什麼 · 一次定律：單向一次推進"]
        direction LR
        G1["審計狀態 · G1 需求<br/>主對話 · 一次確認定稿"] -->|向前對齊| G2["研究狀態 · G2 技術<br/>主對話 · 薄研究"]
...
> - **審計狀態** → 主對話產出 G1（需求／驗收標準），一次確認定稿；後續狀態向前對齊 G1。
...
- **開發 → ms 完成**（觸發：自動觸發收斂 §1.4.2）
  - 通過：合併收斂：本 ms 所有功能經執行層小循環（測試→實作→commit 存檔→驗收）且秘書判決通過 → 提交逐項使用者行為證據 → 老闆確認 ms 價值＋審計狀態對照封存 G1 複驗（結果存 tmp，不寫回 G1）→ 三面向重審皆通過 → 片段清空；只有 §3 條件成立時才加入一次臨時唯讀檢閱；含輕量保鮮 §1.7.1。
