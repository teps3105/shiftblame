...
> 對應 §0 圖：`[nnn 完成] → 輕量保鮮 → 老闆路由`。經 `/sb-proof` 收斂通過後到達 `nnn 完成` 節點（§11）；輕量保鮮是該節點之後的動作。
...
>
> 主圖與 SLUG 合法節點清單中的「證據」「三者重審」是 `/sb-proof` 的**內部子節點**（合併收斂），非獨立顯式觸發階段——它們包在 `/sb-proof` 的一次觸發內完成。
...
| 開發 | `<nnn>` 完成 | `/sb-proof` | 合併收斂：提交行為證據 → 三者各自重審主導文件皆通過 → AUDITOR 已派子代理獨立審核 → 片段清空；含輕量保鮮 §1.7.1 | 任一不通過回三權制衡（同 `<nnn>`） |
