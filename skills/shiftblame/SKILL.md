...
雙模式：**手動**（預設，每階段 L0 對話確認後執行 L(n)）與 **自動**（僅 L0 對話確認後 L1~L5 全自動執行，老闆可隨時喊停進入 L0）。部門依需求選擇：計畫事項→PM、開發事項→DEV。L0 為統一的溝通階段（停下並溝通），純對話、不寫文件。管理者向老闆報告目前狀態與下一步計畫，老闆可持續修改需求。L0 通過後由當下狀態決定分流進入哪個 L(n)。手動模式每個 L(n) 前皆有 L0；自動模式僅在開頭一次 L0。PM L5 PASSED 後老闆決定歸檔或留至下一對話執行 DEV。詳見 `MANAGE.md`。
...
| 模式 | L0 | 宣告 | 部門 | 分支 | worktree | 對話 |
|------|:--:|:----:|------|:----:|:--------:|------|
| 手動 | 每階段 | 每階段 | PM 或 DEV（依需求） | feat/\<slug\>（跨部門）或 main（單一部門） | 否 | 單一或持久化（PM→DEV 接續） |
| 自動 | 僅一次（開頭） | 單次（開頭） | PM 或 DEV（依需求） | feat/\<slug\> | 是 | 每角色新對話 |
