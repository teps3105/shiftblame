...
17. **外部工具整合總則（platform-agnostic）**：框架不綁平台——卸除任一外部工具核心流程仍完整可用（降級鏈／升級鏈／推進禁令／工具清單／驗收機制細則集中 TOOLS/）。TOOLS/ 按功能分類（GAME／MEDIA／AUDIT／DESIGN／E2E／RESEARCH／VISION），各階段按需載入對應分類（使用紀律，agent 依當輪 `<task>` 標的 Read 對應 TOOLS/ 檔，非結構強制）
