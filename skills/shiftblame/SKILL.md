...
14. **固定雙子代理**：永遠只開兩個子代理（G1 計畫視角 + G2 技術視角），不得增開。codex（原則 17）是外部獨立複審工具，不計入子代理數、不改變雙軌結構
...
17. **選用外部角色整合（以 codex 為首例：反方複審／成果驗收／外部查證擔任者）**：未安裝 codex 時框架完整可用（反方由子代理/管理者內部擔任、驗收由管理者自驗、外部查證由內建 MCP/web 搜尋擔任）；已安裝且生效時，下列需要外部獨立性／利益衝突隔離的工作交 codex 子代理——反方獨立複審、實作成果驗收、code review、圖像識別／外部資訊查證（本地工具不足時）。codex 整合有**並存的兩種運行模式**：①**派任務模式**（Claude Code 當主體、codex 當外部幫手、結果回流，走 `/codex:rescue`、`/codex:adversarial-review`、`/codex:review`）；②**環境切換模式**（`/codex:transfer`，老闆手動觸發，把當前 Claude Code session 整體轉移成 codex thread，轉移後 codex 為新主體、外部 codex 插件角色失效——codex 不需插件調用自己，框架結構在 codex 端由其內部機制承接）。transfer 與派任務是並存兩模式，非替代。與 shiftblame 定義檔明文要求的產出衝突時，shiftblame 定義檔條款優先。
    - **與固定雙子代理的關係（雙層）**：codex 不取代子代理反方、不是常駐第三軌。雙層運作——子代理先做初步反方（保留內部對抗），codex 再疊加獨立複審（外部利益衝突隔離）；正方仍由子代理/管理者撰寫。雙子代理結構不變，codex 是疊加的獨立複審工具。transfer（環境切換）不改變管理者收斂權威與雙軌結構——轉移後框架結構（分支／slug／NNN）在 codex 端由 codex 內部機制承接，session 主控權與收斂責任隨轉移交由 codex 端，但雙軌正反收斂的紀律不變
    - **啟用判準**：判斷 codex 是否啟用，問「這件事以內部身份做會不會自我背書／利益衝突／超出本地工具能力」——是，則交 codex；否，則內部做
    - **一律走官方 slash command（派任務模式）**：深度根因／大任務／反方複審標的「無 diff」→ `/codex:rescue`；反方對抗審查標的「已實作 diff」→ `/codex:adversarial-review`；一般 review → `/codex:review`；查狀態→`/codex:status`，取結果→`/codex:result <task-id>`。派發後管理者主動查 `/codex:status` 確認進度、`/codex:result <task-id>` 取結果——codex companion 背景 task 不在 harness 追蹤範圍，不會自動發完成通知，禁止被動等待（詳 CF-SOP #20）。禁止手搓 `Agent(subagent_type:...)`、自 Bash 跑 companion、Monitor 輪詢
    - **環境切換模式（transfer）**：`/codex:transfer` 把當前 Claude Code session 整體轉移成 codex thread，讓 codex 從老闆原話、G1/G2 正反、收斂與決策理由自行推斷意圖，不經管理者二手轉述（二手轉述會使 codex 過度工程、誤解老闆意圖）。**由老闆手動觸發**——當 Claude Code 無法解決問題、需把狀態整體移交 codex 時使用；轉移後 codex 為新主體，Claude Code 原需調用的 codex 外部框架全部轉為 codex 內部工作流觸發
    - **調用機制規範**：①`disable-model-invocation`（當前實例 plugin frontmatter）並非禁止流程自動觸發 codex，是禁止以 Skill 工具觸發——官方 slash command 底層跑 companion 屬正常機制，非違反禁令；②**呼叫入口須為 `/codex:*` slash command 本身**，不得手搓 wrapper／alias／腳本繞過官方指令；③觸發後追蹤責任仍在管理者（見派任務模式）；④frontmatter 以當前 plugin 為準，版本特定操作細節權威在 plugin，框架定義檔不複製
    - **追蹤紀律適用範圍**：派發後查 `/codex:status`、`/codex:result` 的追蹤紀律**僅適用派任務模式回流型指令**（rescue／adversarial-review／review，有 task-id、結果回流 companion）；**transfer 不適用此追蹤紀律**——transfer 是環境切換、fork 後無回傳通道（當前實例：transfer 不產生 job，status 恆 running、result 恆 No job found），對 transfer 查 status／result 不會得終態，**transfer 的完成確認由老闆在 codex 端直接看 thread 結論，不靠 companion**。此與上述「禁止 Monitor 輪詢」不衝突——後者禁的是對回流型任務用錯工具（companion 不走 stdout 通知，Monitor 等不到）；transfer 不回流是根本無追蹤標的可查，連 status／result 都無意義
    - **純文字任務禁操控工具**：反方複審／查證等純文字工作，任務 prompt 須明確寫「不得觸發 computer use、不得操作瀏覽器與桌面、只讀本機檔＋文字產出」。codex 擅自操控工具即越權，立即 cancel
    - **成果驗收門檻（slug PASS 前置，非 commit 前置）**：驗收把關的是「成果可否宣告完成／推進」，不是「可否 commit」。
        ...
        - **slug 結束前強制驗收（兜底）**：無論各 NNN 是否驗收，整條 slug 在老闆拍板 PASS 之前，本輪成果須交 codex 做獨立驗收（成果＋管理者驗收方法與數據交 `/codex:rescue` 或 `/codex:adversarial-review`）；**slug 末驗收走派任務回流型（rescue／adversarial-review），不走 `/codex:transfer`——驗收需結論回流供管理者收斂，transfer 是狀態轉移不回流**（未來 transfer 若支援結果回流可重評）。diff 過大時 codex 可要求分切片補驗。**三事件時序**：管理者宣告完成→codex 驗收→老闆拍板 PASS（驗收落於宣告完成與 PASS 之間）。codex 判退修→不得宣告完成、不得推進 PASS，rework 後重驗（預設疊新 commit，可逆可追溯）。此為 slug PASS 前置，不是 NNN gate
    - **圖像識別／外部資訊查證：內部優先，搜尋類遇本地硬約束即「必須」調 codex，不得帶缺口推進**：分兩類——① 讀 code／跑本地實測／查 repo 內既有檔＝內部既成事實，管理者自行做，不需 codex；② 外部資訊查證（搜尋類：WebSearch/WebFetch 查官方文件、版本號、bundle size、外部做法）管理者可先用內建 web 搜尋嘗試，**一旦本地工具出現硬約束——限流（429）、查不到、辨識精度不足，或因此形成查證缺口（記成 TD-WEBx）——調 codex 做外部查證即從「可以」升為「必須」**，且**不得帶著該缺口一路推進到正反收斂／PASS**（缺口只能押後到「codex 查證可行」的時機，不能押後到「假裝查過」）。Why：codex 走不同網路出口／限流配額，本地 429 不代表 codex 也 429；本地工具不足時不調 codex，等於明知有外部工具可用卻用 LLM 印象硬推，違反證據驅動（核心原則 4）。圖像品質裁判的最終權威仍是老闆的眼睛（GATE 視覺驗收原則 4），codex 圖像識別只當粗篩輔助
    - codex 的 skill 紀律內容權威在其自身 plugin，框架定義檔不複製
18. **決策邊界三條**：
