...
17. **外部工具整合總則（platform-agnostic）**：框架不綁平台——卸除任一外部工具核心流程仍完整可用（降級鏈／升級鏈／推進禁令／工具清單／驗收機制細則集中 TOOLS/）。TOOLS 採 `TOOLS/<功能英文大寫>/<具體工具>.md`，按行為功能載入（如 E2E/MEDIA/AUDIT），不按產品載體建大類；agent 依當輪 `<task>` 標的讀對應工具檔
