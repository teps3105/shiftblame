...
metadata:
  version: "2.4.2"
description: 時序制衡的 agent 協作框架——主對話連續承載意圖→需求→研究→計畫→測試→實作→驗收（requirement→research→plan→test→build→verify），段間切換一律 sb next 旗標切段（狀態同步＋留痕＋寫入權開關）。老闆新輸入未經意圖揭露（shiftblame:think 理解宣告）前 hooks 硬擋寫入與推進——揭露第一動由機械強制。pass/fail 判定權只在老闆兩時點：時點 1（requirement→research——審意圖→需求翻譯）與時點 2（build→verify——審驗收資格：GWT 回指、假綠燈），兩時點皆對抗在前、老闆判定在後，老闆章機械錨定本次對抗報告之後，中鏈零審核。verify＝真驗收執行：GWT 逐條＝驗收劇本實操、行為證據落回指區，完成後老闆看行為證據終審 pass 出口。main 模式最小環：意圖揭露→迭代→提交→老闆判 pass/fail。閘門只讀 git 事實與 flow-state；雙流模型（輸入流唯增＋理解流必然曝光）由機械層承擔。對老闆的輸出為人話——揭露首行＝翻譯、停等首行＝待決事（think SKILL 輸出形狀節）。機制細節見 references/MECHANISMS.md。
---
...
**工作連續性**：每個階段完成時，秘書 MUST 吸收產出、更新短帳本並立即判定下一個已授權狀態——階段完成、局部綠燈、壓縮或老闆沉默都不是停止許可。**連續以時點為界**：層內連續推進，兩時點（requirement→research 邊與 build→verify 邊）是強制停靠點（§11）；中鏈（research→plan→test→build）機械推進不停等。
...
**兩層兩段式（流程主幹）**：定義層寫理論、實作層寫實際，兩層完全對稱各分兩段——**定義層一**＝逐功能寫規劃循環（requirement→research→plan，plan 完成回 requirement 接下一功能；每次 requirement→research 邊過**時點 1**——審意圖→需求翻譯）→規劃收斂→**定義層二**＝整體規劃收斂循環（三份兩兩雙向一致，§10 秘書自查，補正至收斂）→plan→test 機械推進；**實作層一**＝逐功能迭代循環（test→build，單功能單提交——綠燈＋測試真實性核對判決通過經提交閘 commit 後回 test 接下一功能）→功能迭代完成→**實作層二**＝收斂期 E2E 綠燈收斂（test 寫 E2E→build 調環境執行至全綠——build 的功能是讓測試綠）→**時點 2**（build→verify 邊——對抗審驗收資格：GWT 回指、假綠燈，對抗在前老闆 pass 在後）→**verify＝真驗收執行**（GWT 逐條＝驗收劇本實操、行為證據落回指區）→驗收完成老闆終審 pass 出口。細節見 `references/MECHANISMS.md` §4~§6。

**pass/fail 判定權只在老闆兩時點**：時點 1（requirement→research 邊——審意圖有沒有正確被翻譯成需求）與時點 2（build→verify 邊——審驗收資格：GWT 有沒有回指到意圖、測試綠是不是假綠燈）——審核資源前移到需求與驗收兩接縫，讓問題在前期暴露，中鏈（research→plan→test→build）不再花時間與資源審核。兩時點皆**對抗在前、老闆判定在後**：時點 1 對抗與時點 2 對抗（子代理檢閱）出口三分類（①乾淨續行②僅需修復 agents 自動旗標切段回指定 node 續迭代不停③需老闆裁決停經 think）處理至乾淨，對抗乾淨後才交老闆判定 pass/fail；pass 才以 `--boss-ok` 留痕推進／出口（adversarialLog point 條目對照）。中間過渡一律中性流程推進，agents 不代老闆判 pass/fail。

**intent 與 done 在循環外；老闆新輸入回意圖揭露**：老闆任何新輸入——任何段位任何時刻，含兩時點 fail——一律回到意圖揭露（shiftblame:think；未覆蓋即凍結機械保證），揭露後由 intent 路由器路由。**返工輪計數**：僅老闆輸入驅動的返工（經揭露回 intent）計輪＋rewrite 載入閘；旗標切段、段內修復環、閘口自動返工不計輪。**出口**：verify 真驗收完成（行為證據落回指區）老闆終審 pass 後進 done 判定——`sb next intent --new-ms --boss-ok`（開下一里程碑）或 `sb end --boss-ok`（結束 slug 進 ended）——出口 `--boss-ok` 承載終審章，時點 2 對抗章已在進段前完成。

**段內提交閘（單功能單提交）**：實作層一每個功能完成即一次提交——測試碼＋實作碼同 commit，`sb commitmsg` 格式驗證＋印章（hooks 於 git commit 驗章焚章——純機械格式面）；AC 判決屬段內秘書判決（功能級），不佔時點編號。提交審核已於 2.4.0 移除——老闆不看中間產物（早已被翻譯成看不懂的技術文件），提交審核＝審核無人讀；審核資源集中在兩時點。
...
**審計層貫穿全程**：兩時點對抗的報告構成**對抗審計**（`references/AUDIT.md`——唯讀子代理、邊界、攻擊點、證據義務、複審閉環、閘門對接）；回 intent 事件構成**意圖審計**面（SLUG 回指紀錄＋history 時序＋理解流對照）。審計層只讀流程事實（git＋flow-state＋tmp 報告），本身零寫入權。

**固定強制對抗**：時點 1 對抗（requirement→research——審意圖→需求翻譯：GWT 能否從行為矩陣還原一列、翻譯保真）、時點 2 對抗（build→verify 邊——審驗收資格：GWT 回指、假綠燈）、技術不可可靠裁定時的強制技術意見（§3 表）——主對話一律複核後自行裁定，檢閱不承載工作狀態。**所有老闆輸入第一步路由回 shiftblame:think，不字面執行**（§6）。session 冷啟動的載入程序見 §9。
...
flowchart TB
    ...
    subgraph DEF1[定義層一 逐功能寫規劃循環·理論·旗標切段]
        R[requirement] -->|時點1 對抗畢·老闆pass 審意圖→需求翻譯| RS[research] --> P[plan]
        P --> RS --> R
        P -->|下一功能| R
    end
    ...
    subgraph DEF2[定義層二 整體規劃收斂循環·理論·旗標切段]
        R2[requirement] -->|時點1 對抗畢·老闆pass| RS2[research] --> P2[plan]
        P2 --> RS2 --> R2
    end
    ...
    B2 -->|時點2 對抗畢·老闆pass 審驗收資格：GWT回指·假綠燈| V3[verify<br/>真驗收執行 GWT逐條劇本]
    V3 -->|驗收完成·老闆終審pass 行為證據落回指區| DONE[done]
    DONE -->|開新ms| INTENT
...
> **確認綁定與雙流模型**：老闆確認的語義由 G1~G3、兩時點邊、測試、實作、驗收與 pass 出口一次承接（不綁定階段箭頭）；完成類鑰匙＝老闆決策邊 `--boss-ok` 留痕（CLI 驗輸入流時戳新鮮度——純時戳判定零語義）＋時點對抗＋理解流曝光。**雙流模型**：輸入流唯增（永不覆蓋、永不消費）＋理解流雜湊鏈唯增（必然曝光、抗上下文壓縮）——輸入不是鑰匙，未覆蓋即凍結是唯一機械前置；**觸發樣態——揭露第一動；未定案必問；無歧義即執行**。四牆全文（確認綁定／雙流模型／觸發樣態／如實天花板）見 `references/MECHANISMS.md` §1~§3。

## 1. 制衡與讀圖規則

1. **三面向制衡與輪內單向定律**：G1 需求、G2 技術、G3 計畫三面向權限對等，由秘書調控依序產出。輪內定義層單向推進（G1→G2→G3），一次產出、產出即定稿、直接向前對齊；**修正＝回 intent 開新輪**（`sb next intent` 輪次＋1，新輪重寫自洽——定義區整檔重寫、回指區同鍵覆寫，操作紀律見 shiftblame:rewrite）；歷史歸屬分層：repo 檔由 git 承擔，G/SLUG 覆寫即無歷史（跨輪時序由 flow-state history 承擔，需對照脈絡先傾倒 tmp）。G2 外部證據打底（進 research 段後至少一次外部工具調用，hooks 標記 externalEvidence、research→plan 邊驗；大型研究 MUST 外部唯讀子代理承擔，§3）。§10 兩兩一致是文件結構判準，於定義層二收斂時由秘書核對（plan→test 機械推進——一致性屬工作紀律非機械閘）；核對結果二分處置：**G1 清晰下的自身細節缺漏**＝僅需修復——agents 自動旗標切段回責任面向對應段一次補正（CONFORMS——補正後重進 requirement→research 邊即重封存 G1 契約，不計返工輪）；**G2/G3 與 G1 不一致**（矛盾、歧義）＝需老闆裁決——停經 shiftblame:think 呈現語義出入，老闆裁決後回 intent 開新輪（計返工輪＋rewrite 載入閘）由 requirement 段重新定義 G1。只有改變產品方向、G1 語義或授權範圍的重大例外（§1.4.1）才停止開發退回；符合 G1 的架構或技術改選由主對話自行裁定並細化 G2／G3。
2. **文件即制約規則**：每份文件只在其對應工作狀態產出；主對話切換狀態後，其他面向文件保持原產出（改寫權屬其對應工作狀態）。**秘書有全域時序調控權**。執行中間產物與對抗歷史存 `<repo>/.shiftblame/tmp/`（RAM）；G*.md 定義區只放決策結論（時點 1 邊封存時分區封存），回指區由對應落地段收斂寫入——不當流水帳。
3. **單一面向、兩兩一致**：每份文件只回答自己面向，但三份 MUST 兩兩雙向一致才可開發——一致性於定義層二收斂時依 §10 由秘書一次核對，缺漏＝旗標切段回責任面向對應段一次補正（CONFORMS，不計返工輪）後依序重走重核（輪內單向定律）。對齊軸為 G1 需求項，判準見 §10。
4. **兩層兩段式（開發主幹）**：開發由**主對話秘書**連續切換工作狀態並依 G3 推進；commit、判決合格／返工、推進、路由、reset、ms 出口皆由主對話獨佔。每個 `<ms>` 是一個里程碑（一組功能構成的使用者可觀察完整價值）；功能是 **commit 單位**，不屬於本 ms 的功能只記入 SLUG §3 待辦清單。定義層一（逐功能規劃循環——每次 requirement→research 過時點 1：對抗畢、老闆 pass 才 `sb next research --boss-ok --adversarial` 推進，point 1 留痕，審意圖→需求翻譯）→規劃收斂→定義層二（整體對齊至 §10 收斂）→plan→test 機械推進（中鏈零審核）；實作層一（逐功能迭代＋提交閘）→功能迭代完成→實作層二（收斂期 E2E 綠燈收斂——build 讓測試全綠）→時點 2 對抗畢、老闆 pass 推進 verify（point 2 留痕，審驗收資格：GWT 回指、假綠燈）→verify 真驗收執行（GWT 逐條劇本、行為證據落回指區）→驗收完成老闆終審 pass 走出口。**紅燈處置（段內，不停等）**：僅需修復者自動旗標切段回指定 node 續迭代（實作問題回 build、測試定義錯誤回 test 附紅燈證據與理由），不計返工輪；綠燈路徑一律經實作修正達成。**1.4.1 顯式修約／重大例外遷移（＝開新輪）**與**1.4.2 實作層二收斂期（E2E 綠燈收斂→時點 2 進段→真驗收→終審出口）**全文見 `references/MECHANISMS.md` §5~§6。
   ...
   **三層測試與執行時機**：測試依實際驗證對象分類，與工具、檔名、是否在 CI 執行正交。G2 說明測試邊界與依賴，G3 排定適用層級、接合點及 ms 完整流程；重用既有有效測試，每個功能只配置必要的測試。
   ...
   | 端到端測試（E2E） | 從真實使用者入口到最終結果的完整流程 | 實作層二收斂期（build 段內）：test 段撰寫、build 段調整環境並執行至綠燈收斂——build 的功能是讓測試綠，E2E 全綠是時點 2 進段前資格 |
   ...
   **證據沿用與有因重驗**：verify 核對測試版本、被測內容、依賴、環境及操作結果是否仍適用於待驗 commit；相同被測內容的存檔或進入 verify 本身不觸發重跑。每次重跑先記明修復、具體變更影響或證據失效原因；採足以涵蓋影響的最小測試集合。E2E 發現問題後先回對應環節修復，以單點／整合確認，再於時點 2 進段前重跑受影響的端到端流程；影響橫跨全部流程時才完整重跑，理由落 tmp。

   **G1 是封存契約，G2／G3 只准單調細化**：時點 1 邊（requirement→research，`sb next research --boss-ok --adversarial`）即封存邊——推進時將當下 G1 定義區的 SHA-256 記入 `flow-state.json`，本輪自進 research 起全鏈凍結；後續每次 `sb next` 都重算核對，G1 被改動即擋（requirement→research 邊本身例外——重進即重封存）。封存前的需求補正＝research→requirement 旗標切段（CONFORMS——補正 G1 後重進 requirement→research 即重封存，以補正版覆寫契約，不計返工輪）；封存後的開發中局部技術模型修正（不改變 G1 的使用者可觀察結果、範圍／例外、詞義與驗收成功集合）＝回 intent 開新輪至對應定義段單調細化 G2／G3（計返工輪＋rewrite 載入閘——落地段回定義段必經 intent，G1 契約隨回 intent 解除、需求重定義後再過時點 1 重封存；G 檔寫入權綁定義邊段位，落地段無上游寫入權）；codebase 現況、技術限制或局部最適解作為可行性證據供 G1 裁定——G1 的改寫權在修約路徑。出入一律顯式分類：**符合（CONFORMS）**→只修正 G2／G3 並繼續；**契約不足（UNDERSPECIFIED）／契約衝突（CONFLICTS）**→回 intent 開新輪重定義 G1，修約差異寫 tmp，G1 定稿後重新對齊 G2／G3、重新過時點 1。

   **使用者驗收追溯鏈**：G1 每項使用者需求 MUST 以唯一 `AC-ID` 定義使用者／前置／操作／可觀察結果／失敗邊界，證據固定為 `BEHAVIOR`；G3 以相同 ID 排程驗收並映射對應測試。AC-ID 只存在於 G1／G3 與驗收報告——程式碼保持與流程正交（§3）。test 段撰寫的測試碼隨功能實作同 commit 定稿——測試的不可變性由 git 承擔。驗收報告逐項產出 `SATISFIED／UNSATISFIED／UNVERIFIED`、commit、實際操作與觀察；引用專案輸出以**節錄快照**為證，非專案原檔（§3）。`verify` 邊機械核對 working tree 乾淨；功能判決核對 G3 當次範圍的行為證據，未排到或未驗的項目如實保留 UNVERIFIED；verify 真驗收（老闆終審出口前）才要求全部必填 AC 皆為 SATISFIED 行為證據——GWT 逐條劇本實操的結果。

   **開發序列複雜度門檻**——任一成立時，秘書將 G3 行動序列拆成 causal 小步；每步只能依賴已完成項；一致性的跨檔修改保持同一步：行動數 ≥ 4、跨檔數 ≥ 4、強依賴對 ≥ 2。每個功能由主對話在 test→build 段內完整走完，判決（綠燈＋測試真實性核對）通過經提交閘 commit 後回 test 接下一功能；判決含測試真實性核對——測試無真實斷言或與 G1／G3 無對應即判紅燈回 test 重寫。**功能是 commit 單位，ms 是驗收節點，時點 2 老闆 pass 是驗收進段邊（build→verify），老闆終審 pass 是 ms 出口**。
   ...
   **開發期間意圖路由**（時點 1 後、收斂前）：老闆以自然語言表達意圖時，秘書翻譯意圖並依條件自動執行對應狀態遷移（自然語言即授權，不另等確認）——需求方向變更回 G1、技術方向變更回 G2、計畫方向變更回 G3、開發完成走收斂期、無法明確對應時揭露翻譯結果請老闆確認遷移方向。判定圖見 `references/MECHANISMS.md` §4；顯式修約六步與收斂期六步見同檔 §5~§6。
...
原始命題、意圖翻譯與候選方案 MUST 分開；老闆命題保持原話。**意圖翻譯 MUST 是人話**——老闆能直接讀懂的白話；對老闆的輸出形狀依 `skills/think/SKILL.md`「輸出形狀（人話契約）」——揭露首行＝一句翻譯、待決＝具體問題、流程術語不進對話；讀不懂即揭露不通過（Checkpoint 1 天然承載）。框架演化只免除 slug，不免除方案揭露與修改授權。
...
- **pass 出口（verify 真驗收完成、老闆終審 pass 後）** — 老闆選擇 ms 出口：`sb next intent --new-ms --boss-ok` 或 `sb end --boss-ok`；前置：verify 態、老闆輸入新鮮度、working tree 乾淨、全部必填 AC 行為證據完整（時點 2 對抗與老闆 pass 已於 build→verify 進段前完成——出口 `--boss-ok` 是終審章，不重驗對抗）。收尾歸檔隨 end 執行（`references/MECHANISMS.md` §8）。
...
```mermaid
flowchart TB
    SEC["秘書（主對話）<br/>唯一持久角色 · 調控時序進程<br/>commit · 判決合格／返工 · 推進 · 路由 · reset · pass 出口"]
    ...
    subgraph Build["實作層 · 落地段反向回指承載檔（Z 字形）"]
        ...
        DEV --> ACC["verify 段 · 真驗收執行<br/>主對話 · GWT 逐條劇本實操對 G1 判定"]
    end
    SEC -- 調控時序 --> Consult
    SEC -- 調控時序<br/>讀 <repo>/.shiftblame/tmp/ 判決 --> Build
    G1 == "G1 閉環<br/>定義（requirement）→真驗收（verify）" ==> ACC
    G2 == "G2 閉環<br/>定義（research）→落地（build）" ==> DEV
    G3 == "G3 閉環<br/>定義（plan）→落地（test）" ==> TST
```

> - **老闆**：提出命題、授權修改、做決策、選 pass 出口。決定範圍。
> - **秘書（主對話，唯一持久角色）**：保留完整上下文並依序切換工作狀態；負責意圖揭露、G1-G3 產出、測試、實作、驗收、§10 核對、時點推進、判決、commit、reset、pass 出口、路由與收尾歸檔。未授權前唯讀。

**段-檔承載閉環**（Z 字形）：定義段 requirement→research→plan 依序定義 G1→G2→G3；落地段 test→build→verify 反向回收——test 回指 G3 驗收排程、build 回指 G2 技術方案、verify 裁判 G1 需求契約。文件定義後於落地邊復活當裁判，定義與實作由回指縫合。**實作層時序制約**：test 撰寫測試並定義判準，build 依判準實作並執行測試至綠燈（build 的功能是讓測試綠），verify 執行驗收劇本核對真實行為；紅燈若是實作問題，旗標切段回 build 修復且測試不動；若證明測試定義錯誤，記錄紅燈證據與理由，旗標切段回 test 重新定義——段內修復環不停等。verify 邊機械核對 working tree 乾淨。
...
Shiftblame 不提供、安裝或依賴持久外部分工配置，也不因進入某個工作狀態而自動委派。需求定義、研究、規劃、測試、實作與驗收全部由同一主對話連續承載；外部子代理提供一次性唯讀意見（技術意見或對抗檢閱），以及**受治理寫入子代理**（research／build 段並行窗口）：子代理在 `.shiftblame/tmp/<工作區名>/` 內依任務卡產出檔案與報告，**零 repo 寫入權**；整合由主對話以自身寫入權修改回當前所在分支——commit、判決、推進、路由、pass 出口仍由主對話獨佔，子代理是可拋棄的一次性執行器。工作區為拋棄式實驗空間——實驗碼不出 tmp；驗完即整合（非同步流式，不等全體），tmp 以 slug 為單位清理。

對抗邊推進一律帶 --adversarial 宣告——CLI 對照 adversarialLog point 條目與其新鮮度，缺條目或過期即擋（報告全文落 tmp）。下列情境分成**固定強制時點**、條件強制與選擇性觸發：

| 情境 | 主對話動作 | 路由老闆 |
|---|---|---|
| **時點 1 requirement→research 邊（需求翻譯推進前）** | **MUST** 取得一次子代理**對抗翻譯**檢閱——攻擊意圖→需求的翻譯：GWT 能否從行為矩陣還原一列、翻譯保真（標的方主語、可觀察結果、規格工程化）、範圍走私、消融答不出；複核經反向對抗，出口三分類處理至乾淨，`sb adversarial <報告> --point 1` 條目後才交老闆判定，pass 才 `--boss-ok` 推進（G1 契約同邊封存） | 否 |
| **時點 2 build→verify 邊（全部功能與 E2E 綠燈收斂後、進 verify 前）** | **MUST** 取得一次子代理**對抗回指**檢閱——審驗收資格：GWT 回指意圖、假綠燈（測試綠但 AC 從行為矩陣還原不出＝綠燈無效）、錯誤處置完整性；複核經反向對抗＋`sb adversarial --point 2` 條目——對抗乾淨後交老闆判定，pass 才 `sb next verify --boss-ok --adversarial` 推進真驗收 | 否 |
...
**標準攻擊點清單（對抗任務組裝 MUST 轉錄）**：組裝任何對抗任務時 MUST 把清單轉錄給子代理——通用面（誤讀意圖、漏邊界、範圍走私、夾帶未標注的新發明機制、過時描述、命名與內容依賴當下對話、忽略過往定案、產物與標的方價值斷鏈——產物存在但講不出它讓誰的什麼變好、規格工程化——GWT 主語非標的方，工程活動包裝成 AC、註釋與行為不一致、非授權路徑雜檔）、時點 1 面（翻譯攻防——行為矩陣還原、翻譯保真、工程活動走私進 AC、消融答不出）、時點 2 面（假綠燈、證據不足、漏驗、驗收回指意圖斷鏈、same-commit 文件缺漏、錯誤處置完整性）。全文見 `references/MECHANISMS.md` §14；清單是最低覆蓋，子代理可加攻擊面。
...
主對話在定義層依輪內單向定律切換需求定義→研究→plan 段，每一狀態一次產出、產出即定稿並直接向前對齊；三份完成後於定義層二收斂時親自核對 §10。時點 1 邊封存 G1 契約（本輪自進 research 起全鏈凍結），G2／G3 只准 CONFORMS 單調細化；UNDERSPECIFIED／CONFLICTS 走顯式修約。外部子代理檢閱於兩個固定時點強制（時點 1＝requirement→research 邊審意圖→需求翻譯、時點 2＝build→verify 邊審驗收資格，見前表），中鏈（research→plan→test→build）零審核資源——僅機械格式閘，其餘依本節強制／選擇性條件觸發。應執行而未跑的查證、檢閱或 e2e MUST 標「未驗」。
...
- 定義層三份文件 MUST 兩兩雙向一致（§10 於定義層二收斂時由秘書一次核對，缺漏責任面向一次補正），repo 才可改動。
...
- **既有 `<slug>` 開新 `<ms>`** — 同一大需求的新里程碑。路徑：verify 真驗收完成老闆終審 pass→`sb next intent --new-ms --boss-ok`→回 intent 展開新 ms（ms++）。不需先結束 slug。
- **結束 `<slug>`** — 整個 `<slug>` 所有子需求完成，老闆拍板結束。路徑：verify 真驗收完成老闆終審 pass→`sb end --boss-ok`→收尾歸檔（`references/MECHANISMS.md` §8）。
...
歸檔合併的固定訊息依 `references/MECHANISMS.md` §8：`sb commitmsg` 僅在合法 ended 狀態接受目前 slug 的精確 `merge <slug>` 訊息，仍經 `sb commitmsg` 發章與 hooks 驗章（完結戳後固定訊息退役——正常提交走通用格式）。新合併先以 `git merge --no-ff --no-commit` 準備，完成上述檢查後再用相同訊息 `git commit -m`。**合併政策三規則**：在 main 直接作業的工作屬於 main，無合併步驟；slug 工作經上述固定訊息合併回基底分支；外部協作倉庫依該倉庫自身的 issue／PR 策略。以下 type 與繁中描述規則適用一般提交。
...
**訊息與註釋的同源紀律**：提交訊息與代碼註釋同守「描述事實本身」——訊息純描述變更（追蹤編號、流程時序語零入訊息；semver 版號屬正式語義原樣書寫）；註釋純描述代碼行為（流程座標零入碼，§3 註釋紀律）。`sb commitmsg` 發章時對 staged diff 新增行掃**座標結構樣式**（輪次代號、時點圈號等小而穩定的結構集——框架機制檔路徑豁免，樣式集如實標註天花板）作為下限兜底，語義級由時點對抗（攻擊點清單「註釋與行為不一致」「命名與內容依賴當下對話」）與老闆抽查承擔。

**跨 repo 提交錨定**：外部 session 提交內部 repo 以 `git -C <絕對路徑>` 進行——`-C` 絕對目標即該提交段的查證錨點，staged 系統檔、commit 印章與文件鐵律全對目標 repo 生效；相對 `-C` 一律擋（路徑展開元規則）。印章綁定錨定 repo：缺少印章時在本次提交的錨定專案跑 `sb commitmsg`。
...
> 對應 §1.3、§4、§5 的引用。本節為兩兩一致的唯一權威定義——它是**定義層二收斂時的文件結構判準**；達成方式依輪內單向定律（§1.1）：輪內定義層單向推進中每階段向前對齊即定稿，本判準於定義層二收斂時由秘書一次核對（plan→test 機械推進——一致性屬工作紀律非機械閘），缺漏由責任面向一次補正後重核，不環形重跑。
...
> §0 主圖給流程形狀；下圖給階段遷移對應。**授權連續性**——各段條件分別成立時觸發，單一動作完成不是停點：主對話吸收產出後立即檢查下一條件，條件成立就自動觸發下一段，只有遇到老闆決策點（意圖確認、時點 1 需求翻譯推進、時點 2 進段判定、驗收終審出口、開新里程碑）才停止。**時點 1 停靠**：requirement 段完成需求定義後，先完成時點 1 對抗（審意圖→需求翻譯，出口三分類處理至乾淨）與人話簡報，揭露後停等老闆判定，pass 才以 `sb next research --boss-ok --adversarial` 推進（G1 契約同邊封存）；老闆章機械錨定本次對抗報告之後——老闆輸入早於本次對抗條目 CLI 即擋（2.4.2：防重走鏈中舊輸入被當成本次判定的 pass 章雙重消費）；不等判定自行推進即繞過老闆 checkpoint，CLI 與 hooks 雙重攔截。**plan→test 機械推進**：定義層二收斂（§10 秘書自查）後 plan→test 無老闆停靠——中鏈零審核（2.4.0：老闆不看中間產物）。**時點 2 進段停靠**：收斂期 E2E 綠燈收斂後先完成時點 2 對抗至乾淨，交老闆判定 pass/fail——pass 才 `sb next verify --boss-ok --adversarial` 推進真驗收（2.4.1 前移；老闆章同樣錨定本次對抗條目之後），fail 視為老闆新輸入回意圖揭露。**終審停靠**：verify 真驗收完成（GWT 逐條行為證據落回指區）老闆看行為證據終審 pass/fail——pass 走出口（`--new-ms`／`sb end`），fail 視為老闆新輸入。
...
stateDiagram-v2
    ...
    定義層 --> 定義層: 時點1 對抗畢 · 老闆pass（requirement→research）
    定義層 --> 實作層: plan→test 機械推進（中鏈零審核）
    實作層 --> 實作層: 紅燈 · 旗標切段續迭代（不停等）
    實作層 --> 定義層: 老闆新輸入定義級 · 回 intent 開新輪（計返工輪）
    實作層 --> 驗收執行: 全部功能完成 · 收斂期E2E綠燈收斂 · 時點2對抗畢 · 老闆pass（build→verify）
    驗收執行 --> 出口判定: 真驗收執行 · GWT逐條劇本行為證據 · 老闆終審pass
...
- **requirement → research**（時點 1 邊）：通過＝G1 完成經查證的現況事實＋BDD 行為規格；時點 1 對抗（審意圖→需求翻譯——GWT 能否從行為矩陣還原一列、翻譯保真、規格工程化）並複核至乾淨（記錄寫 tmp）後老闆判定 pass，`--boss-ok` 推進（G1 契約同邊封存）。不通過＝翻譯缺漏由 requirement 段修正後重審；老闆 fail＝新輸入回意圖揭露。
- **plan → test**（機械推進）：通過＝定義層一逐功能完成 G1、G2、G3；規劃收斂進定義層二對齊至三份兩兩雙向一致（§10 秘書自查通過）後 `sb next test` 推進——中鏈零審核，無老闆停靠。不通過＝缺漏由責任面向旗標切段補正後重核（不打轉）。
- **實作層 → 真驗收執行**（時點 2，build→verify 邊）：通過＝本 ms 所有功能經實作層一小循環通過→收斂期 E2E 綠燈收斂（build 讓測試全綠——build 的功能是讓測試綠）→時點 2 對抗（審驗收資格：GWT 回指意圖、假綠燈——測試綠但 AC 從行為矩陣還原不出＝綠燈無效）並複核至乾淨（adversarialLog point 2 條目）→老闆判定 pass，`sb next verify --boss-ok --adversarial` 推進。不通過＝紅燈旗標切段回對應段續迭代；老闆 fail＝新輸入回意圖揭露。
- **真驗收執行 → 出口判定**（老闆終審）：通過＝G1 GWT 逐條驗收劇本實操（Given 實際建立→When 實際操作→Then 觀察真實行為）、行為證據落回指區（判定收斂寫 G1 回指區）、全部必填 AC SATISFIED→老闆看行為證據終審 pass（含 pass 出口輕量動作，`references/MECHANISMS.md` §7）。不通過＝老闆 fail 視為新輸入回意圖揭露（修復旗標切段回 test／build）。
- **出口判定 → think**：通過＝開新 ms——老闆明確決定開新 `<ms>`（SLUG §4 加新列、舊 `<ms>` 列節點隨出口定案）；結束 slug——老闆明確拍板整個 `<slug>` 結束。不通過＝老闆未決定則持續等待（決定權全程在老闆）。

退回箭頭（開發途中）：老闆新輸入一律回意圖揭露，經 intent 路由器路由；定義級（G1 契約不足／衝突）＝回 intent 開新輪重定義，CLI 解除舊 G1 契約；段內修復＝旗標切段回指定 node 續迭代（不計返工輪）。G1 定稿後，G2、G3 重新兩兩雙向一致並再次過時點 1 邊才可開發——改義經時點 1 對抗與 §10 一致性顯式完成。
