...
**機制發明屬老闆決策域（防復發）**：agent 自行發明的機制（非老闆拍板語義的直接實作）MUST 在設計揭露中顯性標注為「新發明」並單獨取得老闆同意；藏在實作細節裡靜默通過屬越權設計，一經發現即拆除＋同類清查。意圖對抗與審計邊的標準攻擊點含「夾帶未標注的新發明機制」。
