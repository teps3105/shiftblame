...
4. **收斂循環**：開發採多循環螺旋——每圈一個功能、**先提交再階段驗收**（老闆確認功能正常＋AUDITOR 複驗寫回 G1，視需要輕量補 G2/G3 不重跑制衡），不合格返工並疊加新 commit，合格進入下一圈；全部功能完成後三者各自重審主導文件，發現新問題回三權制衡（同一 `<nnn>`），直到片段問題清空。是否開新 `<nnn>`／`<slug>` 只由老闆決定。
