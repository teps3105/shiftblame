...
17. **選用外部角色整合（以 codex 為首例：反方複審／成果驗收／外部查證擔任者）**：未安裝 codex 時框架完整可用（反方由子代理/管理者內部擔任、驗收由管理者自驗、外部查證由內建 MCP/web 搜尋擔任）；已安裝且生效時，下列需要外部獨立性／利益衝突隔離的工作交 codex 子代理——反方獨立複審、實作成果驗收、code review、圖像識別／外部資訊查證（本地工具不足時）。與 shiftblame 定義檔明文要求的產出衝突時，shiftblame 定義檔條款優先。
    ...
    - **一律走官方 slash command**：深度根因／大任務／反方複審標的「無 diff」→ `/codex:rescue`；反方對抗審查標的「已實作 diff」→ `/codex:adversarial-review`；一般 review → `/codex:review`；查狀態→`/codex:status`，取結果→`/codex:result <task-id>`。禁止手搓 `Agent(subagent_type:...)`、自 Bash 跑 companion、Monitor 輪詢。派發後等背景跑完通知
