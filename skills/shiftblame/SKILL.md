...
> **⚠️ 載入本 skill 後，必須立即執行以下步驟，不等使用者說話。**

1. **專案偵測**：用 `read_file()` 讀取 `.shiftblame/REPO.md`
   - 若不存在（檔案找不到）→ 用 `clarify()` 向老闆確認：
     ```
...
- CLI 能力不足時向上請求工具支援：subagent 透過 terminal() 呼叫 CLI 時可能被 sandbox 或安全掃描阻擋，或遇到 CLI 能力不足的情況。subagent 必須在 result.md 中記錄問題，嘗試替代方案後若仍不足，寫入 `[TOOL_SUPPORT_REQUEST]` 標記請求支援。秘書偵測到此標記後透過 clarify() 向老闆報告。禁止靜默降級或直接跳過任務。閘門不因 CLI 降級而退回（已記錄並有替代方案時），但須在報告中記錄。
