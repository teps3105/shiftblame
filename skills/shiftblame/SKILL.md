...
[SECRETARY 意圖揭露]
 ...
 [老闆是否明確授權修改？]
   ...
   ╚════════════════════════════════════════════╝
                │
         （三份兩兩一致 · SECRETARY 核對 §10 放行）
                │
                ▼
   ┌─────────────────────────────────────────────────┐
   │ [開發]──多循環螺旋，驗收節點是里程碑──           │
   │   SECRETARY 依 G3 里程碑推進，逐個功能           │
   │   派發落地側三權（低複雜度可由 SECRETARY 直接做）│
   │                                                   │
   │ 里程碑（一組功能構成的可觀察完整價值）           │
   │     ▼                                             │
   │ 逐個功能（commit 單位）：                        │
...
| G1 | AUDITOR 角色子代理（顧問側） | 需求制衡流程內；經 SECRETARY 核對 §10 後定稿 |
| G2 | RESEARCHER 角色子代理（顧問側） | 技術制衡流程內；經 SECRETARY 核對 §10 後定稿 |
| G3 | PLANNER 角色子代理（顧問側） | 實作計畫流程內；經 SECRETARY 核對 §10 後定稿。例外：G3 §2.5「階段驗收記錄」由 SECRETARY 補寫（記錄執行結果） |
| repo 實作碼 | DEVELOPER 角色子代理（落地側）或 SECRETARY | 開發階段內、SECRETARY 依 G3 派發；DEVELOPER 可寫實作碼但不可 commit；低複雜度功能 SECRETARY MAY 直接寫 |
| repo 測試碼 | TESTER 角色子代理（落地側）或 SECRETARY | 開發階段內、SECRETARY 依 G3 派發；TESTER 可寫測試碼（定義「過」）但不可 commit |
| repo commit | **SECRETARY（主對話）獨佔** | G1／G2／G3 兩兩雙向一致（SECRETARY 核對 §10）且通過開發門檻；落地側三權產出經 SECRETARY 判決合格後，由 SECRETARY commit |
| 顧問側子代理 | 無 | AUDITOR/RESEARCHER/PLANNER：**對 repo 唯讀**、工作區限 `.shiftblame/`；一律由主對話 SECRETARY 派發；在 `.shiftblame/` 內寫自己主導的管理文件（G1／G2／G3），研究／顧問／審核用途可寫中間產物至 `.shiftblame/tmp/`；**不得判決**；MUST NOT 寫 repo 程式碼、測試或 commit |
| 落地側子代理 | 無 | DEVELOPER/TESTER/ACCEPTOR：一律由主對話 SECRETARY 派發（子代理不能派生子代理）；DEVELOPER 可寫實作碼、TESTER 可寫測試碼、ACCEPTOR 不碰實作碼／測試邏輯但可寫測試環境配套（config/fixture/env vars）並可跑測試命令——**三者皆不可 commit**；產出存 `.shiftblame/tmp/`（子代理間唯一溝通橋樑）；**不得判決**（合格/返工、commit、放行、路由、reset、PASS 皆由 SECRETARY）；互相制約（寫碼/寫測試/環境配套分離） |
...
| **SECRETARY（主對話固定）** | **唯一決策中心**：意圖揭露、忠實記錄路由、交接、收尾與文件保鮮；派發上下六角色子代理、**親自核對 §10 一致性**、放行；開發中依 G3 派發落地側三權、讀產出後**判決合格/返工**、**獨佔 commit**；執行所有判決性工作（commit 保留/reset、跨權協調、PASS、路由判定） | 主對話唯一角色；未授權前唯讀 |

### 顧問側（上三權）— 定義「該做什麼」· 對 repo 唯讀 · 互相制約

| 角色 | 主導文件 | 制約（用自己的文件約束） | 執行方式 |
|------|----------|--------------------------|----------|
| AUDITOR | G1（需求／驗收標準） | 用 G1 制約 G2（技術須滿足需求）、G3（實作計畫須對應驗收） | 由子代理承載（角色為任務參數）；對 repo 唯讀 |
| RESEARCHER | G2（技術方案） | 用 G2 制約 G1（需求須技術可行）、G3（實作計畫須技術可落地） | 由子代理承載（角色為任務參數）；對 repo 唯讀 |
| PLANNER | G3（實作計畫） | 用 G3 制約 G1（需求須可實作）、G2（技術須可排程） | 由子代理承載（角色為任務參數）；對 repo 唯讀 |

### 落地側（下三權）— 執行「怎麼做」· 互相制約 · 判決歸 SECRETARY

| 角色 | 對應顧問 | 職責 | repo 權限 | 執行方式 |
|------|----------|------|-----------|----------|
| **DEVELOPER** | RESEARCHER（G2） | 寫 repo 實作碼 | **可寫實作碼**、不可 commit | 由子代理承載（角色為任務參數） |
| **TESTER** | PLANNER（G3） | 寫測試碼、定義「過」 | **可寫測試碼**、不可 commit | 由子代理承載（角色為任務參數） |
| **ACCEPTOR** | AUDITOR（G1） | 把東西修到綠燈、驗收「完成」 | **不碰實作碼／測試邏輯**、**可寫測試環境配套**（config/fixture/env vars）、可跑測試命令、不可 commit | 由子代理承載（角色為任務參數） |
