...
管理者由目前環境擔任，依狀態機閘門協調 PM 與 DEV 交替迭代。每個角色各有兩個面向：PM（規劃 / 品管）、DEV（開發 / 驗證），每次 NNN 由老闆指定觸發哪個面向。L3 紅隊攻擊與 L4 藍隊防禦依複雜度在對話內執行或開子代理。
...
雙模式：**手動**（預設，每階段 L0 對話確認後執行 L(n)）與 **自動**（僅 L0 對話確認後 L1~L5 全自動執行，老闆可隨時喊停進入 L0）。部門依需求選擇：計畫事項→PM、開發事項→DEV。每次 NNN 老闆指定面向（PM：規劃/品管；DEV：開發/驗證）。L0 為統一的溝通階段（停下並溝通），純對話、不寫文件。管理者向老闆報告目前狀態與下一步計畫，老闆可持續修改需求。L0 通過後由當下狀態決定分流進入哪個 L(n)。手動模式每個 L(n) 前皆有 L0；自動模式僅在開頭一次 L0。PM L5 PASSED 後老闆決定歸檔或留至下一對話執行 DEV。詳見 `MANAGE.md`。
...
| 模式 | L0 | 宣告 | 部門 | 分支 | worktree | 對話 | 面向 |
|------|:--:|:----:|------|:----:|:--------:|------|
| 手動 | 每階段 | 每階段 | PM 或 DEV（依需求） | feat/\<slug\>（跨部門）或 main（單一部門） | 否 | 單一或持久化（PM→DEV 接續） | 老闆在 L0 指定面向 |
| 自動 | 僅一次（開頭） | 單次（開頭） | PM 或 DEV（依需求） | feat/\<slug\> | 是 | 每角色新對話 | 老闆在 L0 指定面向 |
