...
5. 復判通過且老闆選擇「確認歸檔」→ 進入歸檔流程：
   ```
   秘書復判通過 → 刪除 worktree（git worktree remove）→ 歸檔（mv slug → archive）→ 推送（git push origin main）→ 更新 REPO.md → 刪除分支
   ```
6. 秘書依歸檔步驟順序執行收尾操作（見歸檔流程區段）
...
4. 復判通過且老闆選擇「確認歸檔」→ 進入歸檔流程：
   ```
   秘書復判通過 → 刪除 worktree（git worktree remove）→ 歸檔（mv slug → archive）→ 推送（git push origin main）→ 更新 REPO.md → 刪除分支
   ```
5. 秘書依歸檔步驟順序執行收尾操作（見歸檔流程區段）

秘書不建立或修改 MIS 部門報告。MIS 部門報告是 MIS 部門的產出，秘書無權代為產出。

### 修正作為新 slug 處理

歸檔後若需修正，秘書啟動新的 slug，走完整的 RES → ... → MIS 流程。不在原 slug 上修正。
...
**REPO.md 更新（push 後）：**
秘書在 push 成功後，必須依據 MIS 收尾產出的差異報告更新 `.shiftblame/REPO.md`。
