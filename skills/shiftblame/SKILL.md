...
name: shiftblame
revision: 0.9.0
description: 以時序制衡約束 agent——秘書（主對話）是唯一持久角色，職責是調控時序進程到合適的工作階段（需求→技術→計畫→測試→開發→驗收）；這些是工作階段而非角色身份，全部去人化。三份文件面向不變：G1 需求／G2 技術／G3 計畫。定義層受一次定律約束——單向一次推進：G1 需求一次確認定稿後，薄研究（G2）、薄規劃（G3）直接推進到實作，不在定義流程打轉；§10 兩兩一致於放行前一次核對，缺漏由責任面向一次補正，重大例外（§1.4.1）才退回。承載歸屬依對外視窗控制原則——涉及對外視窗控制的階段（研究技術 G2、開發）由主對話親自執行；不涉及對外視窗的階段（需求 G1、計畫 G3、測試、驗收）由秘書派發子代理執行。子代理是無思想的外包執行單元，基於秘書決策工作，無決策權、無對外視窗控制權、不可 commit；跨子代理結論與證據存 .shiftblame/tmp/（唯一溝通橋樑）。G3 每項驗收須可自動化執行（子代理獨立跑，不依賴對外視窗）。commit、判決合格／返工、放行、路由、reset、PASS 一律由秘書獨佔。每個 ms 就是里程碑（一組功能構成的使用者可觀察完整價值），所有功能 commit 後直接收斂，不合格返工疊加新 commit，ms 完成後做輕量保鮮，老闆決定開新 ms 或結束 slug，結束 slug 才走 PASS 與完整收尾保鮮。
---
...
本圖是流程權威。**秘書**（主對話）是唯一持久角色，職責是調控時序進程到合適的**工作階段**。工作階段（審計→研究→規劃→測試→開發→驗收）不是角色身份，全部去人化——它們是時序上的活動節點，由秘書調控推進、在每階段決定自己做或派發子代理。三份文件面向不變：G1 需求／G2 技術／G3 計畫，§10 兩兩一致於放行前一次核對後放行。**承載歸屬依對外視窗控制原則**（§3）：涉及對外視窗控制的階段（研究 G2、開發 G2）由主對話親自執行；不涉及對外視窗的階段（審計 G1、規劃 G3、測試 G3、驗收 G1）由秘書派發子代理執行。子代理是無思想的外包執行單元，基於秘書決策工作。定義層（定義）與執行層（執行）構成**鏡像迴圈**：審計(G1)→研究(G2)→規劃(G3) 正向展開定義，**受一次定律約束——單向一次推進、薄產出、直接推進到實作，不在定義流程打轉（§1.1）**；測試(G3)→開發(G2)→驗收(G1) 反向收攏執行，在 G1 驗收處閉合。**所有老闆輸入第一步路由回 sb-think，不字面執行**（§6）。session 冷啟動的載入程序見 §9。
...
flowchart TD
    ...
    subgraph Consult["定義層 · 定義該做什麼 · 鏡像上半 · 一次定律：單向一次推進"]
        direction LR
...
1. **三面向制衡與一次定律**：G1 需求、G2 技術、G3 計畫三面向權限對等，由秘書調控其對應工作階段依序產出。**一次定律**：定義層單向一次推進——時序固定 審計(G1)→研究(G2)→規劃(G3)，每個階段**一次產出、產出即定稿**；產出時直接向前對齊（G2 對齊 G1、G3 對齊 G1＋G2）。**G1 需求一次確認定稿後，薄研究（G2）、薄規劃（G3）直接推進到實作，不在定義流程打轉。** §10 兩兩一致是文件結構判準，於放行前（`sb-do`）一次核對；核對缺漏由**責任面向一次補正**後重核，MUST NOT 環形重跑三面向制衡。只有重大例外（需求不可行／改變方向或架構、需重新授權，§1.4.1）才停止退回對應階段重新定義，退回後仍單向重走。
2. **文件即制約規則**：每份文件只由其對應面向的工作階段產出，保持乾淨的決策結論；MUST NOT 直接改寫他人面向文件。**秘書有全域時序調控權**，決定流程調度、派發與段落寫入歸屬（如 G3 §2.5 收斂執行記錄由秘書補寫）。執行層各階段的執行中間產物存 `.shiftblame/tmp/`，不寫入 G1/G2/G3——G*.md 只放決策結論，不當流水帳。向前對齊時發現前面文件有缺漏（如 G3 發現 G2 缺技術分析），由秘書協調責任面向**一次補正**後繼續推進（一次定律，§1.1），MUST NOT 環形重跑迫使三份重新一致。
...
- **三面向制衡 → 開發**（觸發：`sb-do` skill）
  - 通過：G1、G2、G3 各由對應面向依一次定律單向一次產出定稿（審計/規劃階段子代理、研究階段主對話）；三份兩兩雙向一致（§10 放行前一次核對通過）；觸發獨立研究條件（複雜／高風險／無先例／與老闆直覺衝突／老闆指定，§3）者已由秘書代派子代理複核（結論存 `tmp/`）——未觸發條件者不要求配套，不阻擋放行。
