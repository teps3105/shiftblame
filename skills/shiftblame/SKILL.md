...
部門產物皆寫入該部門任務目錄的 `result.md`，每部門固定三段：專案計畫為需求釐清 + 市場研究 + 產品規格，品質保證為安全標準 + 操作標準 + 系統規格，產品開發為技術規劃 + 技術設計 + 技術實作，驗收上線為驗收計畫 + 驗收報告 + 驗收結論。這些名稱不是額外檔名，不得建立同名 `.md` 檔作為替代產物。

功能開發管線以本輪使用者明確想實現的功能為中心：先開專案計畫，在 `result.md` 產出需求釐清、市場研究、產品規格，完成市場調查、通用方法、設計模式、CVE 或版本差異等建立標準前的研究；品質保證再依專案計畫結果產出安全標準、操作標準、系統規格，並承擔原專案計畫的產品規格、任務拆解與實作規劃職責。不得把 `.shiftblame/ROADMAP.md` 內的既有規劃當成本輪必做內容，除非使用者本輪明確要求。

進入產品開發前，管理者必須先用中文詢問老闆想先完成品質保證結果中的哪個可見功能，並把本回合要實際開發的功能寫成非技術描述。產品開發必須先在 `task.md` 建立技術規劃、技術設計、技術實作的前置內容，再依此開發；開發期間允許老闆多次提出「想看目前變化」或「下一步想調整什麼」。管理者提供可操作 URL、畫面、截圖或驗證結果，這類即時預覽不取代正式閘門。
