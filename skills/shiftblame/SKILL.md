...
metadata:
  version: "2.4.0"
description: 以時序制衡約束 agent——主對話秘書是唯一持久角色，連續承載意圖、需求、研究、計畫、測試、實作與驗收；意圖揭露與六段流程（requirement→research→plan→test→build→verify）以兩層兩段式承載（定義層與實作層完全對稱），所有段間切換一律旗標切段（sb next＝flow-state 同步＋留痕＋寫入權開關三位一體）。未覆蓋即凍結：老闆新輸入未經意圖揭露前 hooks 硬擋一切寫入與推進——揭露第一動由機械強制。pass/fail 判定權只在老闆兩時點（時點 1＝requirement→research 邊——審意圖→需求翻譯；時點 2＝ms 出口——驗收回指意圖），審核資源集中在兩接縫、中鏈零審核，兩時點皆對抗在前、老闆判定在後；main 模式最小制衡環：意圖揭露→迭代→提交→老闆判 pass/fail。閘門只讀 git 事實與 flow-state；雙流模型（輸入流唯增＋理解流必然曝光）由機械層承擔。機制細節見 references/MECHANISMS.md。
---
...
**工作連續性**：每個階段完成時，秘書 MUST 吸收產出、更新短帳本並立即判定下一個已授權狀態——階段完成、局部綠燈、壓縮或老闆沉默都不是停止許可。**連續以時點為界**：層內連續推進，兩時點（requirement→research 邊與 ms 出口）是強制停靠點（§11）；中鏈（research→plan→test→build→verify）機械推進不停等。
...
**兩層兩段式（流程主幹）**：定義層寫理論、實作層寫實際，兩層完全對稱各分兩段——**定義層一**＝逐功能寫規劃循環（requirement→research→plan，plan 完成回 requirement 接下一功能；每次 requirement→research 邊過**時點 1**——審意圖→需求翻譯）→規劃收斂→**定義層二**＝整體規劃收斂循環（三份兩兩雙向一致，§10 秘書自查，補正至收斂）→plan→test 機械推進；**實作層一**＝逐功能迭代循環（test→build→verify，單功能單提交——判決通過經提交閘 commit 後回 test 接下一功能）→功能迭代完成→**實作層二**＝收斂期 E2E 循環（test 寫 E2E→build 調環境→verify 跑完整流程，修至綠燈收斂）→**時點 2**。細節見 `references/MECHANISMS.md` §4~§6。

**pass/fail 判定權只在老闆兩時點**：時點 1（requirement→research 邊——審意圖有沒有正確被翻譯成需求）與時點 2（ms 出口——驗收有沒有回指到意圖）——審核資源前移到需求與驗收兩接縫，讓問題在前期暴露，中鏈（research→plan→test→build→verify）不再花時間與資源審核。兩時點皆**對抗在前、老闆判定在後**：時點 1 對抗與時點 2 對抗（子代理檢閱）出口三分類（①乾淨續行②僅需修復 agents 自動旗標切段回指定 node 續迭代不停③需老闆裁決停經 think）處理至乾淨，對抗乾淨後才交老闆判定 pass/fail；pass 才以 `--boss-ok` 留痕推進／出口（adversarialLog point 條目對照）。中間過渡一律中性流程推進，agents 不代老闆判 pass/fail。
...
**固定強制對抗**：時點 1 對抗（requirement→research——審意圖→需求翻譯：GWT 能否從行為矩陣還原一列、翻譯保真）、時點 2 對抗（ms 出口——驗收回指意圖）、技術不可可靠裁定時的強制技術意見（§3 表）——主對話一律複核後自行裁定，檢閱不承載工作狀態。**所有老闆輸入第一步路由回 shiftblame:think，不字面執行**（§6）。session 冷啟動的載入程序見 §9。
...
flowchart TB
    ...
    V2 -->|時點2 對抗畢·老闆pass 驗收回指意圖| DONE[done]
    DONE -->|開新ms| INTENT
...
| **時點 2 ms 出口前（收斂期綠燈收斂後、pass 出口前）** | **MUST** 取得一次子代理**對抗回指**檢閱——驗收回指意圖：測試綠但 AC 從行為矩陣還原不出＝假綠燈；攻擊驗收證據對意圖的回指真實性（全 G1 滿足、錯誤處置完整性）；複核經反向對抗＋`sb adversarial --point 2` 條目（兩 pass 出口都驗新鮮度）——對抗乾淨後交老闆判定 pass/fail | 否 |
...
主對話在定義層依輪內單向定律切換需求定義→研究→plan 段，每一狀態一次產出、產出即定稿並直接向前對齊；三份完成後於定義層二收斂時親自核對 §10。時點 1 邊封存 G1 契約（本輪自進 research 起全鏈凍結），G2／G3 只准 CONFORMS 單調細化；UNDERSPECIFIED／CONFLICTS 走顯式修約。外部子代理檢閱於兩個固定時點強制（時點 1＝requirement→research 邊審意圖→需求翻譯、時點 2＝ms 出口前驗收回指意圖，見前表），中鏈（research→plan→test→build→verify）零審核資源——僅機械格式閘，其餘依本節強制／選擇性條件觸發。應執行而未跑的查證、檢閱或 e2e MUST 標「未驗」。
...
> §0 主圖給流程形狀；下圖給階段遷移對應。**授權連續性**——各段條件分別成立時觸發，單一動作完成不是停點：主對話吸收產出後立即檢查下一條件，條件成立就自動觸發下一段，只有遇到老闆決策點（意圖確認、時點 1 需求翻譯推進、時點 2 pass 出口、開新里程碑）才停止。**時點 1 停靠**：requirement 段完成需求定義後，先完成時點 1 對抗（審意圖→需求翻譯，出口三分類處理至乾淨）與人話簡報，揭露後停等老闆判定，pass 才以 `sb next research --boss-ok --adversarial` 推進（G1 契約同邊封存）；不等判定自行推進即繞過老闆 checkpoint，CLI 與 hooks 雙重攔截。**plan→test 機械推進**：定義層二收斂（§10 秘書自查）後 plan→test 無老闆停靠——中鏈零審核（2.4.0：老闆不看中間產物）。**時點 2 出口停靠**：收斂期綠燈收斂後先完成時點 2 對抗至乾淨，交老闆判定 pass/fail——pass 才走出口，fail 視為老闆新輸入回意圖揭露。
...
- **實作層 → 出口判定**（時點 2）：通過＝本 ms 所有功能經實作層一小循環通過→收斂期 E2E 綠燈收斂、彙整逐項使用者行為證據、對照封存 G1 複驗（判定收斂寫 G1 回指區）→時點 2 對抗（驗收回指意圖）並複核至乾淨（adversarialLog point 2 條目）→三面向重審皆通過→老闆判定 pass（含 pass 出口輕量動作，`references/MECHANISMS.md` §7）。不通過＝紅燈旗標切段回對應段續迭代；老闆 fail＝新輸入回意圖揭露。
