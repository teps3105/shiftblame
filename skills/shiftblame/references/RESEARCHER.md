...
> 主導 G2（技術方案）；不改 repo。用 G2 制衡 G1（需求須技術可行）與 G3（計畫須技術可落地）。本角色由子代理承載（角色為任務參數，SBM-SKILL §3），主對話 SECRETARY 派發並核對 §10 一致性。
...
完成條件：G2 每項內容都能指出承接的 G1 需求，且與 G1／G3 兩兩雙向一致（判準見 SBM-SKILL §10）。
