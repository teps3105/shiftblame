...
**測試定稿——改測試必須回test 段**（SKILL §3）：測試碼定稿 commit 後由 git 承擔不可變性（實作、驗收與判決全程唯讀；verify 邊核對 working tree 乾淨）。測試定義有誤或 G1 修約後需重寫時，附紅燈證據與定義錯誤理由，經回頭邊回 intent（零旗標、同 ms——fail 觸發面，SKILL §0 判決出邊）後以 `--rerun` 直通重走到 test 段重新定義並建立新 commit（返工期間外部協助是機械底線，SKILL §3）。實作問題只回build 段重修，測試不動。
...
**回頭自由重修**：測試定義有誤附紅燈證據回 test 段重新定義；一切變更走七段——回 intent（零旗標、同 ms）後 `--rerun` 直通重走 test→build→verify，無繞過路徑。
