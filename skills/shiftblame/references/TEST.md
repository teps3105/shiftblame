...
> **執行層工作段**。主對話把 G1 驗收標準可執行化為測試碼，定義「什麼叫過」。此狀態可寫 repo 測試碼；結束時測試碼定稿 commit（git 承擔不可變性），commit 與合格／返工判決仍由秘書執行。

- **產出**：依 G1（audit 段）驗收標準寫 repo 測試碼，定義「過」的條件
- **對應**：audit 段（G1）——同面向的定義（G1 驗收標準）↔ 落地（測試碼）
...
測試定稿＝測試碼於 test 段 commit。test→build 邊無機械閘（假測試由文件層判準擋下：無斷言不得定稿，git 歷史可稽）。測試碼 MUST NOT 含任何流程代號——AC-ID 與測試的映射由 G3 驗收排程承載（SKILL §1 驗收鏈、§3）。
...
**測試定稿——改測試必須回test 段**（SKILL §3）：測試碼定稿 commit 後由 git 承擔不可變性；實作、驗收或判決當下均不得修改（verify 邊核對 working tree 乾淨）。測試定義有誤或 G1 修約後需重寫時，附紅燈證據與定義錯誤理由，顯式回test 段重新定義並建立新 commit。實作問題只回build 段，測試不動。
...
秘書判決時**檢查測試真實性**：測試對應哪個 G1 AC-ID、斷言是否真實求值。判假即返工。若現有證據不足以可靠判斷測試真實性，依 SKILL §3 必須取得一次外部子代理唯讀技術意見，主對話複核後自行判決，不得請老闆判斷測試設計；
