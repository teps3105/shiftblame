---
name: 開發
revision: 0.9.4
---
# 實作階段 — 依 G2 寫實作碼＝G2 的落地（執行層 · 主對話承載）

> **執行層工作階段**。**G2 的落地面向**：把 G2 定義的技術方案落地為 repo 實作碼。**承載：主對話**（非子代理）——實作階段反覆實機驗證行為，涉及對外視窗控制，控制權不可外流（SKILL §3 承載歸屬原則）。可寫 repo 實作碼；該功能實作完成後由秘書 commit 存檔（先於驗收，建立待驗對象）並在驗收後判決通過/返工——二者皆由秘書獨佔。秘書在實作階段親自執行。

- **產出**：依 G2（研究階段）技術方案寫 repo 實作碼＋實機驗證
- **對應**：研究階段（G2）——同面向的定義（G2 技術）↔ 落地（實作碼）
- **時序制約**：寫出的實作要能通過測試階段定義的測試（由驗收階段跑）
- **不做**：不寫測試碼（測試階段職責）、不跑驗收（驗收階段職責）、不判決通過/返工（判決在驗收後，讀驗收產出）

### 本階段在三面向雙面結構中的位置

```mermaid
flowchart TB
    SEC[秘書<br/>主對話 · 唯一持久角色]
    subgraph Consult["定義層"]
        direction LR
        AUD["審計階段<br/>G1 · 子代理"]
        RES["研究階段<br/>G2 · 主對話"]
        PLA["規劃階段<br/>G3 · 子代理"]
    end
    subgraph Build["執行層 · 時序收攏"]
        direction LR
        TST["測試階段<br/>子代理"]
        DEV["**實作階段<br/>主對話**"]:::thisrole
        ACC["驗收階段<br/>子代理"]
    end
    SEC -- 調控時序 --> DEV
    RES == "同面向<br/>技術" ==> DEV
    classDef thisrole fill:#ffe082,stroke:#f57f17,stroke-width:3px;
```

> 實作階段（執行層）高亮。寫的實作要能過測試階段的測試（由驗收階段跑）；同面向對應研究階段（G2 技術）。二者皆由主對話承載。

### 承載歸屬：為何實作階段是主對話

實作階段高頻需要**反覆實機驗證行為**——改一行、跑一次、看行為、再改。這涉及對外視窗控制（有副作用、難重現、session 敏感），依 SKILL §3 承載歸屬原則，控制權必須在主對話。因此整個實作階段由秘書親自執行，不派子代理。

**認知探索外包**：實作階段的海量無副作用認知工作（讀既有 codebase 理解結構、grep 找模式、查 API 用法）由秘書派發**唯讀探索子代理**外包——子代理產出結論存 `<repo>/.shiftblame/tmp/`，秘書讀取後決策。**判準：子代理能獨立完成且無副作用 → 外包；需要對外視窗控制 → 留主對話。**

### 實作碼的產出與紀律

秘書在實作階段寫 repo 實作碼，依 G2 技術方案落地。執行產出**結構化整理後存 `<repo>/.shiftblame/tmp/build-*.md`**（變更摘要：改了哪些檔、做了什麼、實際用了什麼技術、與 G2 分析的出入、codebase 事實，**必含實機驗證方式與結果**——`sb next verify` 閘門查核本記錄，SKILL §1.8），**不寫入 G2**——G2 保持乾淨只放研究階段的決策結論。

**與測試／驗收階段的時序制約**（SKILL §3）：實作階段寫的實作碼是被測對象——要能通過測試階段定義的「過」（由驗收階段對秘書存檔的 commit 跑出來）。實作階段不自己定義測試、不自己跑驗收，避免「自己寫自己驗」。**測試先行**：測試階段先寫測試（此時紅燈），實作階段才寫實作碼讓測試轉綠——實作依測試階段已定義的「過」為目標實作，不是先寫碼再補測試。

**commit 時點＝實作完成即存檔**：實作階段完成該功能實作碼後，由秘書**立即 commit 存檔**（獨佔；commit＝存檔——建立不可變的待驗對象，先於驗收，不是驗收通過的完成印記），驗收階段對該存檔跑 CI。實作進行中 MUST NOT 隨手 commit；返工修復後疊加新存檔再驗。存檔走 `sb-commit` 流程（精準 add，功能＝commit 單位，不得夾帶範圍外檔案）。

**預設直接修正**：功能未觸發 §1.4 重流程條件時秘書直接寫不派發——實作碼不是每功能必走完整執行層時序，只有行為／介面／多檔／跨層改變或老闆指定才走。對實作的正確性回報如實，不掩飾問題；遇到技術層問題無法突破時如實回報，由秘書判斷是否以研究階段性質派發瓶頸顧問（SKILL §1.4）。
