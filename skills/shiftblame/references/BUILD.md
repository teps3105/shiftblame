...
實作狀態讀完整呼叫路徑、重用既有能力並修正共用根因；需要時反覆做實機驗證。主對話保留 G1／G2／G3 與已定稿測試脈絡；若根因、API、架構或證據解讀仍無法可靠裁定，依 SKILL §3 必須立即取得一次外部子代理唯讀技術意見，複核後自行裁定，不得要求老闆選技術方案。

### 實作碼的產出與紀律

秘書在實作階段寫 repo 實作碼，依 G2 技術方案落地。執行產出結構化整理後存 `<repo>/.shiftblame/tmp/`（自由傾倒區：變更摘要、實機驗證方式與結果——文件層義務，閘門不讀），不寫入 G2——G2 保持乾淨只放研究階段的決策結論。
