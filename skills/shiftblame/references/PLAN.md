...
> **定義層工作階段**。產出 G3（實作計畫），**薄規劃、向前對齊 G1＋G2 一次定稿**（一次定律，SKILL §1.1）——定義層最後一階，產出即定稿，§10 由秘書於放行前一次核對後直接推進到實作。**承載：子代理**（收斂多外部視角）——規劃階段做無副作用、可重現的文件產出，不涉及對外視窗，子代理勝任（SKILL §3 承載歸屬原則）。對 repo 永遠唯讀，不執行開發。**同面向對應測試階段（執行層）**——定義層（G3 實作計畫）↔ 執行層（測試階段寫測試定義「過」）。秘書派發規劃性質子代理執行。
