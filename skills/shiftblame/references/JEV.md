...
`node cli/test/sb-jev-generic-live.mjs --live <輸出絕對路徑>` 實跑不含 recipe 的連續操作；加 `--independent` 改跑陌生工具、來源角色、巢狀參數、省略、生成與缺證案例。模型實際回交也列為未達預期，不因沒有錯誤操作就記成全對。案例均為受控合成資料，結果不替代正式宿主或長時間專案驗收。
