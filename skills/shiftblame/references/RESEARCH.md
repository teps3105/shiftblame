...
> **定義層工作段**。主對話產出 G2（技術方案），外部證據打底、向前對齊 G1 一次定稿。G2 與 G1 不一致時回 intent 開新輪由 requirement 段重新定義 G1（語義出入由 G1 唯一裁定）。**同面向雙面**：G2 技術方案（定義）↔ 實作碼（落地——build 段依 G2 寫碼並回指）。§10 於放行前一次核對。
