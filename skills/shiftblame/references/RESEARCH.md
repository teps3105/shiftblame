...
研究狀態讀 codebase、查證文件並做必要實機驗證，將選型結論、採用理由、測試方式與風險寫入 G2。主對話保留完整 G1 脈絡；臨時不同視角只依 SKILL §3 條件取得，不是研究的固定分工。

### G2 的產出與紀律

主對話在研究狀態寫 G2（決策結論），中間查證可寫 `<repo>/.shiftblame/tmp/`。**薄研究**：G2 只記技術選型結論＋可行性證據，不記發散過程。G2 產出時直接向前對齊 G1；不得在研究狀態直接改寫 G1／G3。

外部研究與不同視角檢閱依 SKILL §3 條件觸發。結果只是輸入，主對話須親自查核來源並負責；未觸發不要求配套，觸發但取不到時在 G2 標「未驗」。
...
**開發中瓶頸處置**（SKILL §1.4）：臨時檢閱意見若為 CONFORMS，回研究狀態細化 G2；若揭示 G1 不足／衝突則走顯式修約，任何局部觀點均不得自行重釋 G1。
