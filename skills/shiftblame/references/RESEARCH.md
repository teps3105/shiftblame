...
- **退回**：G2 與 G1 不一致（含技術不可行、歧義、無法唯一對齊）一律回 intent 開新輪由 requirement 段重新定義 G1（A4），修正後一次重新對齊，不打轉
...
**開發中技術裁定**（SKILL §4）：外部子代理意見若為 CONFORMS，主對話複核後回 intent 開新輪至 research 段細化 G2（時點 1 後落地段回定義段必經 intent——計返工輪，輪次記 flow-state＋rewrite 載入閘；G1 契約隨回 intent 解除，需求重定義後再過時點 1 重封存；§1.1 的 CONFORMS 旗標切段不計輪僅適用時點 1 前定義層）；若揭示 G1 不足／衝突則走顯式修約（同經 intent 開新輪重定義——計返工輪）。任何局部觀點對 G1 的疑義一律走顯式修約；最終技術裁定權在主對話裁定。
