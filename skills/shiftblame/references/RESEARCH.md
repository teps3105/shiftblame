...
（多循環螺旋，SKILL §0、§1.4）：**每個 nnn 就是一個里程碑**（一組功能構成的使用者可觀察完整價值），功能降為 commit 單位。放行後 G2 是**活草稿**——開發情境與技術分析有出入時（做法、測試方式、風險），秘書依實作發現**常態修正對應 G2**（不重跑三面向制衡、不停止開發），再親自查核來源後寫入並對結論負責；只有整體技術方向需重估才走**重大例外遷移**退回三面向制衡（§1.4.1）。
