...
> **定義層工作段**。 **段靈魂——理解實現故事的疆域**：與故事的距離是唯一篩選標準——技術選擇回答「如何讓故事成立」，不是技術清單的堆疊。主對話產出 G2（技術方案），外部證據打底、向前對齊 G1 一次定稿。G2 與 G1 不一致時回 intent 開新輪由 requirement 段重新定義 G1（語義出入由 G1 唯一裁定）。**同面向雙面**：G2 技術方案（定義）↔ 實作碼（落地——build 段依 G2 寫碼並回指）。§10 於放行前一次核對。
...
（多循環螺旋，SKILL §0、§1.4）：放行後 G1 已封存。開發情境與技術分析有出入時，秘書先對照 G1：只有 CONFORMS（不改變使用者可觀察結果、範圍／例外、詞義與驗收成功集合）才能單調細化 G2；UNDERSPECIFIED／CONFLICTS 必須停止並走顯式修約（§1.4.1——fail 回 intent 觸發面之一）。codebase 現況與技術限制是可行性證據，不是改寫 G1 的權威。

**開發中技術裁定**（SKILL §1.4）：外部子代理意見若為 CONFORMS，主對話複核後回 intent 開新輪至 research 段細化 G2（放行後落地段回定義段必經 intent——計返工輪，輪次記 flow-state＋rewrite 載入閘；G1 契約隨回 intent 解除，重新放行時 G1 同 hash 重封存；§1.1 的 CONFORMS 旗標切段不計輪僅適用放行前定義層）；若揭示 G1 不足／衝突則走顯式修約（同經 intent 開新輪重定義——計返工輪）。任何局部觀點對 G1 的疑義一律走顯式修約；最終技術裁定權在主對話裁定。
