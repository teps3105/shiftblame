...
**開發中技術裁定**（SKILL §1.4）：外部子代理意見若為 CONFORMS，主對話複核後回 intent 開新輪至 research 段細化 G2（G1 語義不變——重新放行同 hash 重封存；重走以 `--rerun` 直通）；若揭示 G1 不足／衝突則走顯式修約（開新輪重定義）。任何局部觀點對 G1 的疑義一律走顯式修約；最終技術裁定權在主對話裁定。
