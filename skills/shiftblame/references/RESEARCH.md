...
> **定義層工作段**。主對話產出 G2（技術方案），外部證據打底、向前對齊 G1 一次定稿。G2 與 G1 不一致時回 intent 開新輪由 audit 重新定義 G1（rev 基線凍結；語義出入由 G1 唯一裁定）。**同面向雙面**：G2 技術方案（定義）↔ 實作碼（落地）。§10 於放行前一次核對。

- **產出**：G2：逐項承接 G1 的技術分析、測試方式、風險；引入的每個組件／依賴附消融貢獻（缺了它方案哪裡不成立——拆掉沒差＝冗餘即淘汰，消融原則 SKILL §1.8）
- **制衡**：G1 需求須技術上可實現；G3 實作計畫須與 G2 技術一致
- **退回**：G2 與 G1 不一致（含技術不可行、歧義、無法唯一對齊）一律回 intent 開新輪由 audit 重新定義 G1（SKILL §1.1；rev 基線凍結），修正後一次重新對齊，不打轉
...
完成條件：G2 每項內容都能指出承接的 G1 需求（含 G1 現況基準——技術方案站在已審計的現況上）、向前對齊 G1 一次定稿（輪內單向定律，SKILL §1.1）；§10 兩兩一致由秘書於放行前一次核對。G2 閉環＝research（定義）＋build（落地：實作回指 G2）。
