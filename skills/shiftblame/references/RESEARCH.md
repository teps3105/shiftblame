...
> **定義層工作段**。主對話產出 G2（技術方案），外部證據打底、向前對齊 G1 一次定稿。G2 與 G1 不一致時回audit 段重新正確定義 G1，G2 不得自行改義。**同面向雙面**：G2 技術方案（定義）↔ 實作碼（落地）。§10 於放行前一次核對。
