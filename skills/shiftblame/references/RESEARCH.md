...
> **定義層工作段**。主對話產出 G2（技術方案），外部證據打底、向前對齊 G1 一次定稿。G2 與 G1 不一致時回audit 段重新正確定義 G1（語義出入由 G1 唯一裁定）。**同面向雙面**：G2 技術方案（定義）↔ 實作碼（落地）。§10 於放行前一次核對。
...
**開發中技術裁定**（SKILL §1.4）：外部子代理意見若為 CONFORMS，主對話複核後回research 段細化 G2；若揭示 G1 不足／衝突則走顯式修約。任何局部觀點對 G1 的疑義一律走顯式修約；最終技術裁定權在主對話裁定。
