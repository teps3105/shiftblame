...
**有因重驗**：驗不過先分類為環境、實作或測試定義問題——環境修復後重跑受阻項；實作／測試問題旗標切段返工（實作回 build、測試定義回 test），修復後重驗受影響的 GWT 條目；舊 SATISFIED 不能作為已變更成果的驗收結論，於本段按原鍵更新判定。同一失敗無新處置時先查原因；保留失敗紀錄，不以偶然一次通過覆蓋未解原因。
...
**CI 測試可重現執行**：G3 設計的自動化檢查不依賴對外視窗。若無法重現且根因在 G3 設計，沿 verify→test→plan 回計畫重新設計——驗收證據一律取自行為重現。若現有證據不足以可靠分類為環境、測試或實作問題，依 SKILL §5 必須取得一次外部子代理唯讀技術意見，主對話複核後自行承擔返工路徑判定。
...
**回頭與出口**：代理驗出問題時自主回退；老闆判 fail 才依新意圖重走 intent。段內修復旗標切段回 test／build 續迭代（不計返工輪、不停等），技術定義問題沿合法邊回 plan／research 修正，需求或授權變更才開新輪（計返工輪）；驗收完成、G1 回指閉環後時點 2 對抗至乾淨，老闆終審 pass 走出口（`--new-ms --adversarial --boss-ok` 開新 ms／`sb end --adversarial --boss-ok` 結束 slug）——出口邊＝時點 2 對抗條目＋終審章同一邊承載。
