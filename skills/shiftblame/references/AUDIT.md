...
**nnn 價值制衡**（SKILL §0、§1.4、§10）：規劃階段在 G3 §1.5 定義 nnn 範圍時，審計階段 MUST 對本 nnn 是否構成 G1 的使用者可觀察完整價值進行制衡——價值不成立時要求規劃階段重定範圍（回三面向制衡）。nnn＝里程碑＝驗收節點；審計階段 MUST NOT 接受以單一功能作為驗收節點，也不得在一個 nnn 內塞入多個里程碑。
