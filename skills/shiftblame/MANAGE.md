...
老闆指定簡易→START（載入租約→揭露目標→暫停等老闆確認，**不可跳過**）→正方多子代理多視角提案→反方多子代理多視角質疑→管理者收斂（含執行變更）→老闆 PASS→管理者在 main commit（**僅 repo 檔案；.shiftblame/ 不入 repo**）→結束。同角色分工，不開 slug、不開分支、無 G(n).md、無歸檔。觸發權在老闆。FAIL 以收斂為基線重跑。
