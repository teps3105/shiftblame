...
| 複雜度 | 判定條件 | 執行方式 |
|--------|---------|----------|
| 低 | 計畫 ≤ 3 項變更、且 ≤ 2 份文件受影響 | 管理者在對話內直接執行 L1 實作/L2 驗收/L3/L4 |
| 高 | 計畫 > 3 項變更、或涉及多份文件交叉修改 | 開子代理隔離執行 L1 實作/L2 驗收/L3/L4 |

判定後記錄於 task.md 末行，格式：`複雜度：低/高`。低複雜度時管理者可在對話內依序執行 L1 實作（產出 task.md）→ L2 驗收（產出 result.md）→ L3 外部攻擊（產出 red.md）→ L4 內部驗證（產出 blue.md）。若無法開子代理 → BLOCK，不得使用外部品牌工具。
