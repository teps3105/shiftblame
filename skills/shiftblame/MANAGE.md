...
**證據調度**：管理者判斷是否觸發外部查證。觸發條件包含：新依賴/版本差異、外部 API、法規/安全/效能/成本主張、repo 內無先例、與老闆技術直覺衝突、agent 明確不確定或只能用 LLM 印象回答。低風險且 repo 內已有明確先例者，可引用專案現況作為依據，不強制外查。搜尋類外部查證遇本地硬約束時的 codex 升級規則見 SKILL 核心原則「選用外部角色整合」。
