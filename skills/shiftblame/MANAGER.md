...
5. **主執行者選定**：公平序列輪替（claude → codex → gemini → ...），寫入 meta.md
6. **worktree 確認**：slug 層級共用 worktree 已建立
7. **通訊目錄建立**：`mkdir -p ".shiftblame/$SLUG/$DEPT/$NNN/"{claude,codex,gemini}` + 空白 proposal.md / result.md
8. **task.md 寫入**：目標 + 約束 + YAML frontmatter（不含做法/分工）
9. **meta.md 更新**：派工紀錄表
10. **部門定義確認**：`DEPT/<DEPT>.md` 存在（管理者不注入，CLI 自行讀取）
11. **上游產出驗證**：讀取上游 conclusion.md（非第一個部門時）
12. **公平序列確認**：主執行者與上次不同（除非三方中已有兩方完成輪替）
...
| 全部失敗 | 回報管理者暫停 |
| 共識含技術分歧 | 互助修正或重新辯論；無法收斂採多數決，記錄少數意見 |
...
- 執行部門：寫入 consensus.md 和各 CLI 的 result.md
- meta.md 記錄：「共識收斂階段連續超時，降級為單體彙整」

### CLI 互助互監督

- CLI 互相監督，發現錯誤直接修正，不等管理者
- 提前完成的 CLI 監督同事作業
- 工具與指令定義明文寫在 DEPT/*.md 與 STAFF.md，同事可閱讀驗證
...
CLI 能力不足時：
1. 在產出中記錄限制和已嘗試的方案
2. 嘗試替代工具（`terminal()`、`write_file()`、`patch()`）
3. 仍不足時寫入：
   ```
