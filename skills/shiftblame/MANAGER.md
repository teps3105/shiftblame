...
- **建立**：slug 啟動時（第一次進入 DEV 前），在主工作目錄執行 `git worktree add .shiftblame/worktree/<slug> -b feat/<slug>`
- **開發期間**：DEV 在 worktree 中工作、commit；紅隊在 worktree 中或透過 `git diff` 檢視程式碼
- **收尾清理**：切回主工作目錄 → merge → push → `git worktree remove .shiftblame/worktree/<slug>` → `git branch -d feat/<slug>`
