...
輪次上限與初輪限制：
- 同一個 slug 內，任一部門最多只能建立到 `005`；補強、打回與回溯都計入同部門同 slug 的同一個五輪上限。
- 已存在同部門 `005` 時，不得建立同部門 `006`。若仍未收斂，必須先對該部門執行聚合：重寫該部門 `DEPT/001/task.md`，刪除所有舊的攻防產物與中間文件，聚合成新的單一上游輸入。
- 研究部門（PM/QA）001 經紅藍攻防通過後可作為下游輸入。
- 執行部門（DEV/QC）001 為計畫階段，002 才開始正式執行。執行部門至少完成 002 並通過 Result Check 與 BossConfirm 後，才能推進到下一部門。
- 聚合後不受最低門檻限制：聚合後的 001 通過紅藍攻防與 BossConfirm 即可推進。
...
- 退回修正同樣計入同部門同 slug 的五輪上限；已到 `005` 時不得建立 `006`，須先執行該部門刪檔聚合。
...
- 未達研究過關門檻：PM/QA 001/result.md 以上 result.md 結論判定研究仍不足 → 自動建立下一個 task.md；到 005 則聚合
- 未達執行最低門檻：DEV/QC 001 result.md 結論判定規劃不充分 → 自動建立 002/task.md
- 未達執行過關門檻：DEV/QC 002/result.md 以上 result.md 結論判定執行仍不足 → 自動建立下一個 task.md；到 005 則聚合
