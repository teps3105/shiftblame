...
**面向判定**（L0 期間）：每次新 NNN 時，管理者向老闆提案本輪面向，老闆確認。面向記錄於 task.md frontmatter `aspect` 欄位。手動模式每個 NNN 的 L0 各自指定面向；自動模式僅一次 L0 指定本輪面向（同一 NNN），角色 PASSED 後新對話重新 L0 可指定不同面向。
...
- **面向保持**：退回時 `aspect` 保持原值不變。僅在重新 L0 時老闆可變更面向。自動模式面向不變。
- **預設面向**：若 L0 期間老闆未明確指定面向，管理者使用預設面向（PM：規劃；DEV：開發）並於宣告開始時寫入 task.md。
