...
輪次上限與初輪限制：
- 同一個 slug 內，任一部門最多只能建立到 `005`；攻防延伸、退回修正與補充釐清都計入同部門同 slug 的同一個五輪上限。
- 已存在同部門 `005` 時，不得建立同部門 `006`。若仍未收斂，必須先對該部門執行刪檔聚合：保留有效結論，刪除或合併過度展開、互相衝突、已失效的中間檔，聚合成新的單一上游輸入後再決定重開任務或新 slug。
- 任一部門 `001` 都是初步、不清楚且未經完整攻防的工作結論，不得作為下一部門正式輸入。
- 任一部門至少必須完成 `002` 循環，並通過 Result Check 與 BossConfirm，才能推進到下一部門。
