...
| 問題類型 | 定義 | 退回目標 | 適用階段 |
|----------|------|----------|----------|
| **原則性問題** | 計畫範圍需變更、需求理解有誤、根本性設計缺陷、目標無法達成 | → DECLARED（退回 L1） | L2~L5 |
| **上游階段問題** | 上游產出有遺漏或不完整（L3 發現 L2 驗收漏洞、L4 發現 L3 攻擊不全面） | → 退回上游 L(n-1) | L2~L5 |
| **當前階段問題** | 報告微調、遺漏補充、格式修正、不影響上游的不完整項 | → 該階段直接修復 | L2~L5 |
...
- 上游退回後該上游階段重新執行（不跨越已通過的階段）
- 當前修復不更改狀態，修正後重新宣告完成
- DEV 退回前先 commit
- 定義問題→退回 PM。回溯→撤回該角色所有變更回到 001
- 計畫更動判定→回溯或進路線圖

**自動模式**：原則性問題退回至初始宣告重新開始；上游/當前問題由管理者自動修復後繼續執行。
...
- **跨部門通訊目錄**：`.shiftblame/<slug>/shared/`。PM→DEV 交接資料存放於此。若已有既有分享目錄則不更動。
