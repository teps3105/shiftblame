...
3 觸發點：①③ **老闆-gated、預設關閉**（①收斂後揭露審核升級鏈選項）；② **post-EXECUTOR 硬審核**（必做，揭露升級鏈選項→老闆選擇擔任者，見 TOOLS/AUDIT/）。②未選外部由 read-only 子代理獨立 code review，審 e2e 證據／未驗項，管理者不得自審替代：

1. **① G1/G2 收斂段寫入後**
2. **② `<nnn>` 所有序列（`<task>`）執行完畢後**（EXECUTOR 達成 `<complete>`，必做獨立審核）
3. **③ `<slug>` 分支全域**

流程：①③ 管理者狀態揭露 → 交付老闆 → 老闆選擇是否複審（①含升級鏈選項）；② 管理者依擔任者選擇調度獨立審核 → 回收 code review/e2e 結果 → 狀態揭露（自驗／獨立 review／e2e／未驗項）→ 交付老闆。未跑 e2e 即標未驗；擔任者與工具細則見 TOOLS/。
