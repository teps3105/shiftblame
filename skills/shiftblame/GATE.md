...
Slug 狀態序：PLANNED→DEVELOPED→VERIFIED→PROSPECTED→PASSED。G2（DEVELOPED）為 NNN 迭代出口；G3/G4 不開新 NNN。簡易狀態序：PROPOSED→QUESTIONED→CONVERGED→PASSED。FAIL 以收斂為基線增量增加。

## 閘門生命週期

**Slug START**：驗證上游→載入三層租約（SOP｜SLUG §7｜SKILL+GATE+MANAGE 皆技能定義檔 skills/shiftblame/）→揭露目標→**暫停等老闆確認**。
**Slug END**：呈現管理者收斂+下一步→老闆 PASS/FAIL。G2 為 NNN 迭代出口，收斂後提交；G3/G4 不開新 NNN，收斂後提交修復/文件更新。G3/G4 若需更多開發→開新 slug。反方 H/M/L 標註，不做決策。
...
Slug 管線：G2 為 NNN 迭代出口→收斂後提交→G3/G4 不開新 NNN，收斂後提交→G4 PASS→收尾（確認→合併→推送→清理→歸檔至 .shiftblame/archive/）或暫停。G3/G4 若需更多開發→開新 slug。簡易模式：PASS→commit（僅 repo 檔案）→結束。
