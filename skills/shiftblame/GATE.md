...
- **START**：載入三層租約（SOP｜SLUG §7｜SKILL+GATE+ROLE/+TEMPLATES/）→ 揭露目標 → **暫停等老闆確認**
...
3 觸發點：①③ **老闆-gated、預設關閉**；② **post-EXECUTOR 硬審核**，由管理者調度 read-only 子代理或 AUDIT 外部工具獨立 code review，並審 e2e 證據／未驗項，管理者不得自審替代：

1. **① G1/G2 收斂段寫入後**
2. **② `<nnn>` 所有序列（`<task>`）執行完畢後**（EXECUTOR 達成 `<complete>`，必做獨立審核）
3. **③ `<slug>` 分支全域**

流程：①③ 管理者狀態揭露 → 交付老闆 → 老闆選擇是否複審；② 管理者調度獨立審核 → 回收 code review/e2e 結果 → 狀態揭露（自驗／獨立 review／e2e／未驗項）→ 交付老闆。未跑 e2e 即標未驗；複審擔任者與工具細則見 TOOLS/。
