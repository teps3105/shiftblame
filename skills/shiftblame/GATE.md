...
- **`<nnn>`**：`START → 正方 → 反方 → 收斂 → G3 → 執行 → <complete> → 下一個 <nnn>`（老闆指示推進）。`<nnn>` 無 gate/PASS/FAIL。
...
任一成立→序列派發多執行者（一個完成才開下一個，非並行）：`<task>` 序列 ≥4、跨檔 ≥4、強序列依賴對 ≥2。否則單一執行者。一致性的跨檔 `<task>` MUST 同一執行者。

## 5. Minor/Major 變更

- **Minor**（當前 `<nnn>`）：順序/驗證/細節調整，commit 訊息 `change(<slug>): <摘>`。
- **Major**（新 `<nnn>`）：目標/完成標準/架構改變，重新正反收斂。

## 6. 複審觸發點

- **①③**（G1/G2 收斂後、slug 全域）：老闆-gated，預設關閉。
- **②**（`<nnn>` 序列執行完）：硬審核 MUST 做；管理者揭露升級鏈選項 → 老闆選擇擔任者 → 獨立審 e2e 證據/未驗項。未跑 e2e MUST 標「未驗」。

## 7. 收尾（老闆拍板 PASS 後）

管理者品質確認 → 老闆 PASSED → 更新 SOP/ROADMAP（當下事實）→ `git checkout main && git merge --no-ff feat/<slug>` → 清理 → 歸檔 `.shiftblame/<slug>/` 至 `archive/`。
