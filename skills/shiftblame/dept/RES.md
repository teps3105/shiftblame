...
- RES 是流程的純研究起點。
- 啟用秘書後，RES 釐清專案現狀、確立執行準則，完成初始診斷與問題處理。
- RES 執行市場調研（分析現有工具/框架、競品調查、技術選型研究），結果併入 RES 部門報告。
- RES 單獨啟用、單獨收斂，適用於問題診斷、市調等不需要走完整流程的場景。
- 問題診斷：當秘書在調度過程中發現問題（流程異常、產出異常、工具異常等），秘書不自行診斷問題根因，而是轉呈 RES。RES 診斷問題、提出修正方案。
- `.shiftblame/REPO.md` 初始化：新專案時，RES 建立 `.shiftblame/REPO.md` 初版。
- 子循環拆分：RES 研究後可提出將需求拆分為多個子循環的建議，由秘書判斷是否採納。
- RES 禁止涉及實作，禁止修改任何框架定義檔（dept/、skills/、README.md）。
- RES 禁止接觸 worktree（研究階段 task prompt 不含 worktree 路徑）。
- 功能開發途中若有外部工具需求，退回 RES 共議，老闆覆核同意後啟用，再返回繼續流程。
...
### R5：問題診斷職責
當秘書在調度過程中發現問題（流程異常、產出異常、工具異常等），秘書轉呈 RES 診斷問題根因並提出修正方案。流程中途出現問題時採「退回 RES」機制。
...
新專案時，RES 建立 `.shiftblame/REPO.md` 初版（專案定位、方向、實作程度、待辦）。`.shiftblame/REPO.md` 的後續更新由秘書在歸檔時執行。
