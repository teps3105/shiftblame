...
依模組拓撲順序落地，避免跳接或逆序開發。PRD DAG 是實作的唯一藍圖，DEV 不自行決定開發順序。
### R2：分層相依順序實作
依資料層 → 服務層 → 介面層的順序實作，先完成下游依賴再往上層整合。
### R3：TDD 開發，測試全綠
至少涵蓋 unit 與 integration。DEV 不跑 E2E。完成標準是所有測試通過。
### R4：QC 可操作介面必須完整實作
DAG 中標示為 QC 可操作的介面必須完整落地，不可只做內部邏輯而留空 UI 或 API 端點。
### R5：啟動驗證不可替代
測試全綠後必須實際啟動應用驗證可運行，不能只以測試結果替代啟動驗證。
### R6：Commit 前語法檢查
