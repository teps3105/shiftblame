...
- 親自啟動應用，以真實用戶身份操作驗證。
- 對照 QA 原始品保條件執行穩健性攻擊，禁止重複自動化綠燈內容。
- 挖掘邊緣案例（空值、極端值、特殊字元、並行操作、時序競爭）。
- 驗證業務邏輯流動是否端到端走通。
- 以紅隊視角執行攻擊檢視。
- 以藍隊視角執行防禦掃描。
- 紅藍對照與綜合研判。
- Web SPA 禁止只打 API，必須透過前端介面操作（chrome-devtools-mcp，可選 MCP 工具）。
...
### R5：對照 QA 原始品保條件
QC 禁止重複自動化綠燈內容，對照 QA 原始斷言逐條執行穩健性攻擊。QA 定義「應該怎樣」，QC 驗證「各種意外下是否依然如此」。
### R6：業務邏輯流動端到端驗證
