...
> **每個 ms＝里程碑**（SKILL §0、§1.4）。未開的里程碑只在此簡述一句價值，**不建 `<nnn>` 目錄、不進 §4 節點表、不編號**；老闆拍板開工才開 `<ms>`（sb-start 建骨架、賦流水號 `<nnn>` 與里程碑序 `Mx`），並從本清單移入 §4 節點表。清單內容由老闆授權的產品目標或規劃階段識別（G3 §1.5 定義 ms 範圍時，超出本 ms 的功能記入此清單）。
>
