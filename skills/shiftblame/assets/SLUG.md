...
> G1／G2／G3 三面向範本見本文末「三面向範本」段——**定義單檔、結構分檔**：建立 `<ms>` 時從該段複製對應範本到 `.shiftblame/<slug>/<nnn>/G1.md`／`G2.md`／`G3.md` 分檔（結構見 SKILL §8），不寫入本檔。
...
> **每個 ms＝里程碑**（SKILL §0、§1.4）。未開的里程碑只在此簡述一句價值，**不建 nnn 目錄、不進 §4 節點表**；老闆拍板開工才開 `<ms>`（sb-start 建骨架），並從本清單移入 §4 節點表。清單內容由老闆授權的產品目標或規劃階段識別（G3 §1.5 定義 ms 範圍時，超出本 ms 的功能記入此清單）。
>
> **順序＝優先序**：清單由上到下即開工順序——sb-start 開新 `<ms>` 時依序從最上面未開的里程碑開始。調整順序用 `sb-ms`（老闆指定新順序，秘書重排清單）。
...
只有老闆明確決定開新 `<ms>` 時才能新增一列；同一子需求的擴充更新原列。合法節點與主圖一一對應：`三面向制衡（G1↔G2↔G3）／開發／ms 完成／老闆 PASS／收尾`。不得跳點；退回一律改回 `三面向制衡（G1↔G2↔G3）` 後重走。**級別欄依 SKILL §0.1 規模分級填**（S／M／L，sb-think 路由時老闆拍板）：M 級在三面向制衡節點僅產 G1 單份；S 級不建骨架（不開 ms）。
...
**`<ms>` 完成 ≠ slug PASS**：前者是單一子需求循環收斂（三者重審通過、片段清空，先做輕量保鮮 §1.7.1，再由老闆決定開新 ms 或結束 slug）；後者是整個 slug 結束（老闆拍板，走完整收尾保鮮 §1.7.2 → 移 archive/）。老闆在同一 slug 內開新 ms 不需先 PASS。frontmatter `status`：建立時 `in_progress`，老闆 PASS 後改 `passed`。
...
## 三面向範本（建立 `<ms>` 時複製為分檔，**僅 L 級**）

> 以下為 G1／G2／G3 三份制衡文件的範本（SKILL §0.1：L 級大需求用；M 級僅用 G1 單份，G2/G3 免）。**定義單檔、結構分檔**：建立新 `<ms>` 時，從此處複製三段到 `.shiftblame/<slug>/<nnn>/` 下各自成檔（`G1.md`／`G2.md`／`G3.md`），不寫入本檔。三份各由對應階段改寫，兩兩雙向一致（判準見 SKILL §10）才可開發。
>
> **呈現原則（硬性）**：
...
- [ ] **兩兩雙向一致（判準見 SKILL §10）**——逐項核對三對六向，全部成立：
  ...
  - [ ] G1↔G3：G1 每項需求在 G3 實作計畫有對應項（驗收操作＋實作步驟）；G3 實作計畫每項（驗收與實作步驟）能回指所支持的 G1 需求；本 ms（里程碑）指向一項 G1 可觀察完整價值、能回指該價值（§1.5）。
...
- [ ] **兩兩雙向一致（判準見 SKILL §10）**——逐項核對三對六向，全部成立：
  - [ ] G1↔G2：G1 每項需求在 G2 有對應技術分析；G2 每項技術分析能回指所承接的 G1 需求。
  - [ ] G1↔G3：G1 每項需求在 G3 實作計畫有對應項；本 ms（里程碑）指向一項 G1 可觀察完整價值（§1.5）。
...
> 規劃階段主導。與 G1、G2 兩兩雙向制約。G3 依序完成：①回讀 G1 寫驗收；②驗收完整後定義本 ms 範圍、依 G2 寫實作步驟，不得倒序。G3 只由規劃階段改寫；§2.5 收斂執行記錄由秘書補寫。
...
> ms＝里程碑＝驗收節點。本 ms 所有功能 commit 完成後觸發收斂（§1.4.2）。不屬於本 ms 的功能開新 ms。
...
> 由秘書在本 ms 所有功能 commit 完成後、觸發收斂時撰寫，描述使用者可觀察的行為。讀 `tmp/` 中執行層時序的執行記錄彙整。
