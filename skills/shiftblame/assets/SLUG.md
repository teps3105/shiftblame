...
- [ ] 邊界與不做事項明確。
- [ ] codebase／外部事實附可查核來源。
- [ ] 觸發獨立研究條件（複雜／高風險／無先例／與老闆直覺衝突／老闆指定，SKILL §3）者，已透過秘書代派子代理取得外部獨立研究、反證或替代觀點（結論存 `<repo>/.shiftblame/tmp/`，子代理不能派生子代理，SKILL §3）並親自複核；未觸發條件者不派配套，取不到結論時對應項標「未驗」。
...
- [ ] G2 沒有新增或改寫 G1 需求。
- [ ] 不確定事項已有查證或明確標示。
- [ ] 觸發獨立研究條件（複雜／高風險／無先例／與老闆直覺衝突／老闆指定，SKILL §3）者，已派發子代理取得外部獨立研究、反證或替代觀點（結論存 `<repo>/.shiftblame/tmp/`，子代理不能派生子代理，SKILL §3）並親自複核；未觸發條件者不派配套，取不到結論時對應項標「未驗」。
