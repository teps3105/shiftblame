...
> 本檔保存跨循環不變的命題、授權邊界與目前所在節點；流程看 SKILL §0 主圖。slug 以功能語義命名；本檔與 G1~G3 的定義、名稱與回指可離開對話及 tmp 理解，對話、工作過程與交接敘事一律 tmp。
...
> **每個 ms＝里程碑**（SKILL §0、§1.4）。未開的待辦只在此簡述一句價值，**不建 `<nnn>` 目錄、不進 §4 節點表、不編號**；老闆說「開新 ms」（--new-ms）才開 `<ms>`（sb init／sb next intent 建骨架、賦流水號 `<nnn>`），並從本清單移入 §4 段表。清單內容由老闆授權的產品目標或規劃階段識別（G3 §1.5 定義 ms 範圍時，後續 ms 的功能記入此清單）。
>
...
> requirement 段主導（G1 定義邊；verify 為裁判邊）——需求建立在經查證的現況事實上（盤點 codebase 實況、對照文件差異、識別過時假設），承接 shiftblame:think 已確認語義直接定稿（輪內單向定律，SKILL §1.1）；只寫需求／驗收面向，不寫技術選型或實作步驟。每項 AC-ID 以 BDD 行為規格表達（Given 前置情境／When 操作／Then 可觀察結果＋使用者＋失敗邊界＋消融——拿掉此需求使用者失去什麼可觀察價值，答不出＝偽需求；消融原則 SKILL §1.8）。G1 定義區（`## 回指記錄` 標題前）只由 requirement 段寫，時點 1 過邊（requirement→research）時 CLI 對定義區 hash 封存；verify 段把 AC 判定收斂寫入回指區（段內秘書判決，不觸契約）。契約不足／衝突＝停經 think 老闆裁決後回 intent 開新輪重定義後重新過時點 1。
...
> §10 三對六向（G1↔G2、G1↔G3、G2↔G3）由秘書於定義層二收斂時一次核對（plan→test 機械推進——中鏈零審核），不在各文件內重複自查；缺漏由責任面向一次補正後重核（輪內單向定律，SKILL §1.1），不環形重跑。
...
> §10 三對六向由秘書於定義層二收斂時一次核對（plan→test 機械推進——中鏈零審核），不在各文件內重複自查；缺漏由責任面向一次補正後重核（輪內單向定律，SKILL §1.1），不環形重跑。
...
> 規劃階段主導（G3 定義邊；test 為落地邊——測試碼回指驗收排程），**對齊規劃**：向前對齊 G1＋G2 一次定稿（輪內單向定律，SKILL §1.1）。G3 依序完成：①回讀 G1 寫驗收；②驗收完整後定義本 ms 範圍、③依 G2 寫實作步驟——順序固定（①驗收→②範圍→③步驟）。驗收、ms 範圍、實作步驟為 §10 回指結構所需 MUST 填實；開發策略、預期變更、一致前檢查依複雜度精簡或從缺（§5 由秘書定義層二收斂時 §10 一次核對取代）。G3 定義區只由規劃階段改寫；test 段把測試定稿映射收斂寫入回指區；收斂執行記錄由秘書寫入 SLUG §4（出口前輕量動作——G3 寫入權無例外）。
...
> 三層測試排程：既有「測試」欄填適用層級與案例入口（可列多個）；「需要的證據」填各層能支持的行為範圍。實作步驟標明單點驗證、整合接合點與依賴，另排本 ms 功能全部完成後的 E2E。功能判決只核對當次範圍，未到接合點或尚未執行者保留待驗；ms 驗收才要求全部必填 AC 的行為證據。有效證據沿用，重驗記明原因與範圍（SKILL §1.4）。
...
> 起始效應對策（premortem，SKILL §3 時點 1 對抗·premortem 挑戰＋CLI plan→test 機械閘結構驗）：規劃完美盲信的制衡——列不出真實失敗點＝沒想過會怎麼失敗。plan→test 機械閘查核本段非敷衍；敷衍（無／無風險）即擋。
...
> 精簡錨定（SKILL §1.1、references/PLAN.md）：§10 三對六向的完整核對由秘書於定義層二收斂時一次執行，G3 內不重複自查。簡單 ms 本段從缺；複雜 ms 才精簡列風險自查（如「驗收判準來自 G1，非 G2 反推」）。
