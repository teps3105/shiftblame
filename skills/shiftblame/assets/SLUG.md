...
> 規劃階段主導，**對齊規劃**：向前對齊 G1＋G2 一次定稿（一次定律，SKILL §1.1）。G3 依序完成：①回讀 G1 寫驗收；②驗收完整後定義本 ms 範圍、依 G2 寫實作步驟，不得倒序。驗收、ms 範圍、實作步驟為 §10 回指結構所需 MUST 填實；開發策略、預期變更、一致前檢查依複雜度精簡或從缺（§5 由秘書放行前 §10 一次核對取代）。G3 只由規劃階段改寫；§2.5 收斂執行記錄由秘書補寫。
