...
> 規劃階段主導。與 G1、G2 兩兩雙向制約。G3 依序完成：①回讀 G1 寫驗收；②驗收完整後才依 G2 寫實作步驟，不得倒序。G3 只由規劃階段改寫；§2.5 階段驗收記錄由秘書補寫。
...
> 秘書維護進度高亮：已驗收標綠、進行中標黃、未到標灰。里程碑 = 一組功能構成的使用者可觀察完整價值，是**驗收節點**。由規劃階段切分、審計階段制衡價值成立。單一功能不構成里程碑。
...
- **M1（短名）** — 構成的 G1 可觀察價值：（填）｜審計 價值制衡：（填）｜包含功能：F1、F2
- **M2（短名）** — 構成的 G1 可觀察價值：（填）｜審計 價值制衡：（填）｜包含功能：F3

**2. 實作步驟（驗收完整後填）**

步驟按功能分組、功能隸屬里程碑。寫成 markdown 條列（不是圖節點）：
...
**2.5 階段驗收記錄**（秘書補寫，每個里程碑一條）

> 由秘書在該里程碑所有功能 commit 完成後撰寫，描述使用者可觀察的行為。讀 `tmp/` 中執行層時序的執行記錄彙整。

- **M1** — 做了什麼：（填）｜實現價值：（填）｜發生什麼事：（填）｜各功能 commit：（填）｜老闆確認：（填）｜審計 複驗：（填）｜狀態：待驗收／返工／合格
