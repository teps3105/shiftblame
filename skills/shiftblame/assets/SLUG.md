...
只有老闆明確決定開新 `<ms>` 時才能新增一列；同一子需求的擴充更新原列。合法節點與主圖一一對應：`三面向制衡（G1↔G2↔G3）／開發／ms 完成／老闆 PASS／收尾`。不得跳點；退回一律改回 `三面向制衡（G1↔G2↔G3）` 後重走。
...
> 以下為 G1／G2／G3 三份制衡文件的範本。**定義單檔、結構分檔**：建立新 `<ms>` 時，從此處複製三段到 `.shiftblame/<slug>/<nnn>/` 下各自成檔（`G1.md`／`G2.md`／`G3.md`），不寫入本檔。三份各由對應階段改寫，兩兩雙向一致（判準見 SKILL §10）才可開發。
>
> **呈現原則（硬性）**：
