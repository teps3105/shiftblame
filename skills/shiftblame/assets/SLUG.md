...
- [ ] 邊界與不做事項明確。
- [ ] codebase／外部事實附可查核來源。
- [ ] 觸發獨立研究條件（複雜／高風險／無先例／與老闆直覺衝突／老闆指定，SKILL §3）者，已透過秘書代派子代理取得外部獨立研究、反證或替代觀點（結論存 `tmp/`，子代理不能派生子代理，SKILL §3）並親自複核；未觸發條件者不派配套，取不到結論時對應項標「未驗」。
...
- [ ] G2 沒有新增或改寫 G1 需求。
- [ ] 不確定事項已有查證或明確標示。
- [ ] 觸發獨立研究條件（複雜／高風險／無先例／與老闆直覺衝突／老闆指定，SKILL §3）者，已派發子代理取得外部獨立研究、反證或替代觀點（結論存 `tmp/`，子代理不能派生子代理，SKILL §3）並親自複核；未觸發條件者不派配套，取不到結論時對應項標「未驗」。
...
> 每項 G1 需求 MUST 出現，語意不得偷換。驗收描述使用者可觀察的行為或結果，不以檔案、字串、grep 命中代替。測試階段依 G1 驗收標準寫測試（測試＝G1 落地）；本段驗收操作是 G3 的排程與對應（哪些驗收項納入本 ms、順序、證據），驗收階段照此執行（驗收＝G3 落地）。無法寫出操作、判準或證據時，MUST 退回對應面向一次補正（SKILL §1.1）；屬需求方向問題走重大例外（SKILL §1.4.1）。
