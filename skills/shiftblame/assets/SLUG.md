...
> 本檔保存跨循環不變的命題、授權邊界與目前所在節點；流程看 SKILL §1 主圖。slug 以功能語義命名；本檔與 G1~G3 的定義、名稱與回指可離開對話及 tmp 理解，對話、工作過程與交接敘事一律 tmp。
...
- **已授權方案**：（填）
- **允許修改範圍**：（填）
- **老闆路由決定**：（寫明已授權的對象、範圍與路由；保留可解析的來源回指）

## 3. 待辦清單（簡述）

> **每個 ms＝里程碑**（SKILL §1、§4）。未開的待辦只在此簡述一句價值，**不建 `<nnn>` 目錄、不進 §4 節點表、不編號**；老闆說「開新 ms」（--new-ms）才開 `<ms>`（sb init／sb next intent 建骨架、賦流水號 `<nnn>`），並從本清單移入 §4 段表。清單內容由老闆授權的產品目標或規劃階段識別（G3 §1.5 定義 ms 範圍時，後續 ms 的功能記入此清單）。
>
...
>
> **新增待辦**：老闆想在當前 slug 加功能但不立即開 ms 時，由秘書寫入本清單（老闆指示即授權；一句價值簡述，純短名，無編號，加於清單末尾）。
...
|---------|-------------|------|---------|---------|
| 001 | （已授權路由與來源回指） | intent | — | — |

### 定案索引（同 slug 各 ms 一行式定案——跨 ms 參照載體）

> 真驗收終審 pass 後、走出口前由秘書寫入（MECHANISMS §7）；同 ms 重修後再次 pass 出口＝以 `<nnn>` 為鍵覆寫原行。摘要限指路級（不含參數、數值、承諾）；索引與 G 檔衝突時以 G 檔為準。§9 載入與 requirement／research 段參照義務的標的。
...
> 功能迭代完成、收斂期 E2E 綠燈收斂、機械推進 verify、真驗收完成且時點 2 對抗畢、老闆終審 pass 後出口前由秘書寫入（MECHANISMS §7）；讀 `.shiftblame/tmp/` 實作層各階段記錄彙整成一行。SLUG 由秘書維護恆可寫——執行結果記錄屬回指級內容，G3 寫入權無例外（SKILL 寫入矩陣）。同 ms 重修後再次出口＝以 `<nnn>` 為鍵覆寫原行。

| `<nnn>` | 做了什麼／實現價值／發生什麼事 | 各功能 commit | 複驗判定（含三面向重審） | 狀態 |
|---------|--------------------------------|---------------|--------------------------|------|
| （首 ms 尚無收斂——本 ms 驗收判決後寫入首行） | | | | |
...
- **（技術債短名）** — 來源：（填）｜描述：（填）｜建議：（填）｜狀態：（填）

## 7. 臨時租約

> 無租約時此段留空。每筆一條：

- **（租約短名）** — 描述：（填）｜日期：（填）｜處置：升級 SOP／隨歸檔失效（填）｜狀態：（填）

## 8. 交接回指

- **交接文件**：`<repo>/.shiftblame/tmp/<slug>/handoff.md`（由 shiftblame:save 寫入並讀回後更新此回指與 `last_save`）。
- 本節只存路徑；工作落點、下一步、注意事項與執行過程寫入交接文件。交接缺失時依 flow-state、G1~G3 與實況重新核對，回指本身不構成可接續的證據。
...
- **過往 ms 定案對照**：（對照 SLUG 定案索引與過往 ms G1 定義區——每項新需求顯式判定與既有定案的關係：**延伸／獨立／衝突**；本 slug 首 ms 明寫「無過往定案」）
...
> 每項使用者需求至少一項，`AC-ID` 在本 ms 內穩定且唯一；每項以 `### AC-` 分段（requirement→research 邊 BDD 格式閘機械驗七鍵齊全）。行為規格逼出的問題（誰、什麼情境、做什麼、觀察到什麼、現行是什麼、什麼算失敗）字面搬運答不出——需求以行為語義立法。鍵義：Given＝前置情境（使用者所處狀態）、When＝操作（做什麼）、Then＝可觀察結果（觀察到什麼）、現狀＝現行系統同 Given/When 的實際觀察（需求先驗——差異宣言左邊，現狀＝Then 即偽需求）、使用者＝誰、失敗邊界＝什麼情況判失敗、消融＝拿掉此需求使用者失去什麼可觀察價值。`證據` 一律為 `BEHAVIOR`：檔案、字串、節點、schema 等結構正確只能作開發自驗，不能代替使用者需求。
...
> §10 三對六向（G1↔G2、G1↔G3、G2↔G3）由秘書於定義層二收斂時一次核對（plan→test 機械推進——中鏈零審核），不在各文件內重複自查；缺漏依 A4 回責任段修正，再重核受影響方向。

## 回指記錄

> 執行後回頭記錄（ROM 回指區——隨執行更新，不觸放行封存）。G1 本區由 verify 段收斂寫入（段內秘書判決）；行格式用全形「｜」（禁 `### AC-` 標題與半形 `|`——防機械誤擋）。
...
> 研究階段主導（G2 定義邊；build 為落地邊——實作回指 G2），**外部證據打底**：進段後至少一次外部工具調用（research→plan 邊機械驗），只記選型結論＋可行性證據，承接 G1（含經查證的現況事實），以新證據檢驗技術與需求前提（A4）。G2 定義區只由研究階段改寫，保持乾淨的決策結論；build 段把實作偏離收斂寫入回指區；發現 G1 需求技術不可行屬重大例外（MECHANISMS §5）退回 requirement 段，不在 G2 內改寫需求。
...
每條技術鏈寫成 markdown 條列（不是圖節點）。測試按單點／整合／E2E 說明被測邊界及必要依賴，選擇足夠層級，不強制每條技術鏈各寫三套，規模由防護目標推導（SKILL §4——只為要防的具體風險配置手段）；G3 承接接合點與執行時機：
...
> §10 三對六向由秘書於定義層二收斂時一次核對（plan→test 機械推進——中鏈零審核），不在各文件內重複自查；缺漏依 A4 回責任段修正，再重核受影響方向。
...
> 規劃階段主導（G3 定義邊；test 為落地邊——測試碼回指驗收排程），**對齊規劃**：承接 G1＋G2，計畫前提不成立時依 A4 回責任段修正。G3 依序完成：①回讀 G1 寫驗收；②驗收完整後定義本 ms 範圍、③依 G2 寫實作步驟——順序固定（①驗收→②範圍→③步驟）。驗收、ms 範圍、實作步驟為 §10 回指結構所需 MUST 填實；開發策略、預期變更、一致前檢查依複雜度精簡或從缺（§5 由秘書定義層二收斂時 §10 一次核對取代）。G3 定義區只由規劃階段改寫；test 段把測試定稿映射收斂寫入回指區；收斂執行記錄由秘書寫入 SLUG §4（出口前輕量動作——G3 寫入權無例外）。
...
> ms＝里程碑＝整體驗收節點。實作層一每功能依序 test 撰寫測試→build 實作與相關單點／整合驗證→提交閘 commit（測試碼＋實作碼同 commit）→verify 核對功能證據作 AC 判定（段內判決）。功能迭代完成後進實作層二收斂期：test 寫 E2E→build 調環境執行至綠燈收斂（build 的功能是讓測試綠），機械推進 verify 真驗收（GWT 逐條劇本實操），驗收完成且時點 2 對抗畢、老闆終審 pass 走出口（MECHANISMS §6）。不屬於本 ms 的功能開新 ms。
...
> 每個 G1 AC-ID MUST 恰有一列且語意一一對應。驗收描述使用者可觀察的行為或結果，不以檔案、字串、grep 命中代替。AC-ID 與測試的映射由本表承載（測試欄位，與測試碼正交）；測試碼於 test 撰寫，隨功能實作在 build 同一 commit 定稿，驗收階段逐項產出 SATISFIED／UNSATISFIED／UNVERIFIED。無法寫出操作、判準或證據時，回責任段修正並重驗受影響成果（A4）；屬需求方向問題走重大例外（MECHANISMS §5）。

> 三層測試排程：既有「測試」欄填適用層級與案例入口（可列多個）；「需要的證據」填各層能支持的行為範圍。實作步驟標明單點驗證、整合接合點與依賴，另排本 ms 功能全部完成後的 E2E。功能判決只核對當次範圍，未到接合點或尚未執行者保留待驗；ms 驗收才要求全部必填 AC 的行為證據。有效證據沿用，重驗記明原因與範圍（SKILL §4）。

**1.5 ms 範圍定義（驗收完整後填）**

- **本 ms 構成的 G1 可觀察價值**：（填）｜需求定義價值制衡：（填）
- **本 ms 包含功能**：（逐項引用功能地圖中的功能短名）
- **後續 ms 的功能**（記入 SLUG §3 待辦清單）：（填）
...
> 行動只能依賴已完成項。單點隨相關實作變更執行，整合只在接合點就緒或相關介面／依賴變更時執行。開發以**功能**為 commit 單位；本 ms 功能全部完成後執行端到端完整流程，ms 為整體驗收節點。
...
> 起始效應對策（premortem，SKILL §5 時點 1 對抗·premortem 挑戰＋CLI plan→test 機械閘結構驗）：規劃完美盲信的制衡——列不出真實失敗點＝沒想過會怎麼失敗。plan→test 機械閘查核本段非敷衍；敷衍（無／無風險）即擋。
...
> 精簡錨定（A4、references/PLAN.md）：§10 三對六向的完整核對由秘書於定義層二收斂時一次執行，G3 內不重複自查。簡單 ms 本段從缺；複雜 ms 才精簡列風險自查（如「驗收判準來自 G1，非 G2 反推」）。
