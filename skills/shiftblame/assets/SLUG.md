...
> 審計階段主導，承接 sb-think 已確認語義後直接定稿（一次定律，SKILL §1.1）；只寫需求／驗收面向，不寫技術選型或實作步驟。G1 只由審計段改寫；plan→test 放行後由 CLI hash 封存，執行記錄與複驗結果寫 tmp，不寫回 G1。契約不足／衝突＝回 intent（老闆補充路徑）重定義後重新放行。
...
> 每項使用者需求至少一列，`AC-ID` 在本 ms 內穩定且唯一。固定單行格式供 CLI 建立追溯鏈；值內以全形「｜」替代半形 `|`。`證據` 一律為 `BEHAVIOR`：檔案、字串、節點、schema 等結構正確只能作開發自驗，不能代替使用者需求。
...
> §10 三對六向（G1↔G2、G1↔G3、G2↔G3）由秘書於放行前（plan→test 邊）一次核對，不在各文件內重複自查；缺漏由責任面向一次補正後重核（一次定律，SKILL §1.1），不環形重跑。

### G2 範本 — 技術分析（研究階段主導）

> 研究階段主導，**外部證據打底**：進段後至少一次外部工具調用（research→plan 邊機械驗），只記選型結論＋可行性證據，向前對齊 G1 一次定稿（一次定律，SKILL §1.1）。G2 只由研究階段改寫，保持乾淨的決策結論；發現 G1 需求技術不可行屬重大例外（SKILL §1.4.1）退回審計階段，不在 G2 內改寫需求。
...
> §10 三對六向由秘書於放行前（plan→test 邊）一次核對，不在各文件內重複自查；缺漏由責任面向一次補正後重核（一次定律，SKILL §1.1），不環形重跑。

### G3 範本 — 實作計畫（規劃階段主導）

> 規劃階段主導，**對齊規劃**：向前對齊 G1＋G2 一次定稿（一次定律，SKILL §1.1）。G3 依序完成：①回讀 G1 寫驗收；②驗收完整後定義本 ms 範圍、③依 G2 寫實作步驟——順序固定（①驗收→②範圍→③步驟）。驗收、ms 範圍、實作步驟為 §10 回指結構所需 MUST 填實；開發策略、預期變更、一致前檢查依複雜度精簡或從缺（§5 由秘書放行前 §10 一次核對取代）。G3 只由規劃階段改寫；§2.5 收斂執行記錄由秘書補寫。
