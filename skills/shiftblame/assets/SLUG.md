...
- 狀態：待驗收／返工／合格。每個功能的合格/返工由 SECRETARY 讀過 `tmp/` 中落地側三權的執行記錄後**判決**；落地側三權如實整理各自產出、不下判決。返工分兩類：**常態修正**（與 G1/G2/G3 有出入但不改方向／架構——對應顧問側角色子代理直接修該份文件後，疊加新 commit 重驗）與**重大例外**（改方向／架構——停止開發，走重大例外遷移回三權制衡，SKILL §1.4.1）。合格進入下一個里程碑。
