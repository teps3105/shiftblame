...
只有老闆說「開新 ms」時才能新增一列；同一子需求的追加／重修更新原列（同 ms 回 intent 重走）。合法段與八段一一對應：`intent／audit／research／plan／test／build／verify／done`。回頭自由（任意段→intent 同 ms 重走、done→test 重修）；前進要鑰匙（SKILL 授權章）。
...
### 時點對抗記錄（--adversarial 對照源；CLI 逐字比對「時點①對抗」等字樣）

> 每次時點對抗完成後，秘書在此記一行自然語言摘要（做了什麼攻擊、複核結論、反向對抗判定）；CLI 在對抗邊推進時逐字對照本記錄，缺記錄即擋。

- 時點①對抗（放行前）：（填：攻擊點／複核／反向對抗判定）
- 時點②對抗（判決前）：（填）
- 時點③對抗（收斂複驗）：（填）
...
```mermaid
flowchart LR
    A[intent]:::todo --> B[audit]:::todo --> C[research]:::todo --> D[plan]:::todo
...
> 審計階段主導（G1 定義邊；verify 為裁判邊），**審計先於研究**——先沉澱現況基準（盤點 codebase 實況、對照文件差異、識別過時假設），再承接 sb-think 已確認語義直接定稿（輪內單向定律，SKILL §1.1）；只寫需求／驗收面向，不寫技術選型或實作步驟。每項 AC-ID 以 BDD 行為規格表達（Given 前置情境／When 操作／Then 可觀察結果＋使用者＋失敗邊界）。G1 只由審計段改寫；plan→test 放行後由 CLI hash 封存，執行記錄與複驗結果寫 tmp，不寫回 G1。契約不足／衝突＝回 intent 開新輪（CLI 凍結 rev 基線，老闆補充路徑）重定義後重新放行。
...
> 每項使用者需求至少一項，`AC-ID` 在本 ms 內穩定且唯一；每項以 `### AC-` 分段（audit→research 邊 BDD 格式閘機械驗五鍵齊全）。行為規格逼出的問題（誰、什麼情境、做什麼、觀察到什麼、什麼算失敗）字面搬運答不出——需求以行為語義立法。鍵義：Given＝前置情境（使用者所處狀態）、When＝操作（做什麼）、Then＝可觀察結果（觀察到什麼）、使用者＝誰、失敗邊界＝什麼情況判失敗。`證據` 一律為 `BEHAVIOR`：檔案、字串、節點、schema 等結構正確只能作開發自驗，不能代替使用者需求。
...
> 研究階段主導（G2 定義邊；build 為落地邊——實作回指 G2），**外部證據打底**：進段後至少一次外部工具調用（research→plan 邊機械驗），只記選型結論＋可行性證據，向前對齊 G1（含現況基準）一次定稿（輪內單向定律，SKILL §1.1）。G2 只由研究階段改寫，保持乾淨的決策結論；發現 G1 需求技術不可行屬重大例外（SKILL §1.4.1）退回審計階段，不在 G2 內改寫需求。
...
> 規劃階段主導（G3 定義邊；test 為落地邊——測試碼回指驗收排程），**對齊規劃**：向前對齊 G1＋G2 一次定稿（輪內單向定律，SKILL §1.1）。G3 依序完成：①回讀 G1 寫驗收；②驗收完整後定義本 ms 範圍、③依 G2 寫實作步驟——順序固定（①驗收→②範圍→③步驟）。驗收、ms 範圍、實作步驟為 §10 回指結構所需 MUST 填實；開發策略、預期變更、一致前檢查依複雜度精簡或從缺（§5 由秘書放行前 §10 一次核對取代）。G3 只由規劃階段改寫；§2.5 收斂執行記錄由秘書補寫。
...
> 起始效應對策（SKILL §1.8）：規劃完美盲信的制衡——列不出真實失敗點＝沒想過會怎麼失敗。plan→test 放行邊查核本段非敷衍；敷衍（無／無風險）即擋。
