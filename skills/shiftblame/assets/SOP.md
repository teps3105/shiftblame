...
- shiftblame 的角色、授權、路由、G1／G2／G3、三面向雙面結構（定義／落地）、pass 出口、commit、分支、收尾、歸檔或臨時檢閱規則；這些由中央技能文件管理，不在專案 SOP 重述。
...
SOP 隨 codebase 變更同批更新（same-commit，兩層文件模型 A9）：環境值被替換、服務邊界改變或規範失效時，秘書 在造成變更的同一批修改中刪除舊值並同步更新相關段落——模糊的雙版本說法保持零出現，單一真相。
