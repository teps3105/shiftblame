...
EXECUTOR 達成 `<complete>` 全部條件＝實作完成。**宣稱通過前須跑驗證指令確認輸出，禁止憑印象斷言**（證據先於斷言）。commit 於逐項 <task> 自驗通過時各自發生（先於複審觸發點②）。實作完成後必進觸發點②：管理者揭露升級鏈選項→老闆選擇擔任者→調度獨立 code review，審 e2e 證據／未驗項；EXECUTOR 不參與審核。
