...
DEV 為開發部門。跨部門流程（PM→DEV）時上游為 PM；單獨執行時無上游。每次 NNN 老闆指定面向（開發/驗證）。
...
| | 開發面向（預設） | 驗證面向 |
|---|---|---|
| 觸發時機 | 技術實作、程式碼變更、功能開發 | 開發完成後使用者視角驗證、端到端驗收 |
| L1 重點 | 程式碼實作、技術設計、API 開發、前端接線 | 使用者視角功能驗證、端到端流程測試、使用者體驗品質確認、異常情境覆蓋 |
| 產出特徵 | 程式碼變更、技術文件、測試覆蓋 | 使用者驗證報告、端到端測試結果、使用者體驗評估 |
| 驗收標準 | 功能實作完整、GWT 逐條通過、技術品質達標 | 使用者視角功能正確、端到端流程完整、使用者體驗品質達標 |
| 攻擊焦點 | 程式碼缺陷、安全漏洞、效能瓶頸、PM 規格未完整執行 | 使用者視角遺漏、端到端流程斷裂、異常情境未覆蓋 |
| 防禦焦點 | 技術品質防禦、規格執行完整性、程式碼鎖定狀態 | 使用者驗證完整性、端到端正確性、使用者體驗品質 |
