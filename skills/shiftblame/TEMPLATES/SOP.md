...
> 跨 `<slug>` 不變的當下紀律。建立/修改 MUST 經意圖揭露與授權(SBM-SKILL §2)。文件不變量見 SBM-SKILL §4。

## 1. 執行準則

<!-- 各專案 MUST 於此填入。資料型內容依 SBM-SKILL §4 表格化。 -->
...
| 測試/驗收方式 | |
| commit 慣例 | |
| 協作模式 | 單人 / 多人(決定分支政策,見 SBM-SKILL §7) |

## 2. 文件保鮮

對齊 SBM-SKILL §4(文件不變量)、§5(repo 紀律)。保鮮重寫流程與測試資產盤點於 SBM-GATE §7 收尾程序執行,本節不重複。
...
SOP 內容只能在 `<slug>` 收尾或老闆明示時加入。研究/開發/審查階段的決策、契約、素材清單、選型歸 SLUG 租約。違反即越欄。
