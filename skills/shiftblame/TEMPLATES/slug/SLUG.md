...
**`<nnn>` 狀態**：`START → 正方 → 反方 → 收斂 → G3 執行規劃層（產 <plan>）→ 執行者逐項實作（每項 <task> 自驗通過即 commit）→ <complete> 達成`。`<nnn>` 層級無 gate/PASS/FAIL，推進 MUST 由老闆指示。

**Slug 狀態**：`IN_PROGRESS → PASSED`（老闆拍板 slug 結束）。

## 3. 雙軌策略

固定雙軌（正反收斂）+ G3 執行規劃層：正方主 session（G1 外部研究規劃視角 + G2 內部技術實作視角，身份切換）、反方子代理獨立，身份 MAY 跨 `<nnn>` 續用（正反收斂軌）。G3 為共識後執行規劃層（非正反收斂軌），承接 G1+G2 基線產 `<plan>` 供執行者執行（見 SBM-SKILL §5.13）。執行者為實作軌。
