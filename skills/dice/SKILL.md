...
1. **盤點邊界**：讀取當前 branch／slug／ms／節點、`git status`、working tree diff，以及相對最後已知良好 commit 的提交範圍。逐項標成「保留／應提交／丟棄／未決」——丟棄以全項已決為前提（未決項先由老闆裁定）。
2. **選最小充分層級**：由小到大選第一個足以移除失敗成果的範圍（範圍選擇以證據為準）：
...
4. **執行**：
   - 未提交變更只對已確認 path／hunk 執行精確還原（還原工具限精確形態；`reset --hard`／`clean`／遞迴刪除屬無範圍操作，超出本流程工具箱）。
...
- 主對話秘書獨佔範圍判定、reset、commit、路由與執行後驗證；臨時檢閱意見僅供輸入（決定與執行權在秘書）。
- MUST 選擇**最小充分丟棄範圍**——失效判定以證據為準（證據證明局部即局部）。
- MUST 保護無關變更、使用者既有變更、較早已走 pass 出口的 ms 與已歸檔成果。
- 無法唯一切出 commit／path 範圍，或回復點未核實時，判定為未決並停止——不確定性由揭穿與等待消除（老闆裁定）。
