...
2. **放行後進入開發**：主對話 SECRETARY 依 G3 實作計畫採多循環螺旋開發（§1.4），派發**落地側三權**執行每個功能（commit 單位）；驗收節點是**里程碑**（G3 §1.5）。每個功能：SECRETARY 派發 TESTER 寫測試定義「過」（依 G3）→ DEVELOPER 寫實作碼（依 G2）→ ACCEPTOR 把東西修到綠燈、驗收「完成」（對照 G1）→ SECRETARY 讀 `tmp/` 三份產出後**判決合格才 commit（獨佔）**。**測試先行**：先寫測試（紅燈）再實作，與顧問側「驗收先於實作」對齊。低複雜度功能 SECRETARY MAY 直接執行不派發落地側三權。該里程碑所有功能 commit 完成後才在里程碑邊界做階段驗收。**commit 與所有判決（合格/返工）由 SECRETARY 獨佔**；落地側子代理（DEVELOPER/TESTER/ACCEPTOR）可在授權範圍內寫 repo（DEVELOPER 寫實作碼、TESTER 寫測試碼、ACCEPTOR 不碰實作碼／測試邏輯但可寫測試環境配套並可跑測試命令）但皆不可 commit；顧問側子代理（AUDITOR/RESEARCHER/PLANNER）對 repo 唯讀。
