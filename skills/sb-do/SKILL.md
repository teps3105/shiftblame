...
2. **放行後進入開發**：主對話 SECRETARY 親自依 G3 實作計畫採多循環螺旋開發（§1.4）；驗收節點是**里程碑**（G3 §1.5），功能降為 commit 單位——SECRETARY 逐個功能自驗後 commit，該里程碑所有功能 commit 完成後才在里程碑邊界做階段驗收。repo 開發、commit、測試、自驗唯一執行者，子代理對 repo 一律唯讀。
