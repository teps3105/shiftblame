...
讀取當前 `<repo>/.shiftblame/ROADMAP.md`、`<repo>/.shiftblame/SOP.md`、當前 `<slug>` 與 archive 脈絡，判定修改意圖並提出建議（附脈絡依據）。**提議≠授權：授權來自老闆明確指示**（SKILL §2、§9）——寫入於授權後執行。
...
- 調整 §2 順序只改變計畫的排列位置——排列本身是唯一排序載體（序號、優先級文字等其他排序語意一律排除）。
