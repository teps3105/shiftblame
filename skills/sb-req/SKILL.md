---
name: sb-req
description: 重大例外——開發途中強制停止並回 G1 重新確認需求方向；方向錯誤的 commit 由主對話 SECRETARY 用 reset 回退（判決性工作）。
---
# sb-req — 重大例外：開發途中回 G1 重新確認需求

當使用者要求「回 G1」「重新確認需求」「方向錯了要重來」時執行本 prompt。用於**開發途中**發現**需求方向錯誤**（重大例外，SKILL §1.4）——改變需求方向、整體架構或需老闆重新授權，強制停止開發回到當前 `<nnn>` 的三權制衡。一般需求細節與實作情境的出入由常態修正處理（SECRETARY 派 AUDITOR 直接修 G1），不屬本 prompt。
