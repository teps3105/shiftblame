...
3. **等待兩者完成**
4. **交叉比對**：讀取 `slug.md` + `slug.codex.md`，標記 CONVERGED / CLAUDE_ONLY / CODEX_ONLY / CONFLICT
5. **AskUserQuestion 回報老闆**：收斂項只報數量，分歧項逐條呈報，等老闆裁定
6. **覆述裁定 → 結束 turn**：結構性保證不自動推進
7. **老闆下一則訊息 → 派工下一部門**（含裁決結論注入）
