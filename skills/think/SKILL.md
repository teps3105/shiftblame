...
**Jev 常駐工作判斷**：分發後即依主 SKILL §12 與 `../shiftblame/references/JEV.md` 在日常工作判斷持續分工；工具／資料選擇、除錯定位、方案比較等適用窄判斷，直接委派或重用，不等到流程節點或審查。主對話處理複雜推理、例外與實際執行；本技能的意圖理解與老闆授權不交給 Jev。

think 分發後，agents 依主 SKILL §12 分配日常工作判斷，並保有自主判斷權——判斷「要不要回 think」仍是主對話的職責；入口端維持先理解再執行，不因 Jev 的工作建議改變授權範圍。
