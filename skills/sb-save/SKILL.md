...
當老闆要記錄當前工作落點時執行（由 sb-think 分發）。用於顯式記錄當前工作落點到 <repo>/.shiftblame/<slug>/SLUG.md，讓其他對話能用 sb-resume 無縫恢復。
...
- sb-save **只更新 <repo>/.shiftblame/<slug>/SLUG.md**（frontmatter `last_save` + §4 節點 + §8 交接摘要），不建立快照檔、不動 G1／G2／G3、不動 repo。
