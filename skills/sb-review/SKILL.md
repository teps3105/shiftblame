...
0. **SECRETARY 確認開發完成**（主對話）：G3 所有里程碑驗收合格（§1.4 所有里程碑的階段驗收通過，含每個里程碑所有功能已經落地側三權 DEVELOPER→TESTER→ACCEPTOR 產出、SECRETARY 判決合格並 commit）。
1. **提交證據**（主對話 SECRETARY）：彙整 G3 行為證據與未驗項；證據描述使用者可觀察的行為，不以檔案、字串、grep 命中代替。落地側三權的 `tmp/` 產出（變更摘要、測試報告、驗收報告）為證據來源之一，但仍須 SECRETARY 轉譯為可觀察行為描述。
2. **顧問側三者各自重審主導文件**（SECRETARY 派發三個顧問側角色子代理）：
