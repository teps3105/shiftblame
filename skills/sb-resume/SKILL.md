...
3. **基於既有重新確認 G1~G3**（非清空重寫）：先讀 `flow-state.json`。若目前 ms 已有 `g1Contract`，MUST 先核對 G1 hash，且 G1 保持封存；codebase 差異只能作為可行性證據，不能反向改義需求。依承載歸屬重新確認其餘文件：
   - 仍成立 → 保留。
...
- 重新確認（無 `last_save` 時）基於既有內容，保留仍成立者，不從零重寫；已放行 G1 的任何語意變更都走顯式修約（SKILL §1.4.1），不以 resume 繞過契約鎖定。
- SLUG §4 節點依當前實際狀態標記。
