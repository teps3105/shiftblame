...
3. **建立 slug 骨架**（**定義單檔、結構分檔**，權威結構見 SKILL §8）：
   ...
   - 複製 `assets/SLUG.md` 範本 → `.shiftblame/<slug>/SLUG.md`（SLUG 主體 §1-§8；§4 初始節點 `三面向制衡（G1→G2→G3）`；§3 待辦清單初始為空或填已授權的未開待辦）。
   - 建立 `.shiftblame/<slug>/<nnn>/` 子目錄（初始 nnn 如 `001`）。
   - 從 `assets/SLUG.md`「三面向範本」段複製 G1/G2/G3 三份。
4. **進入制衡**：依承載歸屬推進定義層時序——審計→G1（子代理）、研究→G2（主對話）、規劃→G3（子代理）——產出三份文件（承載細節見 SKILL §3）。
...
2. **SLUG §4 加新列**：在 `.shiftblame/<slug>/SLUG.md` §4 目前節點表新增該 `<nnn>` 列（初始節點 `三面向制衡（G1→G2→G3）`）。該待辦從 SLUG §3 待辦清單移除（開工即出清單）。
3. **建立 nnn 子目錄**：`.shiftblame/<slug>/<nnn>/`。
4. **複製範本為分檔**：從 `assets/SLUG.md`「三面向範本」段複製 G1/G2/G3 三份。
5. **進入制衡**：派發定義層時序產出三份文件。
