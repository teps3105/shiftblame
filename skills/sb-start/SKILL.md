...
3. **建立 slug 骨架**（**定義單檔、結構分檔**，權威結構見 SKILL §8）：
   ...
   - 複製 `assets/SLUG.md` 範本 → `.shiftblame/<slug>/SLUG.md`（SLUG 主體 §1-§8；§4 初始節點 `三面向制衡（G1↔G2↔G3）`＋級別欄；§3 待辦清單初始為空或填已授權的未開待辦）。
...
2. **SLUG §4 加新列**：在 `.shiftblame/<slug>/SLUG.md` §4 目前節點表新增該 `<nnn>` 列（初始節點 `三面向制衡（G1↔G2↔G3）`＋級別）。該待辦從 SLUG §3 待辦清單移除（開工即出清單）。
