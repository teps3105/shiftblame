...
3. **建立 slug 骨架**（**定義單檔、結構分檔**，權威結構見 SKILL §8）：
   ...
   - 複製 `assets/SLUG.md` 範本 → `.shiftblame/<slug>/SLUG.md`（SLUG 主體 §1-§8；§4 初始節點 `三面向制衡（G1↔G2↔G3）`＋級別欄；§3 里程碑清單初始為空或填已授權的未開里程碑）。
...
1. **取里程碑**：依 SLUG §3 里程碑清單**由上到下的順序**取最上面未開的里程碑（順序即優先序；老闆另行指定者除外）。
2. **SLUG §4 加新列**：在 `.shiftblame/<slug>/SLUG.md` §4 目前節點表新增該 `<nnn>` 列（初始節點 `三面向制衡（G1↔G2↔G3）`＋級別）。該里程碑從 SLUG §3 里程碑清單移除（開工即出清單）。
3. **建立 nnn 子目錄**：`.shiftblame/<slug>/<nnn>/`。
4. **複製範本為分檔**：從 `assets/SLUG.md`「三面向範本」段複製——**L 級** → G1/G2/G3 三份；**M 級** → 僅 G1 一份。
5. **進入制衡**：**L 級**派發定義層時序產出三份文件；**M 級**秘書直接產 G1 單份。
