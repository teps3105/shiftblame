...
const CARD = [ // 核心不變量；RAM/ROM 分層（G/SLUG=ROM、tmp+flow-state=RAM）；審計＝確認→分發邊的外部對抗
  '[shiftblame 不變量]',
  '①老闆任何輸入第一步調 shiftblame:think（帶 args 理解宣告），先理解意圖再行動——六欄理解立即呈現給老闆（先跑內部機制再揭露＝讓老闆乾等）。對老闆的輸出是人話：揭露首行＝一句翻譯「你要的是 X」、停等首行＝等你判定的事、待決＝具體問題，無開場白無客套（think SKILL「輸出形狀（人話契約）」）。開放問題明確列出提問停等——沉默≠批准，靜默自裁＝越權；老闆已明確指定者照辦不重問。未覆蓋即凍結：新輸入未經帶 args 的 think 理解宣告覆蓋前，hooks 硬擋寫入類與流程推進——第一步 think 由機械保證，不是意識到才補路由。新需求主動問走 main（意圖揭露→迭代實作→對抗＋提交閘→老闆判 pass/fail）還是開 slug（管理文件＋<type>/<slug> 分支六段流程）；免問：已有活動 slug（續 slug）、純問題類（直接答）、老闆已明確指定。老闆任何新輸入（含兩時點 fail）回意圖揭露經 intent 路由器路由——定義級 sb next intent 同 ms 開新輪（計返工輪＋rewrite 載入閘）、段內修復自動旗標切段；確認→審計（推進指令外部對抗）→分發——審計邊終點＝推進起點。收斂定案權在老闆。',
  '②兩層兩段式（段鏈）：定義層 requirement→research→plan（逐功能規劃循環→規劃收斂）；時點 1 對抗在前、老闆判定在後（pass 才 requirement→research 推進——審意圖→需求翻譯）；plan→test 機械推進（零審核）。實作層 test→build→verify——逐功能：提交閘 commit（單功能單提交，測試碼＋實作碼同 commit）回 test 接下一功能；全部功能完成後時點 2 對抗在前、老闆判定在後（build→verify 邊——審驗收資格：GWT 回指意圖、假綠燈）；verify＝真驗收執行——G1 GWT 逐條＝驗收劇本（Given 實際建立→When 實際操作→Then 觀察真實行為→證據落回指區；驗收依據＝行為是否發生，非測試燈號）；驗收完成老闆終審 pass 出口 next（sb next intent --new-ms --boss-ok）或 end（sb end --boss-ok——終審章，對抗已在進段前）。老闆任何新輸入（含兩時點 fail）回意圖揭露經 intent 路由器路由；段內修復自動旗標切段不停等不計輪。前進要鑰匙：老闆決策邊須 --boss-ok——由老闆實際輸入承載（輸入流時戳新鮮度，CLI 驗；時點對抗邊另須老闆輸入晚於本次對抗條目——老闆章錨定對抗報告之後，防舊輸入冒名），對抗條目與理解宣告不替代老闆章；缺老闆決策即 sb stop-report --question 申報停等。',
  ...
  '⑦版號屬老闆決策。',
  '⑧提交＝sb commitmsg（格式驗證＋staged 系統檔檢查＋印章；hooks 於 commit 驗章焚章——審核不在提交時點，對抗承載於兩時點）；測試碼＋實作碼同 commit——單功能單提交。',
