...
- **L2（基本）**：RES 研究 → 模式確認（L2）→ MIS 執行收尾 → 秘書復判 → 收尾（歸檔）
- **L3（標準）**：RES 研究 → 模式確認（L3）→ 老闆決策 → RES → PRD → DEV（可多輪）→ MIS(尾) → 秘書復判 → 收尾（歸檔）
- **L4（完整）**：RES 研究 → 模式確認（L4）→ 老闆決策 → RES → QA → PRD → DEV（可多輪）→ QC → MIS(尾) → 秘書復判 → 收尾（歸檔）
- **L5（高等）**：RES 研究 → 模式確認（L5）→ 老闆決策 → RES → SEC → QA → PRD → DEV（可多輪）→ QC → EXP → MIS(尾) → 秘書復判 → 收尾（歸檔）
