...
| SECRETARY | 揭露意圖、記錄老闆路由、交接、收尾與文件保鮮；不得自行決定，可提出路由提議但不等於授權 |
| RESEARCHER | 主導 G2，承接 G1 並取得、複核外部獨立研究 |
| DEVELOPER | 主導 G3，先寫驗收再寫計畫；通過三份文件制衡後開發並交證據 |
| AUDITOR | 開發前與開發後都派發唯讀子代理獨立審核，再由主對話複核並回頭對照 G1 |

子代理只提供唯讀研究或審查意見，不取得路由權、修改權或判決權；主對話角色必須複核並承擔結論，repo 寫入只在主對話 DEVELOPER 角色。
