...
`shiftblame` 是一套 AI agents 流程定義框架，以純 Markdown 定義檔構建跨模型的協作流程。三名員工（claude / codex / gemini）透過 `terminal()` 直接呼叫，各自使用獨立模型，在同一個 worktree 上透過固定角色分工機制協作：claude 固定為開發部門主執行者，codex 與 gemini 固定擔任監督者（不具備任何寫入權限）。由秘書統籌研究、派工與收尾，管理者協調部門管線。
...
| 角色 | 身份 | 寫入檔案 | worktree 權限 |
|------|------|----------|:---:|
| claude | 主執行者（固定） | result.md | 獨佔編輯權與 Git 操作權 |
| codex | 監督者（固定） | review.md | 無寫入權限 |
| gemini | 監督者（固定） | review.md | 無寫入權限 |
...
開發部門完成後、進入品管前，管理者執行 E2E 實際驗證閘門：

**前置條件**：專案必須具備實際可 E2E 驗證的框架（如 chrome-devtools-mcp 適用於 Web SPA、其他對應自動化框架適用於其技術棧）。E2E 驗證要求實際開啟程式在實體螢幕上執行，不可僅以 API 呼叫或 headless 模式替代。若無可用框架，管理者搜尋可用 MCP/自動化框架，呈報老闆覆核後決定安裝或退回 DEV 建置。不可跳過。

1. 管理者實際啟動應用，以使用者身份操作端到端流程
2. 驗證核心功能路徑走通（Web SPA 透過 chrome-devtools-mcp；其他技術棧透過對應自動化框架）
3. E2E 通過後以 `clarify()` 呈報老闆覆核
4. 老闆判定通過 → 推進 QC；不通過 → 退回 DEV

QC 退回 DEV 修正後，必須再次走 E2E + 老闆覆核才可重新進入 QC。同一 slug 下最多 5 次，超過退回 PRD。
...
管理者透過 `process(action='poll')` 嗅探各 CLI 執行狀態，自動處理超時與錯誤。部門切換時必須終止前一個部門的嗅探程序。
