...
<p align="center">
  <strong>AI Agents 協作框架</strong> — 用純 Markdown 定義檔，四出口（計畫/開發/驗收/展望）先提出再質疑，G4 展望後提交收尾。
</p>
...
**核心思想：意圖揭露 + 先提出再質疑 + 展望後提交。** AI 的每步行動都必須先向老闆說明「我要做什麼、為什麼」，經確認後才執行。不是事後解釋，是事前揭露。管線採「先提出再質疑」模式——每個閘門至少正→反→收斂三輪辯論。展望後提交，收尾或暫停等下一 NNN。
...
計畫使用 5W1H 格式（Who/What/When/Where/Why/How），開發使用 TDD 模式（紅燈→綠燈→重構），驗收使用 GWT 格式（Given→When→Then）。展望進行文件更新與提出下一步方向。老闆可隨時跳過管線直接在 main 上操作，但責任屬於老闆，框架不提供質疑保障。
