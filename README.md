...
- **段-檔承載與輪內單向。** 意圖與六段由四份文件承載成四條閉環軸——SLUG（intent 入口＋ms 進度與定案）、G1（requirement 定義＋verify 裁判）、G2（research 定義＋build 落地）、G3（plan 定義＋test 落地）；推進呈 Z 字形，落地段反向回指承載檔（test→G3 驗收排程、build→G2 技術方案、verify→G1 逐項 AC 判定）。
  ...
  - G 檔寫入權分區（定義區：G1→requirement／G2→research／G3→plan；回指區：G1←verify 判定／G2←build 偏離／G3←test 映射；時點 1 邊（requirement→research）CLI 對定義區 hash 封存）——跨區（落地段改定義區）＝綁架上游死路。
...
- **提交＝機械格式閘（2.4.0——審核資源前移時點 1 與時點 2）。** 提交審核已移除——中間產物老闆不看，段內提交僅 `sb commitmsg` 格式驗證＋印章，hooks 於實際 commit 驗章焚章（一對一）；審核承載於兩時點對抗（時點 1＝requirement→research 邊審意圖→需求翻譯、時點 2＝ms 出口驗收回指意圖），對抗 MUST 子代理，無自代介面（工具不可用即阻塞等待）。
...
- **兩層文件模型。** 文件↔實況對照是一等公民：永續層（docs/、SOP、ROADMAP、README、skills/）是唯一需與實況對照的文件。
  ...
  - 永續層文件隨程式碼即時變更且文件先行——build 段先把永續層文件改到目標狀態再依文件寫實作碼，不一致回頭修文件再繼續（same-commit：改了什麼就行為什麼文件）——same-commit 文件缺漏是時點 2 對抗的標準攻擊點（staged 程式碼變更 vs 文件零變更正當性與註釋行為一致性）。
...
- **決策權中央集權。** 所有判決（時點對抗後推進、合格/返工、commit、路由、reset、pass 出口）由主對話秘書獨佔；臨時檢閱意見只作輸入。
...
- **兩時點對抗＋adversarialLog point 條目對照（對抗在前、老闆判定在後）。** 時點 1＝requirement→research 邊（審意圖→需求翻譯：GWT 能否從行為矩陣還原一列、翻譯保真）、時點 2＝ms 出口（驗收回指意圖：測試綠但 AC 從行為矩陣還原不出＝假綠燈）——推進帶 `--adversarial` 宣告（`sb adversarial <報告> --point 1|2`），CLI 對照 flow-state 的 adversarialLog point 條目及其新鮮度，不一致即擋；報告全文寫入 tmp。中鏈（research→plan→test→build→verify）與段內提交零審核資源——僅 `sb commitmsg` 機械格式閘。
  - 時點 2 由兩個 pass 出口（`--new-ms`／`sb end`）的 CLI 檢查把關（條目晚於末次進 verify——功能循環多次判決時仍須新鮮）。
  - 複核結論每項裁定綁可查證出處，反向對抗判定成立才推進。
  - 對抗 MUST 外部唯讀子代理——無自代介面：工具不可用即阻塞等待至可用；報告落檔＋`sb adversarial <報告> --point 1|2` 宣告（2.4.0 起 `--point` 必帶）。
...
- **三層測試與功能迭代循環。** test 先定義並定稿判準；build 隨相關變更執行單點測試（函式／元件／規則），到達接合點或介面／依賴變更時執行整合測試（模組協作），再經段內提交閘（`sb commitmsg` 格式閘）以單功能單提交存檔（測試碼＋實作碼同 commit）。
...
- **G1 是封存契約。** 時點 1 邊（requirement→research）CLI 將 G1 的 SHA-256 封存於 flow-state.json——本輪自進 research 起全鏈凍結（該邊本身例外：research→requirement 旗標切段補正後重進即重封存）；後續每次推進重算核對。G2／G3 的一切改寫（含 CONFORMS 單調細化）經回 intent 開新輪至定義段執行（G1 契約隨回 intent 解除，需求重定義後再過時點 1 重封存；history 留時序）。契約不足或衝突＝回 intent 開新輪重定義後重過時點 1。
...
- **驗收使用者需求，不驗收結構幻象。** G1 每項需求使用唯一 AC-ID 與 BEHAVIOR 契約；G3 驗收表逐項排程並映射測試。CI 綠燈或結構正確不能單獨判定完成；功能判決核對當次範圍。
  - ms 整體驗收時，全部必填 AC 須有 commit 與實際操作／觀察的 SATISFIED 行為證據（引用專案輸出以節錄快照為證）。功能 AC 判定屬段內秘書判決（中鏈零審核）、ms 由時點 2 對抗查核（對抗在前、老闆判定在後），verify 邊核對 git 一致性。
- **commit 集權。** commit 一律由主對話秘書執行且必過 `sb commitmsg`（hooks 留痕硬擋）；build 段完成即存檔（commit 先於驗收）。跨 repo 提交以 `git -C <絕對路徑>` 的目標為查證錨點——印章與 staged 檢查都對錨定 repo 生效（相對 `-C` 擋）。
...
> - **臨時外部子代理檢閱**：兩時點強制對抗（時點 1＝requirement→research 邊——審意圖→需求翻譯；時點 2＝ms 出口——驗收回指意圖；對抗在前老闆判定在後）＋純技術不可可靠裁定時強制技術意見；其他高風險情境按需取得。無自代介面：子代理不可用即阻塞等待至可用。不移交工作狀態或裁定權。
