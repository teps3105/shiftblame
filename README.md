...
- **不同視角按需取得。** Shiftblame 不提供持久外部分工配置，也不因階段切換自動委派；能可靠裁定但屬複雜、高風險、無先例或需要反證時，主對話可再依信心增益取得一次唯讀意見。外援只提供輸入，不接管工作狀態或裁定。
...
> - **臨時外部子代理檢閱**：純技術不可可靠裁定時強制取得一次唯讀意見；其他高風險或反證需求可選擇取得。不移交工作狀態或裁定權。
