...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-0.3.0-2ea44f.svg" alt="version 0.3.0"/>
</p>
...
- **三權分立。** G1（需求）、G2（技術）、G3（計畫）各由主導角色產出，互相制衡；三份兩兩雙向一致才可開發。一致以 G1 需求項為對齊軸，三對各須正向承接＋反向回指（判準見 SKILL §10）。
- **驗收先於計畫。** G3 內部先依 G1 寫驗收，再依 G2 寫實作步驟，不得倒序。
- **repo 開發權集中。** 主對話 SECRETARY 是唯一 repo 開發執行者；子代理對 repo 一律唯讀。
- **回退不猜修。** 不符合時回三權制衡重走，而不是直接猜測修補法。
