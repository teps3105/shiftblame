...
<p align="center">
  <strong>AI Agents 協作框架</strong> — 純 Markdown 定義檔，雙軌平行：G1 計畫視角 ‖ G2 技術視角，每 NNN 正→反→收斂。
</p>
...
**核心思想：意圖揭露＋雙軌平行＋先提案再質疑。** AI 的每步行動都必須先向老闆說明「要做什麼、為什麼」，經確認後才執行。不是事後解釋，是事前揭露。管線採「雙軌平行」模式——G1（計畫視角）與 G2（技術視角）同時進行正→反→收斂。收斂分兩份各自結論，按兩份收斂結論實作。每個 slug 依規模開 1~N 個 NNN，每個 NNN = 一輪辯論。
...
| 設計和實作脫節 | **雙軌平行**：計畫問題歸 G1，技術問題歸 G2，同時進行 |
...
每個任務（slug）走雙軌平行管線，G1 與 G2 **同時進行**，不存在先後關係：

```
老闆觸發 → 建立 slug
      │
      ▼
NNN 001：G1（計畫視角）‖ G2（技術視角）同時進行
      │
      ├── 正方（平行）：G1 提計畫 ‖ G2 提技術
...
| 視角 | 正方 | 反方 | 收斂 |
|:----:|------|------|------|
| G1 計畫 | 需求翻譯、5W1H、可行性評估 | 質疑遺漏/可行性/越權 | 管理者收斂計畫結論 |
| G2 技術 | 技術選型、GWT 測試、TDD 實作 | 質疑覆蓋率/品質/一致性 | 管理者收斂技術結論 |

**關鍵**：G1 管需求與可行性（「能不能做」），G2 管技術與實作（「怎麼做」）。雙軌平行，計畫問題歸 G1，技術問題歸 G2。收斂分兩份各自結論，按兩份收斂結論實作。
...
框架會自動：啟動 → 呈現意圖 → 你確認 → 建立 slug → 雙軌平行辯論。
...
├── ROLE/
│   ├── G1.md             # 計畫視角：需求翻譯、5W1H、可行性評估
│   └── G2.md             # 技術視角：技術選型、GWT 測試、TDD
...
**正方**：從技術視角獨立提出技術選型（8 面向）、GWT 測試設計、測試規範（5 層策略）、TDD 紀律（紅燈→綠燈→重構）。不依賴當前 NNN 的 G1 產出。
