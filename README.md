...
**核心思想：意圖揭露 + 先提出再質疑 + 展望後提交。** AI 的每步行動都必須先向老闆說明「我要做什麼、為什麼」，經確認後才執行。不是事後解釋，是事前揭露。管線採「先提出再質疑」模式——每個閘門至少正→反→收斂三輪辯論。展望後提交，收尾或暫停等下一 NNN。小型變更使用簡易模式：完整正反子代理辯論，不開 slug，直接在 main 上 commit。
...
用於不需開 slug 的小型變更。完整正反子代理辯論，管理者收斂並執行變更，直接在 main 上 commit。
...
| 簡易模式 | 老闆指定 → 正→反→收斂 → PASS → main commit |
...
計畫使用 5W1H 格式（Who/What/When/Where/Why/How），開發使用 TDD 模式（紅燈→綠燈→重構），驗收使用 GWT 格式（Given→When→Then）。展望進行文件更新與提出下一步方向。簡易模式提供完整正反辯論，用於不需 slug 的小型變更，直接在 main 上 commit。
