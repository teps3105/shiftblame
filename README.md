...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.7.1-2ea44f.svg" alt="version 1.7.1"/>
</p>
...
- **三面向雙面結構。** G1/G2/G3 不只在定義層——每個面向都有定義與落地兩面：測試＝G1 落地（驗收條件可執行化）、實作＝G2 落地（技術方案落地為碼）、驗收＝G3 落地（照計畫執行收證據）；執行層時序與定義層 G1→G2→G3 正向同構。任何 G2/G3 與 G1 不一致退回 G1 重新正確定義——G1 定義權唯一——語義出入一律退回 G1。
...
- **寫測試與跑測試以狀態分離。** 測試狀態依 G1 定義「過」後即鎖定；綠燈路徑＝實作修正（測試全程唯讀）。紅燈只有兩條路：實作問題回實作狀態，或附定義錯誤理由回測試狀態重新定義。
...
文件與實況對照是一等公民（兩層文件模型）：永續層（ROADMAP、SOP、`<repo>/docs/`、`<repo>/README.md`）隨造成變化的程式碼同批更新（same-commit）——ROADMAP 移除已完成條目並修正剩餘方向、SOP 刪除被取代的值並同步段落、docs 與專案 README 對照 codebase 補齊或刪除。收尾只是機械歸檔（當下層工作文件移至 archive），零文件改寫。同批對照屬維護既有文件；新增方向與產品邊界仍須 owner 明確授權。
