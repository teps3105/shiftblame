...
- **提交＝機械格式閘（2.4.0——審核資源前移時點 1 與時點 2）。** 提交審核已移除——中間產物老闆不看，段內提交僅 `sb commitmsg` 格式驗證＋印章，hooks 於實際 commit 驗章焚章（一對一）；審核承載於兩時點對抗（時點 1＝requirement→research 邊審意圖→需求翻譯、時點 2＝build→verify 邊審驗收資格——GWT 回指、假綠燈），對抗 MUST 子代理，無自代介面（工具不可用即阻塞等待）。
...
- **段-檔承載閉環（Z 字形）。** 意圖與六段由四份文件承載：SLUG 承載 intent 入口與 ms 進度／定案；G1 閉環＝requirement（定義）＋verify（真驗收——GWT 逐條劇本實操、行為證據逐項 AC 判定）；G2 閉環＝research（定義）＋build（落地——實作回指 G2）；G3 閉環＝plan（定義）＋test（落地——測試碼回指驗收排程）。
...
- **兩時點對抗＋adversarialLog point 條目對照（對抗在前、老闆判定在後）。** 時點 1＝requirement→research 邊（審意圖→需求翻譯：GWT 能否從行為矩陣還原一列、翻譯保真）、時點 2＝build→verify 邊（審驗收資格：GWT 回指意圖、假綠燈——測試綠但 AC 從行為矩陣還原不出＝綠燈無效）——推進帶 `--adversarial` 宣告（`sb adversarial <報告> --point 1|2`），CLI 對照 flow-state 的 adversarialLog point 條目及其新鮮度，不一致即擋；報告全文寫入 tmp。中鏈（research→plan→test→build）與段內提交零審核資源——僅 `sb commitmsg` 機械格式閘。
  - 時點 2 由 build→verify 推進的 CLI 檢查把關（條目晚於同邊上次推進——修復多次重推時仍須新鮮）；verify 真驗收完成後的出口（`--new-ms`／`sb end`）＝老闆看行為證據終審——`--boss-ok` 終審章，不重驗對抗。
...
- **三層測試與功能迭代循環。** test 先定義並定稿判準；build 隨相關變更執行單點測試（函式／元件／規則），到達接合點或介面／依賴變更時執行整合測試（模組協作），再經段內提交閘（`sb commitmsg` 格式閘）以單功能單提交存檔（測試碼＋實作碼同 commit）。
  - verify 核對當次功能證據判決（功能 AC 判定＝段內判決，非時點編號）；紅燈段內旗標切段修復（verify→build→test、build→test——不停等不計輪）；通過經提交閘回 test 接下一個功能。本 ms 功能全部完成後才進實作層二收斂期：E2E（使用者入口到最終結果的完整流程）綠燈收斂於 build 段內完成——build 的功能是讓測試綠；時點 2 對抗（對抗在前、老闆判定在後）於 build→verify 邊審驗收資格，pass 才推進 verify 真驗收（GWT 逐條劇本實操、行為證據落回指區），驗收完成老闆終審 pass 才走出口（`--new-ms`／`sb end`）。
...
- **驗收使用者需求，不驗收結構幻象。** G1 每項需求使用唯一 AC-ID 與 BEHAVIOR 契約；G3 驗收表逐項排程並映射測試。CI 綠燈或結構正確不能單獨判定完成；功能判決核對當次範圍。
  - ms 整體驗收時，全部必填 AC 須有 commit 與實際操作／觀察的 SATISFIED 行為證據（引用專案輸出以節錄快照為證）。功能 AC 判定屬段內秘書判決（中鏈零審核）、驗收資格由時點 2 對抗查核（對抗在前、老闆判定在後），verify 邊核對 git 一致性；verify 段＝真驗收執行——GWT 逐條劇本實操，驗收依據＝行為是否發生，非測試燈號。
