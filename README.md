...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.1.0-2ea44f.svg" alt="version 1.1.0"/>
</p>
...
- **對抗檢閱三時點強制。** 計畫放行前強制子代理**對抗方向**、每個功能驗收後判決前強制**對抗成果**、收斂複驗強制**對抗成果**，記錄落 `tmp/review-plan|verify|converge-*.md`（含「對抗要點」與「複核結論」段）並經 CLI 閘門查核；層內連續、層間停靠——放行邊界先揭露後推進，不得無縫銜接。外部子代理不可用時，主對話切換身分以對抗者立場執行最嚴厲攻擊並標示降級來源，放行／判決／收斂時向老闆揭露。外援只提供輸入，不接管工作狀態或裁定；技術證據不足時另強制一次唯讀技術意見，其餘高風險情境按需取得。
