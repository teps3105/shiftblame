...
**核心思想：意圖揭露 + 先提出再質疑 + 強制展望收尾。** AI 的每步行動都必須先向老闆說明「我要做什麼、為什麼」，經確認後才執行。不是事後解釋，是事前揭露。管線採「先提出再質疑」模式——每個閘門至少正→反→收斂三輪辯論。驗收後還要強制經過展望行為，確認下一步方向後才收尾。
...
| 出錯難追 | **迭代收斂**：閘門 FAIL 以收斂產出為起始基線重跑 |
...
閘門 PASS 推進下出口，FAIL 以收斂產出為起始基線重跑（增量更新不重建）。
