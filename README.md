...
秘書動態掃描 agents 目錄，把正確的需求推給正確的部門。每個部門主管親自執行所有職能，出了事由秘書寫入各部門的鍋紀錄。所有修改透過 worktree 隔離，不直推 main。MIS 在部署階段完成專案內文件整理（REPO.md 重寫、README 同步）；部署完畢後，秘書負責常識提煉（跨專案 blame 整理）與物理清理（刪除部門產出檔案、worktree）。
