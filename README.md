...
**核心思想：意圖揭露 + 先提出再質疑 + G2 迭代提交。** AI 的每步行動都必須先向老闆說明「我要做什麼、為什麼」，經確認後才執行。不是事後解釋，是事前揭露。管線採「先提出再質疑」模式——每個閘門至少正→反→收斂三輪辯論。G2 為 NNN 迭代出口，收斂後提交；G3 產品驗收提供 G4 素材；G4 展望後收尾。小型變更使用簡易模式：完整正反子代理辯論，同樣須經意圖揭露確認，不開 slug，直接在 main 上 commit。
...
| 驗收通過就沒下文了 | **展望收尾**：G3 產品驗收後經 G4 展望更新文件、提交、收尾或開新 slug |
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/產品驗收(G3)-blue" alt="產品驗收"/>
  <img src="https://img.shields.io/badge/展望(G4)-blue" alt="展望"/>
</p>

```
計畫（G1）──→ 開發（G2）────↻ 可反覆開 NNN ──→ 產品驗收（G3）──→ 展望（G4）
  G1.md          G2.md                             G3.md            G4.md
    │               │                                │                │
    └─ FAIL → 以收斂為基線增量增加 ──────── 同左
                         │
                         └─ G2 PASS（老闆決定收尾）→ G3 → G4 PASS → 提交 → 收尾或開新 slug
```
...
老闆不通過以收斂為基線增量增加。G2 可反覆開 NNN，老闆決定收尾才推進 G3。G3/G4 不開新 NNN；若需更多開發則開新 slug。
...
| 7 | **迭代收斂** | 閘門 FAIL 以收斂為基線增量增加 |
| 8 | **前置建檔** | 每出口結束前須先建立下一出口文件（G2 迭代不視為出口結束） |
| 9 | **計畫語言** | 計畫使用 5W1H 格式；G3 以使用者視角體驗產品，產出為 G4 展望素材 |
| 10 | **NNN=Commit** | G2 為 NNN 迭代出口，每個 NNN 收斂後提交；G3/G4 不開新 NNN |
...
閘門 PASS 推進下出口，FAIL 以收斂為基線增量增加。

計畫使用 5W1H 格式（Who/What/When/Where/Why/How），開發使用 TDD 模式（紅燈→綠燈→重構），G3 以使用者視角體驗產品（產出為 G4 展望素材）。展望進行文件更新與提出下一步方向。簡易模式提供完整正反辯論（含 START 意圖揭露，不可跳過），用於不需 slug 的小型變更，直接在 main commit（僅 repo 檔案；.shiftblame/ 不入 repo）。
