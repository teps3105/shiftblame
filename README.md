...
實作前規劃（獨立階段：依複雜度定實作策略＋明確化 <complete>）
      │
      ▼
EXECUTOR 逐項實作 <task> 至 <complete>（每項自驗通過即 commit）→ 管理者揭露升級鏈選項→老闆選擇擔任者→獨立 code review＋e2e 證據審核 → 管理者揭露結果／未驗項 → 老闆指示推進下一 <nnn>
...
調度雙軌：正方主 session 身份切換（G1/G2）、反方開子代理獨立對抗；分別收斂雙軌、合併/歸檔。管線 commit 歸 EXECUTOR（<task> 項自驗通過即 commit）；MANAGER 保留收尾 merge。**不自行實作、不自審替代**——收斂後由 EXECUTOR 實作軌承接；post-EXECUTOR 管理者揭露升級鏈選項（見 GATE 複審①②）→ 老闆選擇擔任者 → 獨立 code review，審 e2e 證據／未驗項。
