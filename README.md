...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-0.8.1-2ea44f.svg" alt="version 0.8.1"/>
</p>
...
- **鏡像時序制衡。** 定義層（審計→研究→規劃）受一次定律約束定義「該做什麼」；執行層（測試/開發/驗收）是定義層的鏡像——測試(G3)→開發(G2)→驗收(G1) 反向收攏執行，在 G1 驗收處閉合。
...
- **測試先行（觸發重流程時）。** 執行層執行順序固定：測試階段寫測試（紅燈）→ 開發階段寫實作 → 驗收階段跑 CI 到綠燈。先寫測試再實作，與定義層「驗收先於實作」對齊。
...
flowchart TD
    ...
    subgraph Check["定義層三面向制衡 · 鏡像上半 · 一次定律：單向一次推進"]
        direction LR
