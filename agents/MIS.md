...
- L2（基本）：秘書初判確認需求 → RES → MIS(尾) → 秘書復判 → 歸檔。
- L3（標準）：秘書初判確認需求 → RES → PRD → DEV（可多輪）→ MIS(尾) → 秘書復判 → 歸檔。
- L4（完整）：秘書初判確認需求 → RES → QA → PRD → DEV（可多輪）→ QC → MIS(尾) → 秘書復判 → 歸檔。
- L5（高等）：秘書初判確認需求 → RES → SEC → QA → PRD → DEV（可多輪）→ QC → EXP → MIS(尾) → 秘書復判 → 歸檔。
...
### R16：四模式明確定位
> 四模式流程定義詳見 R2。以下為各模式所需文件的補充說明。
- L2（基本）：秘書初判確認需求 → RES → MIS(尾)。L2 模式文件：RES 部門報告 + MIS 部門報告(尾)。其中 MIS 部門報告(尾) 須包含「流程終點產出」全部項目（含 `.shiftblame/REPO.md` 差異報告）。
- L3（標準）：秘書初判確認需求 → RES → PRD → DEV（可多輪）→ MIS(尾)。L3 模式文件：RES 部門報告 + PRD 部門報告 + DEV 部門報告 + MIS 部門報告(尾)。
- L4（完整）：秘書初判確認需求 → RES → QA → PRD → DEV（可多輪）→ QC → MIS(尾)。L4 模式文件：RES 部門報告 + QA 部門報告 + PRD 部門報告 + DEV 部門報告 + QC 部門報告 + MIS 部門報告(尾)。
- L5（高等）：秘書初判確認需求 → RES → SEC → QA → PRD → DEV（可多輪）→ QC → EXP → MIS(尾)。L5 模式文件：RES 部門報告 + SEC 部門報告 + QA 部門報告 + PRD 部門報告 + DEV 部門報告 + QC 部門報告 + EXP 部門報告 + MIS 部門報告(尾)。

### R17：退回增量記錄
- L3/L4/L5 模式中，退回任意部門時，其部門文件應增量填寫（不替換），確保退回有紀錄。
- 退回後重新派工時，主執行者由 DISPATCH_CHECKLIST.md 步驟 13 動態調配重新選定。
- 退回紀錄格式：
  ```
  ## 退回紀錄
  - 退回來源：<部門名稱>
...
### R21：主執行者選定
主執行者由 DISPATCH_CHECKLIST.md 步驟 13 動態調配選定（依 onwatch 額度狀態自動決定）。老闆可透過 AskUserQuestion 表達意見（通用溝通機制，不限模式或部門）。結果寫入 task.md frontmatter 及 meta.md。不同部門可以有不同的主執行者。meta.md 記錄每輪派工的主執行者。
