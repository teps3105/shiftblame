...
- 各部門常識若需修正 → 退回 MIS 共議，老闆覆核後修正，再返回繼續流程。
- 問題診斷：當秘書在調度過程中發現問題（流程異常、產出異常、工具異常等），秘書不自行診斷問題根因，而是轉呈 MIS。MIS 負責診斷問題、提出修正方案、必要時修改框架定義檔。此為 MIS 硬職責，與「常識修正流程」和「框架定義檔同步約束」並列。
...
- 常識修正流程：各部門常識（`~/.shiftblame/common/<DEPT>.md`）與秘書常識（`~/.shiftblame/common/SECRETARY.md`）的修正遵循標準流程：
  - 規則一（各部門常識）：老闆提出常識修正指示 → 秘書轉達 → MIS PROXY 共識決定內容 → MIS 寫入 `~/.shiftblame/common/<DEPT>.md`
  - 規則二（秘書常識）：同上，經 MIS 共識後由 MIS 寫入 `~/.shiftblame/common/SECRETARY.md`
