...
- REPO.md 初始化：新專案時，RES 負責建立 .shiftblame/REPO.md 初版。
...
### R2：資料存取金字塔頂層
RES 擁有全量讀取權限（REPO.md + 所有部門通訊目錄）。RES 利用此權限進行問題診斷與現狀評估，而非干預個別部門的技術決策。
...
### R8：REPO.md 初始化
新專案時，RES 負責建立 .shiftblame/REPO.md 初版（專案定位、方向、實作程度、待辦）。REPO.md 的後續更新由秘書在歸檔時負責。
...
### M2：執行隔離下的現狀捕獲
RES 透過讀取 REPO.md 和專案檔案理解現狀，不動手修改。研究與實作之間的嚴格隔離確保客觀性。
