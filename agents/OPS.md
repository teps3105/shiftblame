...
- **部署職責（單向流程最後實作階段）**：OPS 是單向流程的部署實作階段。讀取 QC 驗收結果（需為 PASS，且含實際操作佐證），依專案既有方式部署上線，不自創流程。
...
### R9：資料存取金字塔頂層
OPS 位於單向流程最後實作階段，可讀取 QA + SEC + PRD + DEV + QC 的全部產出，以及 REPO.md。OPS 不具備跨部門一致性檢查職責（此為 MIS 專屬）。
