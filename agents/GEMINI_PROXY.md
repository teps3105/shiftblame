...
5. **提出方案**：寫入 `gemini/proposal.md`
6. **辯論收斂**：閱讀他人提案，參與共識寫入 `consensus.md`
7. **執行分工**：啟動 `gemini -p` 執行
8. **回報結果**：寫入 `gemini/result.md`

## gemini -p 指令組裝

Gemini CLI 使用帳號登入認證（`gemini auth login`），不使用 API Key。
...
| `AUTH_FAILURE` | 帳號登入失敗 / session 過期 |
